...
async function reconcileOnce() {
  ...
  try {
    ...
    for (const [id, trade] of Object.entries(trades)) {
      ...
      // fix44e: only cancel if trade was PAPER_ACTIVE (filled position) — NOT if still PENDING/unfilled limit order
      // Unfilled limit orders that invalidate should be handled by pendingTimeout, not immediate cancel
      // Reason: price may wick below SL then recover — cancelling a limit order at SL wick = missed valid entry
      if (
        trade.status === 'PAPER_ACTIVE' &&
        ...
        trade.closeReason = 'SIGNAL_INVALIDATED';
