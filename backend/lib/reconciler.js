...
function deriveStatus(trade, snapshot) {
  ...
  if (trade.status === 'PENDING' || trade.status === 'ORDER_PENDING_FILL') {
  ...
  try {
    ...
    for (const [id, trade] of Object.entries(trades)) {
      ...
      if (status.pos && !trade.tpslAttached && (trade.tp1 || trade.sl)) {
        ...
        trade.tpslStatus = stop.ok ? 'ATTACHED' : 'ATTACH_FAILED';
        trade.tpslResult = stop;
        if (!stop.ok) addLog('WARN', `TP/SL attach failed for ${trade.symbol}: ${stop.error || stop.data?.retMsg || 'unknown'}`, stop);
      }
