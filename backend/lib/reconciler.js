...
// fixWALLETCACHE (07/22): wallet-balance is the flakiest of the six snapshot calls (repeated
// "[bybit/wallet] timeout of 10000ms exceeded" in the Northflank log). A failed call used to
// return availableBalance:0 / walletBalance:0, so every consumer (UI wallet tile, equity snapshot,
// sizing sanity views) flashed zero for that cycle. The wallet moves slowly; the last good read is
// strictly better information than a fabricated 0. On failure serve the cache, marked stale, with
// the age — consumers can distinguish. Never cached across restarts (in-memory only), never used
// when there has been no successful read at all.
let _walletCache = null; // { data, at }
const WALLET_MIN_INTERVAL_MS = 60000; // fixWALLETTHROTTLE (07/22): reconcileOnce runs every 15s and
// was calling live wallet-balance every single cycle — the exact call timing out repeatedly in the
// Northflank log ("[bybit/wallet] timeout of 10000ms exceeded"). Balance doesn't move on a 15s
// cadence; positions/orders still refresh every cycle, only the wallet call itself is throttled to
// 60s, cutting live wallet calls 4x and easing whatever is causing the timeouts (rate limit or
// network path). Cache is served between throttled fetches — same shape consumers already handle.
async function getWallet() {
  if (_walletCache && (Date.now() - _walletCache.at) < WALLET_MIN_INTERVAL_MS) {
    return { ..._walletCache.data, ok: true };
  }
  const r = await safeCall('wallet', () => bybitGet('/v5/account/wallet-balance', { accountType: 'UNIFIED' }), null);
  if (!r.ok) {
    if (_walletCache) {
      return { ..._walletCache.data, ok: true, stale: true, staleSinceMs: Date.now() - _walletCache.at, staleError: r.error };
    }
    return { ok: false, error: r.error, availableBalance: 0, walletBalance: 0 };
  }
  ...
  _walletCache = { data: w, at: Date.now() };
  return w;
}
...
function findClosedForTrade(trade, closedList) {
  ...
  // fixFASTCLOSE (07/21 live): trade.filledAt is stamped Date.now() when the RECONCILER FIRST SEES
  // the position — not when Bybit actually filled it. A trade that opens and closes inside one
  // 15s poll cycle therefore has a closed-pnl row timestamped BEFORE its own recorded fill, and the
  // 5s skew window rejected it. Observed: USUSDT opened 19:00:03, closed 19:00:23, reconciler first
  // saw the position 19:00:38 -> closed row 15s "too old" -> match refused -> trade stranded in
  // ORDER_FILLED_NO_POSITION forever while Bybit showed a booked -0.373U loss.
  // The window only widens once the order is actually Filled/position-seen; a still-resting order
  // keeps the tight 5s anchor so it can never claim an older close. The entry-price discriminator
  // below (0.5%) remains the real guard against grabbing the wrong row.
  const _sawFill = String(trade.orderStatus || '').toLowerCase() === 'filled'
    || String(trade.positionStatus || '').toUpperCase() === 'OPEN'
    || String(trade.status || '') === 'POSITION_OPEN'
    || String(trade.status || '') === 'ORDER_FILLED_NO_POSITION';
  const SKEW_MS = _sawFill ? 5 * 60 * 1000 : 5000;
...
function deriveStatus(trade, snapshot) {
  ...
  if (pos) {
    // fixRISKCAP: POST-FILL RISK AUDIT. Size is solved before the fill; the SL is an absolute
    // structural price that never moves. If the fill lands adversely (or the SL was revised after
    // sizing), real risk = qty x |avgEntry - SL| can exceed the target with nothing to catch it.
    // Live 07/21: ONDO filled +0.343% off plan -> 1.54x stop distance -> -0.517U vs a 0.319U
    // target; LIT -0.497U vs 0.250U. Both were invisible until the loss was already booked.
    // This is an ALARM, not an intervention: closing or resizing a live position from the
    // reconciler is a far bigger hammer than the problem warrants, and the trade may still win.
    // Once-per-trade so it cannot spam the log every 15s cycle.
    if (!trade.riskAudited) {
      const _slAudit = num(trade.slPriceAtSizing) || num(trade.stopLoss);
      const _entryAudit = num(pos.avgPrice) || num(trade.avgEntryPrice);
      const _target = num(trade.riskTargetUsdt);
      const _qtyAudit = num(pos.size);
      if (_slAudit > 0 && _entryAudit > 0 && _target > 0 && _qtyAudit > 0) {
        const realRisk = _qtyAudit * Math.abs(_entryAudit - _slAudit);
        trade.realRiskUsdt = Number(realRisk.toFixed(4));
        trade.riskOvershootX = Number((realRisk / _target).toFixed(2));
        trade.riskAudited = true;
        const RISK_OVERSHOOT_ALARM_X = 1.2; // log when real risk exceeds this multiple of target
        if (realRisk > _target * RISK_OVERSHOOT_ALARM_X) {
          addLog('RISK_OVERSHOOT', `${trade.symbol}: real risk ${realRisk.toFixed(3)}U is ${(realRisk / _target).toFixed(2)}x the ${_target.toFixed(3)}U target (fill ${_entryAudit} vs SL ${_slAudit}) — position left open, review sizing`, { symbol: trade.symbol, realRisk, target: _target, avgEntry: _entryAudit, sl: _slAudit, qty: _qtyAudit });
        }
      }
    }
    trade.posMissStreak = 0;
    ...
    const POS_MISS_CONFIRM = 2; // consecutive CLEAN-cycle absences required; 1 = old single-cycle behaviour
    const _missNeeded = Math.max(1, POS_MISS_CONFIRM);
  ...
  if (trade.status === 'PENDING' || trade.status === 'ORDER_SUBMITTED' || trade.status === 'ORDER_PENDING_FILL') {
    ...
    const WAITING_LIMIT_TIMEOUT_MINUTES = 30; // decoupled from pendingTimeoutMinutes by design
    const _wltMin = Math.max(1, WAITING_LIMIT_TIMEOUT_MINUTES);
    ...
    for (const [id, trade] of Object.entries(trades)) {
      ...
      if (
        ...
        if (cancelRes.ok && !cancelRes.alreadyGone) {
          ...
          changed = true;
          // fixCANCELROW (07/22): bot-initiated cancels were INVISIBLE in bybit_ledger.ndjson.
          // The fix58 STATUS_CHANGE logger only fires on deriveStatus-detected transitions, but this
          // block mutates status AFTER deriveStatus ran, and next cycle isTerminalTrade() skips the
          // trade before deriveStatus can observe the exchange-side Cancelled — so no row is ever
          // written (live: PUMPFUN cancel 07/22 06:37, confirmed in app log, absent from the ledger;
          // ESPORTS' row that morning only exists because the USER cancelled it manually and
          // deriveStatus saw it next cycle). Forensics cross-referencing the two ledgers — which is
          // exactly how the last three bugs were found — needs these rows. Same fields as fix58.
          logBybitEvent('STATUS_CHANGE', {
            symbol: trade.symbol, side: trade.side, tradeId: id, orderId: trade.orderId,
            orderLinkId: trade.orderLinkId, signalId: trade.signalId,
            prevStatus: _statusBeforeDerive, status: 'CANCELLED',
            orderStatus: 'Cancelled', positionStatus: 'NONE',
            qty: num(trade.qty), cumExecQty: num(trade.cumExecQty),
            intendedEntry: num(trade.entry) || null,
            closeReason: trade.closeReason, cancelledBy: 'BOT_INVALIDATE',
          });
      ...
      // fixZOMBIE (07/22, live ESPORTSUSDT 5h43m): the flag-setter above (fixWLT30 block) marks
      // stalePending for PENDING | ORDER_SUBMITTED | ORDER_PENDING_FILL, but this — the block that
      // actually calls Bybit — only fired on the first two. A resting GTC limit gets its status
      // rewritten to ORDER_PENDING_FILL by deriveStatus on the very first cycle after placement, so
      // from cycle 2 onward the cancel condition could NEVER match. Result: every waiting limit that
      // does not fill lives forever on the exchange, the 30-min cap silently does nothing, and the
      // order can still fill hours later into a setup whose structure is long dead.
      // Cancel path is guarded by _unfilled (same rule as the invalidate-cancel above): if a fill
      // landed between cycles we never touch it — liveStateAuthority owns real positions. The
      // alreadyGone (110001) branch below is the second net for the same-tick race.
      // ROUND-3 HARDENING: _unfilled tests the filled-state list against _statusBeforeDerive, which
      // is the PREVIOUS cycle's value. In the fill-and-close-between-cycles case (real — the 20s
      // USUSDT close behind fixFASTCLOSE), deriveStatus has already written POSITION_CLOSED and
      // positionStatus 'CLOSED' this very cycle, yet _unfilled still computes true: the stale status
      // is ORDER_PENDING_FILL and 'CLOSED' !== 'OPEN'. Without the guard below we would fire a cancel
      // at a filled, closed, real trade. No corruption (alreadyGone refuses to force CANCELLED) but
      // it burns an API call and stamps cancelledByTimeout on a genuine position. So: re-test the
      // FRESH status too. The invalidate block above is far less exposed — it also needs
      // signalInvalidated — which is why the guard lives here and not there.
      const _freshUnfilled = !['POSITION_OPEN', 'POSITION_CLOSED', 'ORDER_FILLED_NO_POSITION', 'PARTIALLY_FILLED_NO_POSITION']
        .includes(String(trade.status || '').toUpperCase());
      if (
        (_statusBeforeDerive === 'PENDING' || _statusBeforeDerive === 'ORDER_SUBMITTED' || _statusBeforeDerive === 'ORDER_PENDING_FILL') &&
        _unfilled &&
        _freshUnfilled &&
        trade.stalePending &&
        ...
        if (cancelRes.ok && !cancelRes.alreadyGone) {
          ...
          trade.closedAt = Date.now();
          changed = true;
          // fixCANCELROW (07/22): mirror of the invalidate-branch row above — BOTH bot-cancel paths
          // must be visible in the ndjson ledger (see full rationale there).
          logBybitEvent('STATUS_CHANGE', {
            symbol: trade.symbol, side: trade.side, tradeId: id, orderId: trade.orderId,
            orderLinkId: trade.orderLinkId, signalId: trade.signalId,
            prevStatus: _statusBeforeDerive, status: 'CANCELLED',
            ...
            qty: num(trade.qty), cumExecQty: num(trade.cumExecQty),
            intendedEntry: num(trade.entry) || null,
            closeReason: 'PENDING_TIMEOUT', cancelledBy: 'BOT_TIMEOUT',
          });
        } else if (cancelRes.ok && cancelRes.alreadyGone) {
