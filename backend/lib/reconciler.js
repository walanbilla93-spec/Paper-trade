...
const { bybitGet, keySet } = require('./bybit');
...
function deriveStatus(trade, snapshot) {
  ...
  const closedBySymbol = new Map(snapshot.closedPnl.map(c => [c.symbol, c]));
  ...
  const closed = closedBySymbol.get(trade.symbol);
  ...
  if (openOrder) {
    ...
    if (!pos) trade.status = openOrder.cumExecQty > 0 ? 'PARTIALLY_FILLED' : 'PENDING';
  }
  ...
  if (histOrder) {
    ...
    if (['Cancelled', 'Rejected', 'Deactivated'].includes(histOrder.orderStatus)) {
      trade.status = histOrder.orderStatus.toUpperCase();
      ...
      trade.status = closed ? 'CLOSED' : 'FILLED_NO_POSITION';
    }
  }
  ...
  try {
    ...
    for (const [id, trade] of Object.entries(trades)) {
      ...
      if (status.pos && !trade.tpslAttached && (trade.tp1 || trade.sl)) {
        const stop = await setTradingStopSafe({ symbol: trade.symbol, tp1: trade.tp1, sl: trade.sl });
        trade.tpslSetAttemptedAt = Date.now();
        trade.tpslAttached = !!stop.ok;
