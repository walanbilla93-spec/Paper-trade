...
const DEFAULTS = Object.freeze({
  ...
  // fix48v: CONTEXT WEIGHTING (hypothesis test). When ON, convictionScore gets a contextMult =
  // per-pair alignment × market breadth × asymmetric fear. Default ON to collect the test data.
  // Flip CONTEXT_WEIGHTING=false to run a clean no-context A/B session. Fully reversible.
  contextWeighting: parseBool(process.env.CONTEXT_WEIGHTING, true),
  ...
  version: '4.6.8.48v' // fix48u: DEPRECATED as a source of truth — /health now reports V4_VERSION (live code). Kept aligned to avoid confusion.
});
