...
const DEFAULTS = Object.freeze({
  ...
  marketOrdersOnTestnet: parseBool(process.env.MARKET_ORDERS_ON_TESTNET, false), // fix47d: true→false per session defaults
  ...
  maxOpenTrades: parseInt(process.env.MAX_OPEN_TRADES || process.env.AUTO_TRADE_MAX_OPEN || '10', 10) || 10, // fix47d: 3→10 per session defaults
  ...
  minScoreToTrade: parseNumber(process.env.MIN_SCORE_TO_TRADE, 66), // fix47d: 55→66 per session defaults
  ...
  // Hypothesis: in a BTC-labelled bear, when market breadth is green (≥ minBullPct),
  // the regime label is wrong (composite is ~85% BTC-weighted, breadth muted to ±1) and
  // the bear buy_gate=85 BANS BUYs that breadth says are fine. When ON, relax the bear
  // BUY gate from 85 → BASE_SCORE_GATE (70) ONLY while breadth ≥ minBullPct. SELL untouched,
  // composite/label untouched (keeps logging bear for honest comparison). Every affected
  // BUY is CSV-tagged breadthOverrideAtEntry=true. Flip BREADTH_OVERRIDE_BUY=false to A/B.
  breadthOverrideBuy: parseBool(process.env.BREADTH_OVERRIDE_BUY, true), // default ON this session
  breadthOverrideMinBullPct: parseNumber(process.env.BREADTH_OVERRIDE_MIN_BULL_PCT, 65), // = bull-pressure trigger

  // fix49a: contextWeighting default flipped OFF — penalty curve too aggressive in bearish-breadth
  // markets; every signal took 13-22% score drag → dropped below minScoreToTrade=70 → ghost town.
  // Re-enable (CONTEXT_WEIGHTING=true) after recalibrating penalty strength in the next session.
  contextWeighting: parseBool(process.env.CONTEXT_WEIGHTING, false),
  ...
  v4BreadthChopBreaker: parseBool(process.env.V4_BREADTH_CHOP_BREAKER, true), // fix53: breadth-volatility circuit breaker, default ON
  bybitEventLog: parseBool(process.env.V4_BYBIT_EVENT_LOG, true), // fix58: temporary Bybit ground-truth event ledger, default ON
  ...
  version: '4.6.8.49a' // fix48u: DEPRECATED as a source of truth — /health now reports V4_VERSION (live code). Kept aligned to avoid confusion.
});
...
function clampSettings(s) {
  ...
  out.breadthOverrideBuy = parseBool(out.breadthOverrideBuy, DEFAULTS.breadthOverrideBuy); // fix48s
  out.breadthOverrideMinBullPct = Math.max(50, Math.min(100, parseNumber(out.breadthOverrideMinBullPct, DEFAULTS.breadthOverrideMinBullPct))); // fix48s: floor 50 so it can never relax in true bear breadth
  out.contextWeighting = parseBool(out.contextWeighting, DEFAULTS.contextWeighting); // fix48v
  ...
  // fix75: preset system removed — 'research' is the only mode. Kept as a stamped field for
  // journal-row schema continuity; any stored legacy value is coerced on next settings write.
  out.experimentPresetMode = 'research';
