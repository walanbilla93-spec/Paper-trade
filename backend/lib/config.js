...
const DEFAULTS = Object.freeze({
  ...
  // DATA: score>=65 gives 55.6% WR and good volume. Global fallback lowered from 84.
  minScoreToTrade: parseNumber(process.env.MIN_SCORE_TO_TRADE, 65),

  // DATA: score>=65 is the sweet spot. Was 48 (config) vs 60 (env conflict).
  v4MinBuyScore: parseNumber(process.env.V4_MIN_BUY_SCORE, 65),

  // Was 86 (env) vs 72 (config). 86 near-blocked all SELLs. 72 is correct.
  v4MinSellScore: parseNumber(process.env.V4_MIN_SELL_SCORE, 72),
  ...
  // JOURNAL: ETS=100 = 15.4% WR (13 trades, -2.78 PnL). Anti-predictive. Block it.
  maxEntryTimingScore: parseNumber(process.env.V4_MAX_ENTRY_TIMING_SCORE, 97),
  ...
  minEntryTimingScore: parseNumber(process.env.V4_MIN_ENTRY_TIMING_SCORE || process.env.MIN_ENTRY_TIMING_SCORE, 68),

  // DATA: 53 of 109 signals score exactly 68. Old env=85 blocked ALL of them.
  v4MinBuyEntryTimingScore: parseNumber(process.env.V4_MIN_BUY_ENTRY_TIMING_SCORE, 68),

  // Old env=92 — no signal ever reaches 92. Was a permanent SELL kill switch. Fixed to 70.
  v4MinSellEntryTimingScore: parseNumber(process.env.V4_MIN_SELL_ENTRY_TIMING_SCORE, 70),
  ...
  v4MaxTpAtr: parseNumber(process.env.V4_MAX_TP_ATR, 3.0),
...
function clampSettings(s) {
  ...
  out.minScoreToTrade = Math.max(0, Math.min(100, parseNumber(out.minScoreToTrade, DEFAULTS.minScoreToTrade))); // legacy global fallback — default now 65 (data-driven)
  ...
  out.minEntryTimingScore = Math.max(35, Math.min(100, parseNumber(out.minEntryTimingScore, DEFAULTS.minEntryTimingScore))); // legacy global fallback — default now 68 (data-driven)
