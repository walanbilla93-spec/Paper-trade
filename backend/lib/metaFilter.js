 ...
 */
// fixMETAFEED: HEALTH SELF-CHECK.
// The v8 deploy silently did nothing: ctx.k5 held 100 candles, the feature builder required 210,
// so it returned an EMPTY object on every call. An empty object scores 0.609 — above the 0.47
// threshold — so 75/75 signals passed and the filter appeared "loaded and working" in /health.
// Nothing errored. Nothing logged. Track feature coverage so this can never hide again.
function _trackCoverage(featureObj) {
  const m = _model;
  if (!m) return;
  let present = 0;
  for (const f of m.features) {
    const v = featureObj[f];
    if (v !== null && v !== undefined && v !== '' && Number.isFinite(Number(v))) present++;
  }
  _stats.lastCoverage = present + '/' + m.features.length;
  _stats.coverageSum = (_stats.coverageSum || 0) + present / m.features.length;
  const n = _stats.evaluated || 1;
  const avg = _stats.coverageSum / n;
  // warn once a minute if coverage is poor or nothing is ever blocked despite real volume
  const now = Date.now();
  if (now - (_stats.lastWarnAt || 0) > 60000) {
    if (avg < 0.5) {
      _stats.lastWarnAt = now;
      console.warn(`[metaFilter] LOW FEATURE COVERAGE ${(avg*100).toFixed(0)}% (${_stats.lastCoverage}) — the model is scoring mostly-missing inputs and its output is NOT meaningful. Check the candle supply feeding buildMetaFeatures.`);
    } else if (_stats.evaluated >= 50 && _stats.blocked === 0) {
      _stats.lastWarnAt = now;
      console.warn(`[metaFilter] ${_stats.evaluated} signals evaluated, ZERO blocked. Expected roughly 70-75% blocked. The filter is almost certainly not receiving real features.`);
    }
  }
}
...
function shouldTake(featureObj) {
  ...
  _stats.evaluated++;
  try { _load(); _trackCoverage(featureObj || {}); } catch (_e) {}
  let p = null;
  try {
    p = score(featureObj);
  } catch (e) {
...
function getStats() {
  const n = _stats.evaluated || 1;
  return { ..._stats,
    avgCoverage: _stats.coverageSum ? +(_stats.coverageSum / n).toFixed(3) : null,
    blockRate: _stats.evaluated ? +(_stats.blocked / _stats.evaluated).toFixed(3) : null,
    enabled: ENABLED, loaded: !!_model, loadError: _loadErr,
