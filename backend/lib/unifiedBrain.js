...
function applyDecision(signal, decision) {
  ...
  if (decision.state === L.STATES.CLOSED_WIN || decision.state === L.STATES.CLOSED_LOSS) {
    ...
    // fixCANCELSTATUS (07/21 regression from fixLIVESTATUS): mirroring LIVE_RESTING onto
    // status:'PENDING' was correct, but CANCELLED only refreshed stateReason — so a signal whose
    // order was cancelled (manually on Bybit, or by the stale-order sweep) kept status 'PENDING'
    // and every status-keyed view still called it waiting. Observed live: ESPORTS 20:14 and LA
    // 20:06 both read CANCELLED in paperState and PENDING in status. Terminal state needs a
    // terminal status, using the vocabulary the rest of the pipeline already treats as final.
    signal.status = 'STALE_CANCELLED';
    signal.result = signal.result || 'CANCELLED';
    signal.stateReason = 'unified: Bybit order cancelled/rejected — no position taken';
  }
  // fixLIVESTATUS: MIRROR LIVE STATES ONTO `status` TOO — the terminal states above were the only
  // ones written back, so a signal whose position Bybit had already FILLED kept whatever `status`
  // the paper path last set (usually 'DETECTED') while paperState correctly read LIVE_OPEN.
  // Everything that keys on `status` — the ACTIVE counter, the signals table, journal filters —
  // therefore showed open live positions as still waiting. Observed 07/21: three positions open on
  // Bybit (HANA, TREE, 1000BONK), all correctly LIVE_OPEN in paperState, two still status DETECTED,
  // UI reporting ACTIVE 0. Not a reconciliation failure — a write-back gap in the display field.
  // The stale waiting-phase stateReason is refreshed for the same reason (it read
  // "WAITING_ENTRY: price not in zone" on a filled position).
  else if (decision.state === L.STATES.LIVE_OPEN) {
    signal.status = 'ACTIVE';
    signal.stateReason = 'unified: Bybit position OPEN (live fill confirmed)';
  } else if (decision.state === L.STATES.LIVE_RESTING) {
    signal.status = 'PENDING';
    signal.stateReason = 'unified: limit order resting on Bybit — awaiting fill';
  } else if (decision.state === L.STATES.LIVE_PENDING) {
    signal.status = 'PENDING';
    signal.stateReason = 'unified: order submitted to Bybit — awaiting acknowledgement';
  }
