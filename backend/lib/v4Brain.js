...
let _pctStats = null;
function _pctReset() { _pctStats = { fresh: 0, fetched: 0, capped: 0, strong: 0, f4leaders: 0, overrides: 0, conflicts: 0, blocks: {} }; }
...
async function applyPerCoinTrend(leader, gate) {
  ...
  if (!regimeBlocked && !admitting) return; // decision doesn't hinge on the coin trend — skip the fetch
  ...
  const gate = shouldReject(leader, settings, signals);
    ...
    try {
      if (_pctStats && (_pctStats.fresh > 0 || _pctStats.strong > 0)) {
        ...
        addLog('PCT_SCAN', `fresh=${_pctStats.fresh} fetched=${_pctStats.fetched}${_pctStats.capped ? ` capped=${_pctStats.capped}` : ''} strong=${_pctStats.strong} f4leaders=${_pctStats.f4leaders} overrides=${_pctStats.overrides} conflicts=${_pctStats.conflicts} | strongBlocks: ${b}`);
      }
    } catch (_ps) {}
  }
}
