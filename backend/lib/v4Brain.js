...
const FIXCONFIRM_ENTRY_MARKET = true;
...
let _pctStats = null;
function _pctReset() { _pctStats = { fresh: 0, fetched: 0, capped: 0, strong: 0, f4leaders: 0, overrides: 0, conflicts: 0, scoreBypass: 0, blocks: {} }; }
...
async function coinTrendEngine(symbol) {
  try {
    ...
    if (!sym) return { bias: 'mixed', strength: 0, t1: '?', t3: '?', t5: '?', t15: '?', cross: null };
    ...
    const up = [t1, t3, t5].filter(t => t === 'bull').length;
    const dn = [t1, t3, t5].filter(t => t === 'bear').length;
    let bias = 'mixed', strength = 0;
    if (t15 === 'bull' && up >= 2)      { bias = 'up';   strength = 1 + up; } // 3 or 4
    else if (t15 === 'bear' && dn >= 2) { bias = 'down'; strength = 1 + dn; }
    ...
    const data = { bias, strength, t1, t3, t5, t15, cross };
    _coinTrendCache.set(sym, { at: Date.now(), data });
    return data;
  } catch (_e) {
    return { bias: 'mixed', strength: 0, t1: '?', t3: '?', t5: '?', t15: '?', cross: null };
  }
}
...
async function applyPerCoinTrend(leader, gate) {
  ...
  if (!regimeBlocked && !admitting && !bypassCandidate) return; // decision doesn't hinge on the coin trend — skip the fetch
  ...
  const gate = shouldReject(leader, settings, signals);
    ...
    try {
      if (_pctStats && (_pctStats.fresh > 0 || _pctStats.strong > 0)) {
        ...
        addLog('PCT_SCAN', `fresh=${_pctStats.fresh} fetched=${_pctStats.fetched}${_pctStats.capped ? ` capped=${_pctStats.capped}` : ''} strong=${_pctStats.strong} f4leaders=${_pctStats.f4leaders} overrides=${_pctStats.overrides} scoreBypass=${_pctStats.scoreBypass} conflicts=${_pctStats.conflicts} | strongBlocks: ${b}`);
      }
    } catch (_ps) {}
  }
}
