...
function shouldReject(leader, settings, signals = []) {
  ...
  // fix39: stale sentinel — warn only, no gate penalty (penalty was raising gate to 80+ blocking everything)
  const sentinelAgeMs = sent && sent.updatedAt ? Date.now() - sent.updatedAt : Infinity;
  const sentinelStale = sentinelAgeMs > 5 * 60 * 1000; // 5min threshold, warn only
  if (sentinelStale && sentimentReady) {
    warnings.push(`SENTINEL_STALE_${Math.round(sentinelAgeMs/1000)}s`);
  }

  // fix38b: Sentinel vs BTC MTF conflict detector
  // leader.btcRegime = regimeBrain label (BULL_TREND/BEAR_TREND etc)
  // sent.market_regime = sentinelBrain label (STRONG_BULL/BULL/CHOP/BEAR/STRONG_BEAR)
  // When they disagree → reduce confidence → raise score gate +15
  ...
  // fix39: removed conflictPenalty gate — calibratedScore already penalizes -20 for regime conflict.
  // Double-penalizing here (score -20 + gate +15) = gate mathematically unreachable. Warn only.
  ...
  // data-driven: RR>3.0 = 32% WR across 22 trades. Cap at 3.0.
...
function ingestCandidate(payload = {}) {
  ...
  // fix-rr-cap: apply same RR>3.0 cap to frontend candidates (ingestCandidate bypassed shouldReject)
