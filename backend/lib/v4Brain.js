...
const MAX_REJECTED_CANDIDATES = Math.max(50, Math.min(2000, parseInt(process.env.V4_MAX_REJECTED_CANDIDATES || '500', 10)));
...
const V4_VERSION = '4.6.4-senior-candidate-gate';
...
function getCandidateRejections() {
  const rows = store.read('v4_candidate_rejections', []);
  return Array.isArray(rows) ? rows : [];
}

function saveCandidateRejections(rows) {
  store.write('v4_candidate_rejections', (Array.isArray(rows) ? rows : []).slice(0, MAX_REJECTED_CANDIDATES));
}

function recordCandidateRejection(row = {}) {
  const now = Date.now();
  const item = {
    id: row.id || `rej_${now}_${Math.random().toString(36).slice(2, 8)}`,
    at: now,
    iso: new Date(now).toISOString(),
    ...row,
  };
  saveCandidateRejections([item, ...getCandidateRejections()]);
  return item;
}
...
function symbolLossLockoutReason(signals, leader) {
  ...
  const losses = (signals || [])
    .filter(s => s && (s.sym === leader.symbol || s.symbol === leader.symbol) && s.side === leader.side && s.paperState === 'SL_HIT')
    .map(s => num(s.closedAt || s.updatedAt || s.createdAt, 0))
    .filter(Boolean)
    .sort((a, b) => b - a);
  if (losses.length >= SYMBOL_SESSION_LOCKOUT_LOSSES) return `SYMBOL_${leader.symbol}_${leader.side}_SESSION_LOCKOUT`;
  if (losses.length >= 1 && now - losses[0] <= SYMBOL_LOSS_LOCKOUT_MS) return `SYMBOL_${leader.symbol}_${leader.side}_LOSS_LOCKOUT`;
  return '';
}
  ...
  try {
    ...
    for (const t of universe) {
      try {
        ...
        if (gate.ok && openSignalCount(signals) < settings.maxOpenTrades && canCreateForSymbol(signals, leader.symbol, settings)) {
    ...
    snapshot = {
      ...
      summary,
      btcRegime,
      recentRejected: getCandidateRejections().slice(0, 25),
      settings: {
...
function ingestCandidate(payload = {}) {
  const settings = currentSettings();
  let symbol = String(payload.symbol || payload.sym || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (symbol && !symbol.endsWith('USDT') && /^[A-Z0-9]{2,20}$/.test(symbol)) symbol += 'USDT';
  ...
  const stateText = String(payload.state || payload.plannerState || payload.status || '').toUpperCase();
  const candidateSource = String(payload.source || payload.captureSource || payload.candidateSource || '').toLowerCase();
  const reasons = [];
  const warnings = [];

  // v4.6.4 guard: only explicit modal/manual planner candidates are accepted.
  // This prevents the legacy background scanner from creating random frontend-candidate rows.
  if (!/modal|manual|planner/.test(candidateSource)) reasons.push('CANDIDATE_SOURCE_NOT_EXPLICIT');
  if (stateText && !stateText.includes('READY')) reasons.push(stateText.includes('WATCH') ? 'NOT_READY_WATCH_ONLY' : `NOT_READY_${stateText}`);
  ...
  let signals = getSignals();
  const duplicate = signals.some(s => s && (s.sym === symbol || s.symbol === symbol) && s.side === side && ACTIVE_STATES.has(s.paperState));
  if (duplicate && settings.oneTradePerSymbol !== false) reasons.push('ACTIVE_DUPLICATE_SYMBOL_SIDE');
  if (openSignalCount(signals) >= settings.maxOpenTrades) reasons.push('MAX_OPEN_TRADES_REACHED');

  const rejected = [...new Set(reasons)].length > 0;
  if (rejected) {
    const cleanReasons = [...new Set(reasons)];
    const rejection = recordCandidateRejection({
      symbol,
      side,
      score,
      entryTimingScore,
    ...
    addLog('V4_CANDIDATE_REJECTED', `${symbol || 'UNKNOWN'} ${side || 'NEU'} ${cleanReasons.slice(0,3).join(',')}`, {score, rr, slPct, btcRegime, warnings, source:candidateSource});
    return { ok: false, rejected: true, reasons: cleanReasons, warnings: [...new Set(warnings)], rejection };
  }
  ...
  const sig = {
    ...
    setupScore: score,
    entryTimingScore,
    tier: (payload.tier && payload.tier !== '-') ? payload.tier : tierFromScore(score),
    entry,
    sl,
    tp1,
    rr: Number(rr.toFixed(3)),
    rr1: Number(rr.toFixed(3)),
    margin,
    leverage,
    position,
    tp1Profit: Number(tpPct.toFixed(3)),
    slPct: Number(slPct.toFixed(3)),
    tp1ProfitUSDT: Number(grossTp.toFixed(4)),
    slLossUSDT: Number(grossSl.toFixed(4)),
    netTpUSDT: Number((grossTp - feeEst).toFixed(4)),
    netSlUSDT: Number((-(grossSl + feeEst)).toFixed(4)),
    feeEstUSDT: Number(feeEst.toFixed(4)),
    btcRegime: btcRegime || 'unknown',
    rejectWarnings: [...new Set(warnings)],
    reasons: Array.isArray(payload.reasons) ? payload.reasons.slice(0, 8) : [],
    risks: Array.isArray(payload.risks) ? payload.risks.slice(0, 8) : [],
    debug: payload.debug || '',
    candidateSource: candidateSource || 'frontend-modal-planner',
    planner: {
    ...
    entryHit: false,
    stateReason: 'V4.6.4: READY modal setup captured as WAITING_ENTRY; activation waits for entry timing/price zone',
    history: [{ at: now, state: 'WAITING_ENTRY', reason: 'Frontend modal READY candidate captured by backend; entry timing still gates activation' }],
    time: new Date(now).toLocaleTimeString().slice(0,5),
    createdAt: now,
    updatedAt: now,
  };
  signals.unshift(sig);
  signals = signals.sort((a,b)=>num(b.createdAt)-num(a.createdAt)).slice(0, MAX_SIGNALS);
  saveSignals(signals);
  const ledger = upsertLedger(signals);
  snapshot = { ...snapshot, ok: true, ts: now, version: V4_VERSION, signals, visibleSignals: signals.length, ledger: ledger.slice(0, RECENT_LEDGER_LIMIT), recentRejected: getCandidateRejections().slice(0, 25), summary: { ...computeLedgerSummary(ledger), visible: computeSummary(signals), source: 'v4_paper_ledger' } };
  store.write('v4_snapshot', snapshot);
  addLog('V4_CANDIDATE_CAPTURED', `${symbol} ${side} WAITING_ENTRY`, {score, rr, slPct, btcRegime, warnings, source:candidateSource});
  return { ok: true, signal: sig, summary: snapshot.summary, warnings: [...new Set(warnings)] };
}
...
function getSnapshot() {
  ...
  return {
    ...
    ledger: ledger.slice(0, RECENT_LEDGER_LIMIT),
    recentRejected: getCandidateRejections().slice(0, 25),
    summary: { ...computeLedgerSummary(ledger), visible: computeSummary(signals), source: 'v4_paper_ledger' },
    lastError
  };
}
...
function freshJournal() {
  const signalsBefore = getSignals();
  const ledgerBefore = upsertLedger(signalsBefore);
  saveSignals([]);
  saveLedger([]);
  saveCandidateRejections([]);
  snapshot = {
    ...
    ledger: [],
    recentRejected: [],
    summary: { ...computeLedgerSummary([]), visible: computeSummary([]), source: 'v4_paper_ledger' },
    btcRegime: snapshot.btcRegime || { regime: 'unknown' },
    lastError: ''
  };
  store.write('v4_snapshot', snapshot);
  return { ok: true, clearedSignals: signalsBefore.length, clearedLedger: ledgerBefore.length };
}
