  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') {
      ...
      if (timing.inZone && timing.directional) {
        ...
        changed = true;
        // fix44b: live execution hook — fire Bybit order when signal goes PAPER_ACTIVE
...
async function autoRetryRejections(currentSnapshot, settings) {
  ...
  for (const r of deduped) {
    ...
    const payload = r.rawPayload || (() => {
      // fix44b: estimate atr from plan sl distance — prevents TP_TOO_FAR gate bypass when atr=0
      const entry = r.plan?.entry || 0;
      const sl = r.plan?.sl || 0;
      const atrEst = entry && sl ? Math.abs(entry - sl) / 0.55 : 0; // reverse of atr*0.55=sl distance
      return {
  ...
  try {
    ...
    for (const t of universe) {
      try {
        ...
        // fix44a: skip alert removed — no longer collecting rejections
        ...
        if (gate.ok && openSignalCount(signals) < settings.maxOpenTrades) {
          ...
          if (_deadHour && !_highConfidence) {
            ...
            if (dupReason === 'DUPLICATE_SYMBOL_SIDE_WAITING') {
              ...
              // fix44b: live execution hook — fire Bybit order if signal created as PAPER_ACTIVE
...
function clearSignals({ archive = true, clearLedger = false } = {}) {
  ...
  store.write('trades', {}); // fix44b-hotfix: clear trade slots on session reset — paper trades were blocking live execution
...
function freshJournal() {
  saveSignals([]);
  saveLedger([]);
  store.write('trades', {}); // fix44b-hotfix: clear trade slots on fresh session
