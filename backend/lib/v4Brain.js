...
function shouldReject(leader, settings, signals = []) {
  ...
  if (targetAtr && targetAtr > maxTargetAtr * 1.10) reasons.push(`TP_TOO_FAR_${targetAtr.toFixed(2)}ATR_GT_${maxTargetAtr}`);
...
function ingestCandidate(payload = {}) {
  ...
  if (targetDistanceAtr && targetDistanceAtr > maxTargetAtr * 1.10) reasons.push(`TP_TOO_FAR_${targetDistanceAtr.toFixed(2)}ATR_GT_${maxTargetAtr}`);
