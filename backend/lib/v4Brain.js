...
let _breadthPctileCache = { at: 0, pctile: null, samples: 0, cur: null, hi: null, lo: null, btcRsi: null, composite: null, windowMin: 0 };
function breadthRangePctile() {
  const CACHE_MS = 60 * 1000;
  const now = Date.now();
  if (_breadthPctileCache.at && (now - _breadthPctileCache.at) < CACHE_MS) return _breadthPctileCache;
  const result = { at: now, pctile: null, samples: 0, cur: null, hi: null, lo: null, btcRsi: null, composite: null, windowMin: BOTTOM_WINDOW_MIN };
  try {
    ...
    if (pts.length >= BOTTOM_MIN_SAMPLES) {
      ...
      result.hi = Number(Math.max(...vals).toFixed(1));
      result.lo = Number(Math.min(...vals).toFixed(1));
      result.pctile = Number((100 * vals.filter(v => v < cur).length / vals.length).toFixed(1));
