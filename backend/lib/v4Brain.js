...
const V4_VERSION = '4.6.8.48v'; // fix48v: CONTEXT MULTIPLIER (hypothesis test). New contextMult on convictionScore = per-pair alignment (pair's own trend5/15/1h+localRegime, weighted highest) × market breadth (vol_bull_pct, secondary) × asymmetric fear (extreme F&G≤25 suppresses BUYs multiplicatively even when aligned; greed weighted weaker). Penalty leg calibrated to data (breadth-fighting=21% WR), boost leg gentle (unproven). Toggle contextWeighting (default ON). Records fearGreed/perPairAlign/contextMult. Carries 48s-48u.
...
function initActiveDiagnostics(s, entryPrice, atr, now, settings) {
  ...
  try {
    ...
    // fix48v: CONTEXT-MULTIPLIER tags. Records the live hypothesis inputs at entry so we can later
    // attribute outcome to per-pair alignment, market breadth, and fear independently.
    const _ctx = (s.scoring || s.scoreInfo)?.components?._context || null;
