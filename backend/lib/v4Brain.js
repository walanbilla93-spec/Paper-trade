...
const V4_VERSION = '4.6.8.43d'; // fix43d: ingestCandidate EXT/MOM/EW→warnings; trendFromCloses 0.2→0.5%; detectSwings >=→>; lockout clamp 180→240; WAITING_REFRESH_DELTA 0→5; saveLedger duplicate expiry removed; dead hour score-threshold ≥85 passes
...
function trendFromCloses(closes) {
  ...
  // fix43d: widened band 0.2%→0.5% — 0.2% was noise-level on volatile altcoins, many trending pairs returned 'neutral'
  if (price > slow && fast >= slow && price >= base * 0.995) return 'bull';
  if (price < slow && fast <= slow && price <= base * 1.005) return 'bear';
  return 'neutral';
}
...
function minRRFor(side, settings = currentSettings(), score = null) {
  // fix43: removed Math.max(2.0,...) hardcode — was overriding MIN_RR=1.8 env var and causing 1268 rejections.
  // Data that justified 2.0 floor mixed low/high score signals. Let score gate handle quality, not RR floor.
  const baseRR = Math.max(1.0, num(side === 'SELL' ? settings.v4MinSellRR : settings.v4MinRR, side === 'SELL' ? MIN_SELL_RR : MIN_RR));
  return baseRR;
}
...
// fix43b: removed UTC 17 (LK 22:30) — only 4 trades in sample, insufficient evidence.
// Keeping only UTC 1 (LK 06:30, 0% WR, confirmed) and UTC 19 (LK 00:30, 25% WR -3.14U, confirmed).
const BAD_SCAN_HOURS_UTC = new Set([1, 19]);
function isDeadHour() {
