...
const V4_VERSION = '4.6.8.66'; // fixMICRO: per-COIN funding + OI capture (fundingRate/openInterest already present on Bybit's /v5/market/tickers response, zero new API calls, previously dropped by the ticker mapper). Threaded end-to-end: fetchTickers -> ctx.ticker -> leader.coinFundingRate/coinOpenInterest (buildStructurePlan) -> signalFromLeader (explicit carry) -> s.diag.coinFundingRateAtEntry/coinOpenInterestAtEntry/fundingDivergenceAtEntry (capture block). fundingDivergenceAtEntry = this coin's funding minus market-wide funding: positive = LESS short-crowded than market average, negative = MORE (the squeeze-fuel signature). Pure data capture, ZERO behavior change -- no gate/score/direction logic reads these yet. Stage 1 of the standalone Direction Brain project: the walk-forward harness run against the existing ledgers found the macro+micro fusion underpowered because per-coin funding divergence did not exist in the data (micro layer was really just trend5). This lets it accumulate so the next walk-forward run has real per-coin data. Known gap: ingestCandidate() (manual/API path) does not yet carry these fields -- only the primary scanner path does. // v4.6.8.65 was fixADAPT: // fixADAPT (V4_ADAPT_ENABLED, default OFF) on top of fixFLIP: adaptive per-side regime from Orayan's OWN realized results — the only signal that generalized across 4 sessions (trailing-N SELL WR <35% → next-short 36% vs 52% healthy; every STATIC entry feature overfit). When a side's trailing-WINDOW win rate falls below threshold (min-sample guarded) that side is HOSTILE: (tilt1) detectFlip's volume bar is relaxed so more squeezing positions flip to the correct side, and (tilt2) new same-side scores get a soft ×0.85 dampen so marginal entries fall below the EXISTING minScore gate — not a new hard gate, self-relaxes when results recover. Both sides. Regime exposed via getAdaptiveRegime() + ADAPT_REGIME logs. // fixFLIP: wires the previously-DEAD detectFlip() into a 3-layer contrarian system. LAYER2 (V4_FLIP_WAITING, on when master on): a still-waiting SELL whose closed flip-TF bar (V4_FLIP_TF, default 1m — 5m was too wide) prints back above entry on a ≥1.5× volume spike is flipped to a fresh BUY limit-retest at the reclaimed level (symmetric for BUY via FAILED_BUY_SUPPORT). Pre-fill only: parent→terminal CONTRARIAN_FLIP, order cancelled, ZERO loss; child = fresh-lineage WAITING_ENTRY, flippedOnce guard, RR<1 skip, live-squeeze price guard. LAYER3 (V4_FLIP_ACTIVE, default OFF/opt-in — books mid-trade P&L): a FILLED underwater short that prints the same reclaim is cut at market (partial loss, not full SL) and reversed; paper-accounted like regime-flip-exit (no live close order — paper testing only). LAYER1 (V4_FLIP_AT_BIRTH, default OFF — OVERFIT): composite-based birth inversion looked 0W/15L on 2 squeeze sessions but 42% on the full 442-trade pool (sacrifices 13 winners; threshold doesn't generalize) — toggle only. BUY mirror is a proven asymmetry: 117 pooled BUYs show NO bull-trap failure (euphoria BUYs 53%), so structural detectFlip stays symmetric but composite BUY-inversion stays off. Every flip logs sentinelComposite. (.63 skipped: 6+3=9.) // fix62: /health now reports unifiedBrain:{enabled,bybitAuthorityCount,frozenCount} — V4_UNIFIED_BRAIN was invisible on /health before (behavior-only confirmation). fix61 (flag V4_UNIFIED_BRAIN, default OFF): single-authority lifecycle core + integration — order-bearing signals driven by reconciled Bybit truth, snapshot can no longer abandon a live position (BTW fix); real closedPnl into journal. fix60: closed-pnl symbol+time fallback. fix59: surface tradeId → fix57 live authority. — append-only ground-truth log (bybit_ledger.ndjson) of every Bybit event (executions incl. partial fills by execId; lifecycle transitions: opens, closes, cancellations w/ reason, rejects, invalidations) with real net P&L + summed fees + entry slippage + duration on close. Joins trade ledger + breadth log via orderLinkId/tradeId/signalId. No extra API calls. TEMPORARY: settings.bybitEventLog=false or V4_BYBIT_EVENT_LOG_DISABLE=1. GET /api/v4/bybit-ledger. Carries fix57 (live fill authority + divergence detector) + fix55 (breadth-fight). Pairs with fix56 executor.
...
const FINAL_STATES = new Set(['TP_HIT', 'SL_HIT', 'INVALIDATED', 'EXPIRED', 'REJECTED', 'STALE_CANCELLED', 'REGIME_FLIP_EXIT', 'CONTRARIAN_FLIP', 'ACTIVE_FLIP_EXIT']);

// fix52: DEAD-HOUR window in LK local time (Sri Lanka, UTC+5:30, no DST). Default 03:30–05:30 LK.
// fix47 ripped dead hours out entirely (isDeadHour() is a dead stub). Re-introduced here keyed to
// LK local time and — critically — enforced at FILL time, not signal-creation time. The Hr17 leak
// (24/06 analysis: dead block still showing real fills) was caused by creation-time-only gating: a
// signal created before the window could still FILL inside it. Gating the activation/fill instant
// closes that. Time-based ⇒ identical treatment for BUY and SELL by construction (symmetry rule).
// Tunable without a logic redeploy via env; set start==end to disable.
const LK_OFFSET_MIN = 330; // UTC+5:30
const DEAD_START_MIN = parseInt(process.env.V4_DEAD_START_MIN || '210', 10); // 210 = 03:30 LK
const DEAD_END_MIN   = parseInt(process.env.V4_DEAD_END_MIN   || '330', 10); // 330 = 05:30 LK
...
function isDeadHourLK(now = Date.now()) {
  if (!(DEAD_END_MIN > DEAD_START_MIN)) return false; // disabled when start>=end
  const mod = lkMinuteOfDay(now);
  return mod >= DEAD_START_MIN && mod < DEAD_END_MIN;
}
function deadHourLabel() {
  const fmt = (m) => `${String(Math.floor(m / 60)).padStart(2, '0')}:${String(m % 60).padStart(2, '0')}`;
  return `LK ${fmt(DEAD_START_MIN)}–${fmt(DEAD_END_MIN)}`;
}
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') {
      ...
      if (timing.inZone && timing.directional) {
        ...
        if (_liveMode57 && String(process.env.V4_LIVE_ACTIVATION_BYBIT_ONLY || 'true').toLowerCase() !== 'false') {
          if (!s._autoTraded && !s.liveOrderId && !s.tradeId) {
            ...
            if (_liveCount57 >= (settings.maxOpenTrades || 3)) {
              ...
              s._autoTraded = true;
              require('./executor').executeSignal({ ...s, plan: s.plan || { entry: s.entry, sl: s.sl, tp1: s.tp1 }, leverage: settings.leverage || 5, margin: settings.maxTradeUsdt || 5 }, settings)
                .then(r => {
                  if (r && r.ok) {
          ...
          if (s.paperState !== 'WAITING_ENTRY' || s.displayState !== 'LIVE_PENDING_FILL') {
            ...
            s.positionStatus = 'NONE';
            s.entryHit = false;
            s.stateReason = `${V4_VERSION}: LIVE — entry reached, order working on Bybit; ACTIVE only on confirmed fill`;
