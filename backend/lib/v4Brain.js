...
async function applyPerCoinTrend(leader, gate) {
  ...
  if (!regimeBlocked && !admitting && !bypassCandidate && !breadthFightBlocked) return; // decision doesn't hinge on the coin trend — skip the fetch
  ...
  if (breadthFightBlocked && withSide) {
    ...
    leader.breadthFightShadow = {
      side, entry: leader.entry, sl: leader.sl, tp1: leader.tp1,
    ...
    try {
      addLog('BREADTH_FIGHT_SHADOW',
        `${leader.symbol}: ${side} would-be entry=${leader.entry} sl=${leader.sl} tp=${leader.tp1} rr=${num(leader.math && leader.math.rr, 0).toFixed(2)} score=${leader.score} — blocked by ${bfReason}, coin trend ${ct.bias} str${ct.strength} (1m=${ct.t1} 3m=${ct.t3} 5m=${ct.t5} 15m=${ct.t15}${ct.cross ? ' ' + ct.cross : ''})`
      );
    } catch (_l) {}
  }
