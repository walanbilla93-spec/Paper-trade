...
function buildMetaFeatures(ctx, k5, side, extFeat, price) {
  ...
  try {
    ...
    if (closes.length < 210) return out;   // not enough history -> all-default, fails open
    ...
    const rets = [];
    for (let i = n - 120; i < n; i++) if (i > 0 && closes[i] > 0 && closes[i - 1] > 0) rets.push(Math.log(closes[i] / closes[i - 1]));
    const sigma = sd(rets);
    if (!(sigma > 0)) return out;

    const s21 = smaN(21), s50 = smaN(50), s200 = smaN(200);
    out.side_num = s;
    out.is_buy = s === 1 ? 1 : 0;
    out.ext21 = s * (lc(px) - lc(s21)) / sigma;
    out.ext50 = Number.isFinite(extFeat && extFeat.ext50) ? extFeat.ext50 : s * (lc(px) - lc(s50)) / sigma;
    out.ext200 = s * (lc(px) - lc(s200)) / sigma;
    ...
    out.mom1 = mom(1); out.mom3 = mom(3); out.mom6 = mom(6); out.mom12 = mom(12); out.mom48 = mom(48);
