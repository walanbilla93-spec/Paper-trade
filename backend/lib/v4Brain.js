...
const V4_VERSION = '4.6.8.48w'; // fix48w: CAPTURE-PATH FIX for 48v contextDiag. signalFromLeader is an explicit literal — scoreInfo.components._context never survived to the tracked signal, so all 48v context fields landed null. Fix: store contextDiag directly on leader after convictionScore (and after flip rewrite), copy explicitly in signalFromLeader + ingestCandidate, read s.contextDiag at capture. NO scoring-logic change. Carries 48v contextMult + all prior fixes.
