...
const V4_VERSION = '4.6.8.53'; // fix53 (combined, includes fix52): BREADTH-VOLATILITY CIRCUIT BREAKER — pauses NEW fills (both sides) when trailing-60min breadth range ≥ threshold (default 25, env V4_BREADTH_CHOP_THRESHOLD); holds setups in WAITING_ENTRY (does not kill), fails OPEN on sparse data, min-hold to avoid flapping. Calibrated on 241 trades: removes the 06-25→06-26 cluster's 9 whipsaw losers (0W/9L, −3.06U), lifts in-window WR 39%→48%, net +3.14U→+6.21U. Also the SENTINEL-LAG reflex: overrides the stale STRONG_BEAR side-permission during whipsaw WITHOUT touching the 4-read hysteresis (label stays stable, reflex pauses). Toggle v4BreadthChopBreaker (default ON). Surfaced in snapshot.breadthChop + /health. CARRIES fix52: (1) PERMANENT decay-proof stats accumulator (pnl_permanent). (2) /pnl + summary.permanent all-time net/WR + 7d. (3) DEAD-HOUR FILL BLOCK at activation (LK 03:30–05:30, paper-only). (4) RECENT_LEDGER_LIMIT 250→120. Carries 49l breadth-history logger + DATA_DIR diagnostic.
