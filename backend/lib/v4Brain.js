...
const MAX_TP_ATR = Math.max(1.0, Number(process.env.V4_MAX_TP_ATR || '2.4'));
const RISK_WINDOW_HOURS = Math.max(1, Number(process.env.V4_RISK_WINDOW_HOURS || '6'));
const MAX_CONSECUTIVE_LOSSES = Math.max(1, parseInt(process.env.V4_MAX_CONSECUTIVE_LOSSES || '5', 10));
const MAX_SESSION_LOSS_USDT = Math.max(0, Number(process.env.V4_MAX_SESSION_LOSS_USDT || '5.0'));
...
function currentSettings() {
  ...
  return {
    ...
    v4MinBuyEntryTimingScore: Math.max(35, Math.min(100, num(s.v4MinBuyEntryTimingScore, 85))),
