...
const V4_VERSION = '4.6.7-architecture-entry-hardening';
...
function signalFromLeader(leader, settings) {
  ...
  return {
    ...
    updatedAt: now,
    expireMs: EXPIRY_MS,
    stateReason: state === 'PAPER_ACTIVE' ? `V4.6.7: ${timing.reason}` : `V4.6.7: ${timing.reason}`,
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY') {
      ...
      if (timing.inZone && timing.directional) {
        ...
        s.entryHit = true;
        s.openedAt = now;
        s.stateReason = 'V4.6.7: ' + timing.reason;
...
function ingestCandidate(payload = {}) {
  ...
  const sig = {
    ...
    source: 'frontend-candidate-v4.6.7',
    plannerVersion: V4_VERSION,
    sym: symbol,
    symbol,
    side,
    ...
    entryHit: false,
    stateReason: 'V4.6.7: READY setup captured as WAITING_ENTRY; activation requires tight entry zone + directional reaction',
...
function freshJournal() {
  ...
  store.write('legacy_browser_signals', { at: Date.now(), total: 0, note: 'cleared by v4.6.7 freshJournal' });
  snapshot = { ...snapshot, ts: Date.now(), signals: [], visibleSignals: 0, ledger: [], summary: computeLedgerSummary([]) };
  store.write('v4_snapshot', snapshot);
  return { ok: true, cleared: true, source: 'v4.6.7 freshJournal' };
}
