...
const ENTRY_CONFIRMATION_WINDOW_MS = Math.max(1000, Number(process.env.V4_ENTRY_CONFIRMATION_WINDOW_SECONDS || '90') * 1000);
...
const V4_VERSION = '4.6.8.42'; // fix42: corrected UTC bad hours (1,14,17,19); SL hard cap >2.5% blocked; retry source tag fixed
...
function minRRFor(side, settings = currentSettings(), score = null) {
  // data-driven fix: RR<2.0 = 40% WR, -1.93 net on backend (151 trades).
  // RR 2.0-2.5 = 65-68% WR sweet spot for both sources.
  // Raise hard floor to 2.0 regardless of score.
...
function shouldReject(leader, settings, signals = []) {
  ...
  // data-driven: RR>3.0 = 32% WR. Cap at 3.0.
  if (leader.math.rr > 3.0) reasons.push('RR_GT_3_CAPPED');
  // data-driven: LOCAL_BEAR = 0% WR, -3.39 net (151 trades, both sources).
  // Block all signals when local pair regime is bearish — price structure fighting the trade.
  const _localR = String(leader?.scoreInfo?.localRegime || leader?.plan?.localRegime || '').toUpperCase();
  if (_localR === 'LOCAL_BEAR') reasons.push('LOCAL_BEAR_BLOCKED');
  ...
  const _sentRegime = String(sent && sent.market_regime || '').toUpperCase();
  const _isBullRegime = _sentRegime === 'STRONG_BULL' || _sentRegime === 'BULL';
  ...
  if (settings.rejectMomentumConflict) {
    ...
    // fix36: normalize both regimeBrain and sentinelBrain label spaces
...
// fix42: corrected BAD_SCAN_HOURS_UTC — previous values (6,7,23,0) were derived from Sri Lanka local time
// but getUTCHours() runs on UTC. CSV timestamps are SL local (UTC+5:30).
// Recalculated from big sample (118 trades) in actual UTC:
// UTC 01 = 0% WR (SL 06:30) | UTC 19 = 25% WR -3.14U (SL 00:30) | UTC 17 = 38% WR -1.31U (SL 22:30)
// UTC 14 = 25% WR -0.78U (SL 19:30) — borderline, included for safety.
// Existing open signals are still managed (SL/TP tracking continues). Only NEW signal generation blocked.
const BAD_SCAN_HOURS_UTC = new Set([1, 14, 17, 19]);
function isDeadHour() {
  ...
  try {
    ...
    for (const t of universe) {
      try {
        ...
        if (gate.ok && openSignalCount(signals) < settings.maxOpenTrades) {
          // fix-timegate: dead hour — don't open new positions, log as rejection
          if (_deadHour) {
            leader.rejected = true;
...
function ingestCandidate(payload = {}) {
  ...
  if (isDeadHour()) reasons.push('DEAD_HOUR_NO_NEW_ENTRIES');
  ...
  if (rr > 3.0) reasons.push('RR_GT_3_CAPPED');
  ...
  if (_ingLocalR === 'LOCAL_BEAR') reasons.push('LOCAL_BEAR_BLOCKED');
  const riskBlock = globalRiskBlockReason(signals, settings);
  if (riskBlock) reasons.push(riskBlock);
...
function start() {
  ...
  try {
    ...
    const cleaned = raw.map(s => {
      ...
      const isOldVersion = src.includes('backend-planner') && !src.includes(V4_VERSION);
      if (age > expMs || isOldVersion || tsRaw === 0) {
        purged++;
...
async function manualRetryRejections({ maxAgeMinutes = 240, minScore = 60 } = {}) {
  ...
  for (const r of deduped) {
    ...
    // fix42: override stale btcRegime with current live regime — rejection may be hours old
    payload.btcRegime = getBTCRegime()?.regime || payload.btcRegime || '';
    const result = ingestCandidate(payload);
