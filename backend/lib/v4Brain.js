...
async function autoRetryRejections(currentSnapshot, settings) {
  ...
  // fix44: skip any symbol+side that already has a live WAITING or ACTIVE signal
  ...
  for (const r of deduped) {
    ...
    const payload = r.rawPayload || {
      ...
      entryTimingScore: r.entryTimingScore || 0, // fix42: was 68 (frontend default) — use 0 if unknown
      source: `backend-retry-${V4_VERSION}`,     // fix42: was missing — retried signals were tagged as frontend
...
// fix43e: added UTC 11 (LK 17:00, 31% WR confirmed) and UTC 21 (LK 03:00, 21% WR confirmed)
// Both confirmed across multiple sessions. UTC 1 and 19 remain from fix43b.
const BAD_SCAN_HOURS_UTC = new Set([1, 11, 19, 21]);
function isDeadHour() {
...
async function scanOnce() {
  ...
  try {
    ...
    const _scanRejectionBatch = []; // fix-skip-batch: collect rejections, write once at end of scan
    ...
    // fix-timegate: skip new signal generation in dead hours — only manage existing open signals
    const _deadHour = isDeadHour();
    if (_deadHour) {
      addLog('SCAN_SKIP', `Dead hour (${new Date().getUTCHours()}:xx UTC) — managing open signals only, no new entries`);
    }
    ...
    for (const t of universe) {
      try {
        ...
        // fix-skip-batch: collect rejection in memory — write once at end of scan (was per-signal, caused CPU spike)
        if (leader.rejected && leader.rejectReasons && leader.rejectReasons.length) {
          _scanRejectionBatch.push({
            at: Date.now(), iso: new Date().toISOString(),
            symbol: leader.symbol, side: leader.side,
            score: leader.score, rr: leader.math?.rr || 0,
            slPct: slDistancePct(leader), btcRegime: leader.btcRegime || '',
            reasons: leader.rejectReasons, warnings: leader.rejectWarnings || [],
            source: `scan-${V4_VERSION}`, plan: leader.plan || {}, math: leader.math || {},
          });
        }

        if (gate.ok && openSignalCount(signals) < settings.maxOpenTrades) {
          // fix43d: dead hour now score-threshold based — score>=85 passes even in dead hours
          // Full block was too aggressive; high-conviction setups shouldn't be wasted on time-of-day
          const _highConfidence = leader.score >= 85;
          if (_deadHour && !_highConfidence) {
            leader.rejected = true;
            leader.rejectReasons = ['DEAD_HOUR_NO_NEW_ENTRIES'];
          } else {
    ...
    lastError = '';
    // fix-skip-batch: single store write for all rejections collected this scan cycle
    if (_scanRejectionBatch.length > 0) {
      try {
        const _existing = store.read('v4_candidate_rejections', []);
        store.write('v4_candidate_rejections',
          [..._scanRejectionBatch, ...(Array.isArray(_existing) ? _existing : [])].slice(0, 1000)
        );
...
function ingestCandidate(payload = {}) {
  ...
  // fix-timegate: block frontend candidates in dead hours — same rule as backend scan
  // fix43d: score>=85 passes through dead hours (high conviction shouldn't be wasted)
  if (isDeadHour() && score < 85) reasons.push('DEAD_HOUR_NO_NEW_ENTRIES');
