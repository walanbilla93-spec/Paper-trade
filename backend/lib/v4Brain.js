...
const V4_VERSION = '4.6.3-candidate-score-hotfix';
...
function ingestCandidate(payload = {}) {
  ...
  const sig = {
    ...
    source: 'frontend-candidate-v4.6.3',
    plannerVersion: V4_VERSION,
    sym: symbol,
    symbol,
    side,
    ...
    entryHit: false,
    stateReason: 'V4.6.3: READY setup captured as WAITING_ENTRY using setup confidence; activation waits for entry timing confirmation',
    history: [{ at: now, state: 'WAITING_ENTRY', reason: 'Frontend READY candidate captured by backend; entry timing still gates activation' }],
    time: new Date(now).toLocaleTimeString().slice(0,5),
    createdAt: now,
    updatedAt: now,
  };
