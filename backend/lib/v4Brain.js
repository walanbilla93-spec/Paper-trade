...
function convictionScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  if (_contestedBull) {
    ...
    // fix49j(J): BREADTH-OVERRIDE-BUY (default OFF). Mirror of the btcRegimeDecision unblock.
    // A BUY opposed by a BEAR btc label, but breadth is strongly bull (>= threshold): remove the
    // counter-trend PENALTY (floor to neutral 0.90). We do NOT add a bonus — the hypothesis under
    // test is "removing the regime veto lets good breadth-aligned BUYs through", not "breadth
    // deserves a reward". Every affected signal is tagged for isolated analysis.
    if (side === 'BUY' && String(process.env.V4_BREADTH_OVERRIDE_BUY || 'false').toLowerCase() === 'true') {
      try {
        const _sJ = (typeof sentinel?.getSentinel === 'function') ? sentinel.getSentinel() : null;
        const _pairsJ = num(_sJ?.vol?.pair_count, 0);
        const _pctJ = _pairsJ > 0 ? num(_sJ?.vol?.bull_pct, NaN) : NaN;
        const OVRJ = Math.max(50, Math.min(100, Number(process.env.V4_BREADTH_OVERRIDE_PCT || '70')));
        if (Number.isFinite(_pctJ) && _pctJ >= OVRJ && regimeMult < 0.90) {
          regimeMult = 0.90; // penalty removed, no bonus
          regimeNote = `BREADTH_OVERRIDE_BUY: breadth ${_pctJ.toFixed(0)}% bull >= ${OVRJ} — bear-regime penalty neutralized (experiment)`;
          risks.push(regimeNote);
          components._breadthOverrideBuy = { breadthPct: Number(_pctJ.toFixed(1)), threshold: OVRJ, btcRegime: btcR };
        }
...
function btcRegimeDecision(leader, settings, btcUnknownException = false) {
  ...
  if (side === 'BUY' && BEAR_REGIMES.includes(regime)) {
    // fix49j(J): BREADTH-OVERRIDE-BUY experiment (default OFF). If breadth is strongly bull
    // (>= V4_BREADTH_OVERRIDE_PCT) we treat the bear LABEL as untrustworthy for the BUY side and
    // do NOT hard-block — letting us test whether breadth supersedes regime. The scoring side of
    // this (penalty neutralization + tag) lives in convictionScore. Mirror of the H2 SELL gate.
    let _breadthUnblock = false;
    if (String(process.env.V4_BREADTH_OVERRIDE_BUY || 'false').toLowerCase() === 'true') {
      try {
        const _s = (typeof sentinel?.getSentinel === 'function') ? sentinel.getSentinel() : null;
        const _pairs = num(_s?.vol?.pair_count, 0);
        const _pct = _pairs > 0 ? num(_s?.vol?.bull_pct, NaN) : NaN;
        const OVR = Math.max(50, Math.min(100, Number(process.env.V4_BREADTH_OVERRIDE_PCT || '70')));
        if (Number.isFinite(_pct) && _pct >= OVR) _breadthUnblock = true;
      } catch (_e) { /* fail-safe: no override, normal block applies */ }
    }
    if (_breadthUnblock) {
      warnings.push(`BREADTH_OVERRIDE_BUY_UNBLOCK_${regime}`); // experiment: bear label ignored due to strong breadth
...
function shouldReject(leader, settings, signals = []) {
  ...
  try {
    ...
    if (_bUsable) {
      const BUY_MIN_BREADTH  = Math.max(0, Math.min(50, Number(process.env.V4_BUY_MIN_BREADTH  || '30')));
      const SELL_MAX_BREADTH = Math.max(50, Math.min(100, Number(process.env.V4_SELL_MAX_BREADTH || '70')));
      if (_sideUp === 'BUY') {
        if (_bPct <= BUY_MIN_BREADTH) reasons.push(`BREADTH_FIGHT_BUY_${_bPct.toFixed(0)}PCT_LE_${BUY_MIN_BREADTH}`);
        else if (_bPct <= 40) warnings.push(`BREADTH_SOFT_BUY_${_bPct.toFixed(0)}PCT`);
      } else if (_sideUp === 'SELL') {
        if (_bPct >= SELL_MAX_BREADTH) reasons.push(`BREADTH_FIGHT_SELL_${_bPct.toFixed(0)}PCT_GE_${SELL_MAX_BREADTH}`);
        else if (_bPct >= 60) warnings.push(`BREADTH_SOFT_SELL_${_bPct.toFixed(0)}PCT`);
      }
    } else {
...
function initActiveDiagnostics(s, entryPrice, atr, now, settings) {
  ...
  try {
    ...
    try {
      ...
      // fix49j(J): tag whether the breadth-override-BUY experiment touched this signal's score
      s.diag.breadthOverrideBuyApplied = !!(s.scoring?.components?._breadthOverrideBuy ||
                                            s.scoreInfo?.components?._breadthOverrideBuy);
