...
const V4_VERSION = '4.6.8.48d'; // fix48d: WAITING hard-cap 45min, ENTRY_TIMING=100 reverts at activation, timingScore persisted, rich diagnostic capture (MFE/MAE/regime path) for adaptive analysis
...
const FINAL_STATES = new Set(['TP_HIT', 'SL_HIT', 'INVALIDATED', 'EXPIRED', 'REJECTED', 'STALE_CANCELLED']);
...
// Old design had 6+ cascading Math.min() caps causing almost all signals to score 70
// regardless of quality (confirmed in fix47d data: 50/72 signals = exactly 70).
// New design: clean 0-100 with three additive buckets. No score caps.
//
//   BUCKET 1: Family agreement (0-60)  — how many signal families agree?
//     3 families agree  → 60 pts
//     2 families agree  → 40 pts (minimum to fire a signal)
//   BUCKET 2: Entry quality   (0-15)  — where is price vs the planned entry zone?
//   BUCKET 3: Regime alignment (0-25) — does BTC macro regime support this trade?
//
// Hard reject is handled by shouldReject (score < minScore, RR gate, etc).
// No internal caps — score must reflect real conviction.
function convictionScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  // ── BUCKET 1: Family agreement (0–60) ─────────────────────────────────────
  // decision.edge = number of families that voted for the winning side (2 or 3)
  const familiesAgreed = num(decision.edge, 0);
  if (familiesAgreed >= 3) {
    components.family = 60;
    confluence.push('3-family consensus: Elliott + structure + trend');
  } else if (familiesAgreed >= 2) {
    components.family = 40;
    confluence.push(`2-family consensus: ${(decision.reasons || []).join(', ')}`);
  } else {
    components.family = 0;
  }

  // ── BUCKET 2: Entry quality (0–15) ────────────────────────────────────────
  // fix43e data confirmed: tight entries (≤0.25 ATR) = 16% WR; zone-touch (0.5-1.0 ATR) = 51% WR.
  // fix48: ≤0.25 ATR now also blocked in shouldReject (ENTRY_ALREADY_AT_ZONE).
  if (entryDistanceAtr <= 0.25) {
    components.entry = 0;  // breakout/momentum chase — blocked in shouldReject too
    risks.push('entry already at zone — tight stop, noise risk');
  } else if (entryDistanceAtr <= 0.50) {
    components.entry = 10;
    confluence.push('entry close to planner zone');
  } else if (entryDistanceAtr <= 1.00) {
    components.entry = 15;  // best: price pulled back to zone
    confluence.push('entry in zone-touch sweet spot');
  } else if (entryDistanceAtr <= 1.80) {
    components.entry = 5;
    risks.push('entry stretched from current price');
  } else {
    components.entry = 0;
    risks.push('entry too far from current price');
  }

  // ── BUCKET 3: Regime alignment (0–25) ─────────────────────────────────────
  ...
  if (side === 'BUY') {
    switch (btcR) {
      case 'BULL_TREND': regimeAlignPts = localRegime === 'LOCAL_BEAR' ? 15 : 25; regimeNote = 'BTC bull trend aligns BUY'; break;
      case 'BREAKOUT':   regimeAlignPts = 22; regimeNote = 'BTC breakout — BUY directional'; break;
      case 'BULL_RANGE': regimeAlignPts = localRegime === 'LOCAL_BULL' ? 18 : 12; regimeNote = 'BTC bull range — BUY at support'; break;
      case 'BEAR_RANGE': regimeAlignPts = 6;  regimeNote = 'BTC bear range — BUY weak'; break;
      case 'BEAR_TREND': regimeAlignPts = 0;  risks.push('BTC bear trend opposes BUY'); break;
      case 'CHOP':       regimeAlignPts = 0;  risks.push('BTC chop — BUY unreliable'); break;
      default:           regimeAlignPts = 6;  risks.push('BTC regime unknown'); break;
    }
  } else {
    switch (btcR) {
      case 'BEAR_TREND': regimeAlignPts = localRegime === 'LOCAL_BULL' ? 15 : 25; regimeNote = 'BTC bear trend aligns SELL'; break;
      case 'BREAKOUT':   regimeAlignPts = 22; regimeNote = 'BTC breakout — SELL directional'; break;
      case 'BEAR_RANGE': regimeAlignPts = localRegime === 'LOCAL_BEAR' ? 18 : 12; regimeNote = 'BTC bear range — SELL at resistance'; break;
      case 'BULL_RANGE': regimeAlignPts = 6;  regimeNote = 'BTC bull range — SELL weak'; break;
      case 'BULL_TREND': regimeAlignPts = 0;  risks.push('BTC bull trend opposes SELL'); break;
      case 'CHOP':       regimeAlignPts = 0;  risks.push('BTC chop — SELL unreliable'); break;
      default:           regimeAlignPts = 6;  risks.push('BTC regime unknown'); break;
    }
  }
  components.regimeAlignment = clamp(regimeAlignPts, 0, 25);
  if (regimeNote) confluence.push(regimeNote);

  // ── Supporting context (logged only — no score impact) ────────────────────
  ...
  // fix48b: rsiPart.penalty intentionally not applied to score — conviction model uses no penalties.
  // RSI is advisory only: logged in risks/confluence, stored in components.rsi for CSV compat.
  // Hard RSI gate (RSI_OVERBOUGHT_ON_LONG) is in shouldReject when rejectRsiConflict=true.
  ...
  // ── Final score: pure sum of 3 buckets — NO caps ─────────────────────────
  // Supporting components (volume, rsi, liquidity) are tracked but do not inflate score.
  // Max possible: 60 + 15 + 25 = 100.
  const raw = components.family + components.entry + components.regimeAlignment;
  const score = clamp(Math.round(raw), 0, 100);

  // fix48b: add CSV-compat aliases to components so frontend sc_ columns stay populated.
  // Frontend reads: bd.base, bd.direction, bd.trend, bd.structure, bd.rr, bd.elliottWave
  // These are now derived from components.family (the 2/3-family vote bucket).
  // Split family pts proportionally so CSV shows meaningful non-zero values.
  const _familyPts = components.family || 0;
  components.base      = 8;                                   // always 8 (base conviction)
  components.direction = _familyPts > 0 ? Math.round(_familyPts * 0.35) : 0; // ~35% of family
  components.trend     = _familyPts > 0 ? Math.round(_familyPts * 0.25) : 0; // ~25% of family
  components.structure = _familyPts > 0 ? Math.round(_familyPts * 0.25) : 0; // ~25% of family
  components.rr        = _familyPts > 0 ? Math.round(_familyPts * 0.15) : 0; // ~15% of family
  components.elliottWave = (ctx.elliottAnalysis?.signal) ? 8 : 0;             // signal present?

  return {
    score,
    rawScore: Number(raw.toFixed(2)),
    penalties: 0,  // no penalty model in fix48 — kept for CSV compat
    components,
    confluence,
    risks,
...
function shouldReject(leader, settings, signals = []) {
  ...
  // fix48c: RR ceiling at 3.0 — data: RR 3.0+ = ~30% WR across 445+ trades (-2.77U).
  // Exempt structureSignal: W5 exhaustion or CHoCH SELL targeting distant support IS the trade.
  // Exempt contrarianFlip: already validated in buildStructurePlan with its own RR gate.
  if (leader.math.rr > 3.0 && !leader.structureSignal && !leader.contrarianFlip) reasons.push(`RR_GT_3.0_BLOCKED_${leader.math.rr.toFixed(2)}`);
...
function getSignals() {
  ...
  const normalized = data.map(s => {
    ...
    if ((s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && now - num(s.createdAt || s.updatedAt, now) > num(s.expireMs, EXPIRY_MS)) {
      changed = true;
      return { ...s, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', positionStatus: 'NONE', updatedAt: now, closedAt: now, stateReason: `${V4_VERSION}: stale waiting setup expired` };
    }
    return s;
  });
...
function initActiveDiagnostics(s, entryPrice, atr, now, settings) {
  ...
  s.diag.rsiAtEntry = num(s.rsi, null);
}
...
function ingestCandidate(payload = {}) {
  ...
  // fix48c: RR ceiling at 3.0 mirrored from shouldReject — keeps ingestCandidate consistent
  // Exempt structureSignal and contrarianFlip same as shouldReject
  const _ingStructSig = payload.structureSignal || (payload.scoreInfo?.components?.family >= 60);
  const _ingFlip = payload.contrarianFlip || false;
  if (rr > 3.0 && !_ingStructSig && !_ingFlip) reasons.push(`RR_GT_3.0_BLOCKED_${rr.toFixed(2)}`);
