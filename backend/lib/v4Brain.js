...
const V4_VERSION = '4.6.8.48h'; // fix48h: continuous multiplicative conviction score (replaces additive buckets) — score now ranks by expected WR; counter-trend/poor-entry self-score-out; RSI folded in; threshold recalibrated for new scale
...
function convictionScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  // BASE=46 calibrated so a flawless setup lands ~100 (not clamped), a good one ~65, marginal ~38,
  // counter-trend bleed ~18 — a real spread that ranks by expected win rate.
  const BASE = 46;
  ...
  else if (familiesAgreed >= 2) { familyMult = 1.00; confluence.push(`2-family consensus: ${(decision.reasons || []).join(', ')}`); }
  ...
  if (aligned(btcR, side)) {
    ...
    // CHOP / UNKNOWN — no edge from regime
    regimeMult = 0.75;
...
function getSignals() {
  ...
  const normalized = data.map(s => {
    if (!s) return s;
    if ((s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && now - num(s.createdAt || s.updatedAt, now) > Math.min(num(s.expireMs, EXPIRY_MS), WAITING_HARD_CAP_MS)) {
      changed = true;
      return { ...s, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', positionStatus: 'NONE', updatedAt: now, closedAt: now, stateReason: `${V4_VERSION}: stale waiting setup expired (45min cap)` };
    }
    return s;
  });
...
function updateExistingSignals(signals, priceMap) {
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') {
      const _ageNoPrice = now - num(s.createdAt || s.updatedAt, now);
