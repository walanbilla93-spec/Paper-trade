  ...
  try {
    ...
    for (const t of universe) {
      try {
        ...
        // fix-skip: log scan-loop rejections to v4_candidate_rejections
        if (leader.rejected && leader.rejectReasons && leader.rejectReasons.length) {
          try {
            const _rej = store.read('v4_candidate_rejections', []);
            const _row = {
            ...
            store.write('v4_candidate_rejections', [_row].concat(Array.isArray(_rej) ? _rej : []).slice(0, 1000));
