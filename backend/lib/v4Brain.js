...
function currentSettings() {
  ...
  return {
    ...s,
    version: V4_VERSION,
    minScoreToTrade: num(s.minScoreToTrade, 75),
    ...
    minEntryTimingScore: Math.max(35, Math.min(75, num(s.minEntryTimingScore, 48))),
    v4EntryReactionBps: Math.max(1, Math.min(15, num(s.v4EntryReactionBps, ENTRY_REACTION_BPS))),
    v4EntryWaitSeconds: Math.max(5, Math.min(600, num(s.v4EntryWaitSeconds, ENTRY_CONFIRMATION_WINDOW_MS / 1000))),
...
function signalEntryTiming(s, price, atr, now = Date.now(), settings = currentSettings()) {
  ...
  const expiredWindow = elapsed > Math.max(5000, num(settings.v4EntryWaitSeconds, ENTRY_CONFIRMATION_WINDOW_MS / 1000) * 1000);
...
function refreshSignalPlan(existing, candidate, reason = 'REFRESHED_WAITING_SETUP') {
  ...
  Object.assign(existing, candidate, preserve, {
    ...
    updatedAt: Date.now(),
    stateReason: `V4.6.8.14: ${reason}; refreshed waiting setup with newer/better planner data`,
  });
...
function shouldReject(leader, settings, signals = []) {
  const reasons = [];
  const warnings = [];
  const minScore = num(settings.minScoreToTrade, 75);
...
function signalFromLeader(leader, settings) {
  ...
  return {
    ...
    updatedAt: now,
    expireMs: EXPIRY_MS,
    stateReason: state === 'PAPER_ACTIVE' ? `V4.6.8.14: ${timing.reason}` : `V4.6.8.14: ${timing.reason}`,
...
function getSignals() {
  ...
  const normalized = data.map(s => {
    ...
    if ((s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && now - num(s.createdAt || s.updatedAt, now) > num(s.expireMs, EXPIRY_MS)) {
      changed = true;
      return { ...s, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', positionStatus: 'NONE', updatedAt: now, closedAt: now, stateReason: 'V4.6.8.14: stale waiting setup expired' };
    }
    return s;
  });
...
function saveLedger(rows) {
  ...
  const prepared = (rows || []).map(r => {
    ...
    if ((state === 'WAITING_ENTRY' || state === 'WAITING_REACTION') && age > num(r.expireMs, EXPIRY_MS)) {
      return { ...r, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', result: 'EXPIRED', updatedAt: now, closedAt: now, stateReason: r.stateReason || 'V4.6.8.14: stale waiting setup expired by ledger cleanup' };
    }
    return r;
  });
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY') {
      ...
      if (timing.inZone && timing.directional) {
        ...
        s.entryHit = true;
        s.openedAt = now;
        s.stateReason = 'V4.6.8.14: ' + timing.reason;
...
function ingestCandidate(payload = {}) {
  ...
  const minEntryTimingScore = num(settings.minEntryTimingScore, 48);
  if (entryTimingScore > 0 && entryTimingScore < minEntryTimingScore) reasons.push(`ENTRY_TIMING_LT_${minEntryTimingScore}`);
  ...
  if (rejected) {
    ...
    const row = { at: Date.now(), symbol, side, score, rr, slPct, btcRegime, reasons, warnings, source: V4_VERSION };
    store.write('v4_candidate_rejections', [row].concat(Array.isArray(rej)?rej:[]).slice(0,100));
  ...
  const sig = {
    ...
    source: 'frontend-candidate-v4.6.8.14',
    plannerVersion: V4_VERSION,
    sym: symbol,
    symbol,
    side,
    ...
    entryHit: false,
    stateReason: 'V4.6.8.14: READY setup captured as WAITING_ENTRY; activation requires tight entry zone + directional reaction',
...
function freshJournal() {
  ...
  store.write('legacy_browser_signals', { at: Date.now(), total: 0, note: 'cleared by v4.6.8.14 freshJournal' });
  snapshot = { ...snapshot, ts: Date.now(), signals: [], visibleSignals: 0, ledger: [], summary: computeLedgerSummary([]) };
  store.write('v4_snapshot', snapshot);
  return { ok: true, cleared: true, source: 'v4.6.8.14 freshJournal' };
}
