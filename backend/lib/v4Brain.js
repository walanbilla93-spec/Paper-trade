...
const V4_VERSION = '4.6.8.48c'; // fix48c: RR ceiling 3.0 re-applied (RR>3.0=30%WR confirmed), frontend version string synced
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') {
      if (age > num(s.expireMs, expiryMsFor(settings))) {
        ...
        s.closedAt = now;
        s.stateReason = `${V4_VERSION}: setup expired before entry confirmation`;
