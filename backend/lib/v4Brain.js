...
const V4_VERSION = '4.6.2-candidate-capture-hotfix';
...
function ingestCandidate(payload = {}) {
  ...
  const score = num(payload.score || payload.confidence || payload.setupScore, 0);
  ...
  const sig = {
    ...
    source: 'frontend-candidate-v4.6.2',
    plannerVersion: V4_VERSION,
    sym: symbol,
    symbol,
    side,
    ...
    entryHit: false,
    stateReason: 'V4.6.2: READY setup captured as WAITING_ENTRY; activation waits for planner entry zone',
    history: [{ at: now, state: 'WAITING_ENTRY', reason: 'Frontend READY candidate captured by backend' }],
    time: new Date(now).toLocaleTimeString().slice(0,5),
    createdAt: now,
    updatedAt: now,
  };
