...
const V4_VERSION = '4.6.8.5-experiment-settings';
...
function refreshSignalPlan(existing, candidate, reason = 'REFRESHED_WAITING_SETUP') {
  ...
  Object.assign(existing, candidate, preserve, {
    ...
    updatedAt: Date.now(),
    stateReason: `V4.6.8.3: ${reason}; refreshed waiting setup with newer/better planner data`,
  });
...
function signalFromLeader(leader, settings) {
  ...
  return {
    id,
    source: 'backend-planner-v4.6.8.5',
    ...
    updatedAt: now,
    expireMs: EXPIRY_MS,
    stateReason: state === 'PAPER_ACTIVE' ? `V4.6.8.3: ${timing.reason}` : `V4.6.8.3: ${timing.reason}`,
...
function getSignals() {
  ...
  const normalized = data.map(s => {
    ...
    if ((s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && now - num(s.createdAt || s.updatedAt, now) > num(s.expireMs, EXPIRY_MS)) {
      changed = true;
      return { ...s, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', positionStatus: 'NONE', updatedAt: now, closedAt: now, stateReason: 'V4.6.8.3: stale waiting setup expired' };
    }
    return s;
  });
...
function saveLedger(rows) {
  ...
  const prepared = (rows || []).map(r => {
    ...
    if ((state === 'WAITING_ENTRY' || state === 'WAITING_REACTION') && age > num(r.expireMs, EXPIRY_MS)) {
      return { ...r, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', result: 'EXPIRED', updatedAt: now, closedAt: now, stateReason: r.stateReason || 'V4.6.8.3: stale waiting setup expired by ledger cleanup' };
    }
    return r;
  });
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY') {
      ...
      if (timing.inZone && timing.directional) {
        ...
        s.entryHit = true;
        s.openedAt = now;
        s.stateReason = 'V4.6.8.3: ' + timing.reason;
...
function ingestCandidate(payload = {}) {
  ...
  const sig = {
    ...
    source: 'frontend-candidate-v4.6.8.5',
    plannerVersion: V4_VERSION,
    sym: symbol,
    symbol,
    side,
    ...
    entryHit: false,
    stateReason: 'V4.6.8.3: READY setup captured as WAITING_ENTRY; activation requires tight entry zone + directional reaction',
...
function freshJournal() {
  ...
  store.write('legacy_browser_signals', { at: Date.now(), total: 0, note: 'cleared by v4.6.8.4 freshJournal' });
  snapshot = { ...snapshot, ts: Date.now(), signals: [], visibleSignals: 0, ledger: [], summary: computeLedgerSummary([]) };
  store.write('v4_snapshot', snapshot);
  return { ok: true, cleared: true, source: 'v4.6.8.4 freshJournal' };
}
