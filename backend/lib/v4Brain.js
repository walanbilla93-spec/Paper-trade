...
const V4_VERSION = '4.6.8.67'; // fixMICRO2: birth-time funding/OI capture fallback. fixMICRO (4.6.8.66) only stamped coinFundingRateAtEntry/coinOpenInterestAtEntry/fundingDivergenceAtEntry in initActiveDiagnostics (fires on ACTIVE fill) -- any signal that never filled (flipped/expired/stale-cancelled/invalidated while WAITING) stayed permanently null, silently dropping exactly the pre-fill population fixFLIP and the Direction Brain need most. Now ALSO stamped in signalFromLeader at birth from data already on hand (leader.coinFundingRate/coinOpenInterest + a sentinel read), into a local _birthDiag object that becomes the signal's diag: field. If the signal later fills, initActiveDiagnostics's existing unconditional stamp overwrites with the (currently identical, since no re-fetch happens) fill-time value -- birth capture is a fallback, not a competing source of truth, zero change to fill-time behavior. SAFETY: purely additive to diag:{} -- verified zero references to paperState/status/tradeId/positionStatus/orderStatus anywhere in the new code path; structurally cannot affect fills, cancels, or invalidations. // v4.6.8.66 was fixMICRO:
