...
const V4_VERSION = '4.6.8.49d'; // fix49a: contextWeighting default→OFF (was ghost-towning all markets with bearish breadth; 13-22% score penalty pushed every signal below minScoreToTrade=70). Penalty curve needs recalibration — re-enable via CONTEXT_WEIGHTING=true. Carries 48z SELL-unblock + zombie sweep.
...
// fix49d: LIVE-FILL GATE.
// The paper brain activates a signal on price/timing (missed-move fix31, entry-zone touch)
// with no knowledge of whether the real Bybit GTC limit actually filled. In paper-only mode
// that is correct and unchanged. In LIVE mode it caused Orayan to show PAPER_ACTIVE while the
// Bybit order was still resting unfilled (observed: POWER/ALLO — price moved to TP before the
// limit filled, so Orayan showed ACTIVE but Bybit had NOT ON BYBIT / 0 fill).
//
// Fix: in LIVE mode only, hold the signal in WAITING_ENTRY until the reconciler confirms the
// Bybit order filled / position open. Paper mode untouched. Reversible: V4_LIVE_FILL_GATE=false.
function liveFillGate(signal, settings) {
  const liveMode = settings.botMode === 'LIVE_REAL_BYBIT' && settings.tradingEnabled === true;
  if (!liveMode) return 'PAPER';
  if (String(process.env.V4_LIVE_FILL_GATE || 'true').toLowerCase() === 'false') return 'PAPER';

  let trades;
  try { trades = store.read('trades', {}); } catch (_e) { trades = null; }
  if (!trades) return 'NO_ORDER';

  const sid = String(signal.id || signal.signalId || '');
  const trade = Object.values(trades).find(t => t && String(t.signalId || '') === sid);
  if (!trade) return 'NO_ORDER';

  const filled = trade.positionStatus === 'OPEN'
    || num(trade.cumExecQty) > 0
    || trade.orderStatus === 'Filled'
    || trade.status === 'POSITION_OPEN';
  if (filled) return 'FILLED';

  if (['CANCELLED', 'ORDER_CANCELLED', 'ORDER_REJECTED', 'ORDER_DEACTIVATED'].includes(trade.status)) return 'NO_ORDER';
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') {
      ...
      if (age > _effectiveExpiry) {
        ...
        // fix49c: cancel any GTC waiting limit when signal hits hard cap — order must not fill into dead setup
        if (s.tradeId) { const _expT = store.read('trades', {}); if (_expT[s.tradeId]) { _expT[s.tradeId].signalInvalidated = true; store.write('trades', _expT); } }
        changed = true;
        continue;
      }
      ...
      if (timing.inZone && timing.directional) {
        ...
        // fix49d: LIVE-FILL GATE — in LIVE mode, hold until Bybit confirms fill. Paper falls through.
        const _fill = liveFillGate(s, settings);
        if (_fill === 'UNFILLED' || _fill === 'NO_ORDER') {
          if (s.paperState !== 'WAITING_ENTRY') changed = true;
          s.paperState = 'WAITING_ENTRY';
          s.displayState = 'WAITING_ENTRY';
          s.status = 'DETECTED';
          s.positionStatus = 'NONE';
          s.entryTimingScore = _activationTimingScore;
          s.stateReason = `${V4_VERSION}: LIVE fill gate — timing ready but Bybit order ${_fill === 'NO_ORDER' ? 'not placed/seen' : 'not yet filled'}; holding WAITING`;
          appendSignalDiagnostic('LIVE_FILL_HELD', s, { prevState, price, timing, fillState: _fill }, settings);
          continue;
        }
        ...
        s.positionStatus = (_fill === 'FILLED') ? 'OPEN' : 'PAPER';
        s.entryHit = true;
        s.openedAt = now;
