...
const V4_VERSION = '4.6.9.4'; // NEW in .4 — hotfix30: fixPHASELOG (real bug, found via user's uploaded
...
function decideLiveEntry(s, market, timing, settings, atr) {
  ...
  try {
    ...
    if (activeTrades().length >= (settings.maxOpenTrades || 3)) return { wouldPlace: false, reason: 'MAXOPEN' };
  } catch (_e) { return { wouldPlace: false, reason: 'EXECUTOR_UNAVAILABLE' }; }
...
// been touched FIRST. Both-sides symmetric (BUY: high>=tp1 win / low<=sl loss; SELL mirrored).
...
async function analyzeExpiredOpportunityCost({ lookaheadMs = 3 * 60 * 60 * 1000 } = {}) {
  ...
  const now = Date.now();
  const all = ledger.filter(r => String(r.paperState || r.result || '').toUpperCase() === 'EXPIRED');
  ...
  for (let i = 0; i < candidates.length; i += CONC) {
    ...
    const batchResults = await Promise.all(batch.map(async (row) => {
      try {
        ...
        if (!(start > 0) || end <= start) return { ...baseRow(row), outcome: 'DATA_UNAVAILABLE', note: 'bad timestamps' };
        const candles = await fetchKlineRange(row.symbol || row.sym, '1', start, end);
        if (!candles.length) return { ...baseRow(row), outcome: 'DATA_UNAVAILABLE', note: 'no candles returned' };
        const outcome = walkOutcome(row, candles);
        return { ...baseRow(row), outcome };
      } catch (e) {
        return { ...baseRow(row), outcome: 'DATA_UNAVAILABLE', note: e.message };
      }
    }));
  ...
  // no new P&L model invented here.
...
function updateExistingSignals(signals, priceMap) {
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION' || s.paperState === 'CANDIDATE') {
      ...
      try {
        const _p1Verdict = decideLiveEntry(s, market, timing, settings, atr);
