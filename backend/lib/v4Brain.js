...
const V4_VERSION = '4.6.8.95'; // NEW in .95 — fixRETEST: user-directed redesign replacing the whole 07/26
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION' || s.paperState === 'CANDIDATE') {
      ...
      if (timing.inZone && timing.directional) {
        ...
        if (settings.botMode === 'LIVE_REAL_BYBIT' && settings.tradingEnabled && !s._autoTraded && !s.liveOrderId && s.hadFirstTouch && !timing.missedMove) {
