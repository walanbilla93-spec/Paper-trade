...
const V4_VERSION = '4.6.8.48n2'; // fix48n2: REGIME_CONFLICT now neutralizes BOTH sides (was BUY-only). Contested bull label (sentinel breadth<=30% bull OR composite<=-3) -> regimeMult 0.90 for BUY AND SELL. Removes phantom counter-trend penalty that was suppressing SELLs in mislabeled bear, so SELL-in-bear edge can be tested on clean data. No block/flip/force/boost. Plus 48n: 7 sentinel CSV export cols.
