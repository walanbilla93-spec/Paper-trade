...
const SELL_MIN_RR = Math.max(MIN_RR, Number(process.env.V4_SELL_MIN_RR || '2.0'));
const MIN_SL_DISTANCE_PCT = Math.max(0, Number(process.env.V4_MIN_SL_DISTANCE_PCT || '0.60'));
const ENTRY_CONFIRMATION_MS = Math.max(0, Number(process.env.V4_ENTRY_CONFIRMATION_SECONDS || '10')) * 1000;
const ENTRY_REACTION_BPS = Math.max(0, Number(process.env.V4_ENTRY_REACTION_BPS || '2')) / 10000;
...
const V4_VERSION = '4.6.6-stability-confirmation';
...
function currentSettings() {
  ...
  return {
    ...
    leverage: Math.max(1, num(s.leverage, 5)),
...
function isTerminalResultValue(v) {
  const x = String(v || '').toUpperCase();
  return ['WIN', 'LOSS', 'TP_HIT', 'SL_HIT', 'INVALIDATED', 'EXPIRED', 'REJECTED'].includes(x);
}

function isTerminalRow(r) {
  if (!r) return false;
  return isTerminalResultValue(r.result) || FINAL_STATES.has(String(r.paperState || '').toUpperCase()) || ['WIN', 'LOSS'].includes(String(r.status || '').toUpperCase());
}

function terminalRank(r) {
  if (!r) return 0;
  const result = String(r.result || '').toUpperCase();
  const state = String(r.paperState || r.status || '').toUpperCase();
  if (result === 'WIN' || state === 'TP_HIT' || state === 'WIN') return 5;
  if (result === 'LOSS' || state === 'SL_HIT' || state === 'LOSS') return 5;
  if (['INVALIDATED', 'EXPIRED', 'REJECTED'].includes(result) || ['INVALIDATED', 'EXPIRED', 'REJECTED'].includes(state)) return 3;
  if (state === 'PAPER_ACTIVE' || result === 'ACTIVE') return 2;
  if (state === 'WAITING_ENTRY' || result === 'WAITING_ENTRY') return 1;
  return 0;
}

function entryZoneTolerance(entry, atr) {
  return Math.max(num(atr) * ENTRY_TOLERANCE_ATR, num(entry) * 0.0025);
}

function isInsideEntryZone(side, price, entry, atr) {
  price = num(price); entry = num(entry);
  if (!price || !entry) return false;
  return Math.abs(price - entry) <= entryZoneTolerance(entry, atr);
}

function candleDirection(c) {
  if (!c) return 0;
  const open = num(c.open), close = num(c.close);
  if (!open || !close) return 0;
  const pct = (close - open) / open;
  if (pct > 0.0004) return 1;
  if (pct < -0.0004) return -1;
  return 0;
}

function hasEntryReactionForLeader(leader) {
  const side = leader.side;
  const reaction = leader.entryReaction || {};
  if (reaction.confirmed) return true;
  const dir = sideSign(side);
  if (dir === 1 && reaction.lastCandleDir > 0) return true;
  if (dir === -1 && reaction.lastCandleDir < 0) return true;
  if (dir === 1 && num(reaction.momentumPct) > 0.20) return true;
  if (dir === -1 && num(reaction.momentumPct) < -0.20) return true;
  return false;
}

function shouldActivateFromWaiting(s, price, now) {
  ...
  if (!isInsideEntryZone(side, price, entry, atr)) return { ok: false, reason: 'waiting for entry zone' };

  if (!s.entryZoneTouchedAt) {
    s.entryZoneTouchedAt = now;
    s.entryZoneTouchPrice = roundPrice(price);
    return { ok: false, reason: 'entry zone touched; waiting for next-tick confirmation' };
  }

  const touch = num(s.entryZoneTouchPrice, price);
  const age = now - num(s.entryZoneTouchedAt, now);
  const favorable = side === 'BUY'
    ? price >= touch * (1 + ENTRY_REACTION_BPS)
    : price <= touch * (1 - ENTRY_REACTION_BPS);
  const stillValid = side === 'BUY' ? price > sl : price < sl;
  if (favorable && stillValid) return { ok: true, reason: 'entry-zone reaction confirmed' };
  if (age >= ENTRY_CONFIRMATION_MS && stillValid && isInsideEntryZone(side, price, entry, atr)) {
    return { ok: true, reason: 'entry-zone confirmed after 10s hold' };
  }
  return { ok: false, reason: 'waiting for entry reaction' };
}
...
function buildStructurePlan(ctx, settings, btcRegime) {
  ...
  const leader = {
    ...
    entryReaction: (() => {
      const last = Array.isArray(k5) && k5.length ? k5[k5.length - 1] : null;
      const dir = candleDirection(last);
      const inside = isInsideEntryZone(side, price, plan.entry, atr);
      const confirmed = inside && ((side === 'BUY' && (dir > 0 || momentumPct > 0.20)) || (side === 'SELL' && (dir < 0 || momentumPct < -0.20)));
      return { inside, confirmed, lastCandleDir: dir, momentumPct: Number(num(momentumPct).toFixed(3)), checkedAt: Date.now() };
    })(),
  };
...
function shouldReject(leader, settings) {
  ...
  if (leader.math.rr < (leader.side === 'SELL' ? SELL_MIN_RR : MIN_RR)) reasons.push(`RR_LT_${leader.side === 'SELL' ? SELL_MIN_RR : MIN_RR}`);
  const slDistancePct = leader.plan && leader.plan.entry ? (Math.abs(num(leader.plan.entry) - num(leader.plan.sl)) / num(leader.plan.entry)) * 100 : 0;
  if (slDistancePct < MIN_SL_DISTANCE_PCT) reasons.push(`SL_DISTANCE_LT_${MIN_SL_DISTANCE_PCT}`);
...
function signalFromLeader(leader, settings) {
  ...
  const near = isInsideEntryZone(leader.side, price, entry, atr);
  const reactionConfirmed = near && hasEntryReactionForLeader(leader);
  const state = reactionConfirmed ? 'PAPER_ACTIVE' : 'WAITING_ENTRY';
  const id = `v4_${leader.symbol}_${leader.side}_${now}`;
  return {
    id,
    source: 'planner-v4.6.6',
    ...
    updatedAt: now,
    expireMs: EXPIRY_MS,
    stateReason: state === 'PAPER_ACTIVE' ? 'V4.6.6: entry zone + reaction confirmed' : (near ? 'V4.6.6: entry zone touched; waiting for reaction confirmation' : 'V4.6.6: waiting for planner entry zone'),
    entryZoneTouchedAt: near ? now : 0,
    entryZoneTouchPrice: near ? roundPrice(price) : 0,
    reasons: leader.reasons,
    risks: leader.risks,
    planner: {
      source: 'backend-planner-v4.6.6',
    ...
    history: [{ at: now, state, reason: state === 'PAPER_ACTIVE' ? 'Entry reaction confirmed by backend planner' : 'Planner created and waiting entry' }],
  };
}
...
function normalizeLedgerTrade(s) {
  ...
  const result =
    ...
    s.paperState === 'WAITING_ENTRY' ? 'WAITING_ENTRY' : (s.paperState || s.status || 'UNKNOWN');
...
function saveLedger(rows) {
  ...
  for (const r of rows || []) {
    ...
    // Terminal-lock: a closed row is the source of truth forever for that trade key.
    // Later active/planner refreshes may update live metadata but must never downgrade
    // WIN/LOSS/EXPIRED/INVALIDATED/REJECTED back to PAPER_ACTIVE/WAITING_ENTRY.
    const oldTerminal = isTerminalRow(old);
    const nextTerminal = isTerminalRow(next);
    if (oldTerminal && !nextTerminal) continue;
    if (!oldTerminal && nextTerminal) {
      unique.set(key, next);
      continue;
    }
    if (oldTerminal && nextTerminal) {
      const oldClosed = num(old.closedAt || old.updatedAt || old.createdAt);
      const nextClosed = num(next.closedAt || next.updatedAt || next.createdAt);
      unique.set(key, nextClosed >= oldClosed ? { ...old, ...next, result: next.result || old.result } : old);
      continue;
    }
    if (terminalRank(next) > terminalRank(old) || num(next.updatedAt || next.createdAt) >= num(old.updatedAt || old.createdAt)) {
      unique.set(key, next);
    }
  }
...
function displayRowsFromLedgerAndSignals(ledgerRows = getLedger(), signals = getSignals()) {
  const map = new Map();
  for (const r of ledgerRows || []) {
    const nr = normalizeLedgerTrade(r);
    map.set(nr.key || tradeKey(nr), nr);
  }
  for (const s of signals || []) {
    const ns = normalizeLedgerTrade(s);
    const key = ns.key || tradeKey(ns);
    const old = map.get(key);
    if (old && isTerminalRow(old) && !isTerminalRow(ns)) continue;
    if (!old || terminalRank(ns) >= terminalRank(old) || num(ns.updatedAt || ns.createdAt) >= num(old.updatedAt || old.createdAt)) map.set(key, ns);
  }
  const order = { ACTIVE: 0, WAITING_ENTRY: 1, WIN: 2, LOSS: 3, INVALIDATED: 4, EXPIRED: 5, REJECTED: 6 };
  return [...map.values()].sort((a, b) => (order[a.result] ?? 9) - (order[b.result] ?? 9) || num(b.createdAt || b.openedAt) - num(a.createdAt || a.openedAt)).slice(0, MAX_SIGNALS);
}
...
function updateExistingSignals(signals, priceMap) {
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY') {
      ...
      const activation = shouldActivateFromWaiting(s, price, now);
      if (activation.ok) {
        ...
        s.entryHit = true;
        s.openedAt = s.openedAt || now;
        s.stateReason = 'V4.6.6: ' + activation.reason;
        s.history = [...(s.history || []), { at: now, state: 'PAPER_ACTIVE', reason: s.stateReason, price }];
        changed = true;
      } else if (activation.reason && activation.reason !== s.stateReason) {
        s.stateReason = 'V4.6.6: ' + activation.reason;
        changed = true;
      }
      continue;
    ...
    if (s.paperState === 'PAPER_ACTIVE') {
      let final = null;
      if (side === 'BUY' && price >= tp) final = { state: 'TP_HIT', status: 'WIN', reason: 'V4.6.6: TP hit' };
      if (side === 'SELL' && price <= tp) final = { state: 'TP_HIT', status: 'WIN', reason: 'V4.6.6: TP hit' };
      if (side === 'BUY' && price <= sl) final = { state: 'SL_HIT', status: 'LOSS', reason: 'V4.6.6: SL hit' };
      if (side === 'SELL' && price >= sl) final = { state: 'SL_HIT', status: 'LOSS', reason: 'V4.6.6: SL hit' };
      if (final) {
...
async function scanOnce() {
  ...
  try {
    ...
    const displayRows = displayRowsFromLedgerAndSignals(ledger, signals);
    const visibleSummary = computeSummary(displayRows);
    ...
    snapshot = {
      ...
      signals: displayRows,
      visibleSignals: displayRows.length,
...
function getSnapshot() {
  ...
  const signals = getSignals(); const ledger = upsertLedger(signals); const displayRows = displayRowsFromLedgerAndSignals(ledger, signals); return { ...snapshot, signals: displayRows, visibleSignals: displayRows.length, ledger: ledger.slice(0, RECENT_LEDGER_LIMIT), summary: { ...computeLedgerSummary(ledger), visible: computeSummary(displayRows), source: 'v4_paper_ledger' }, lastError };
}
...
module.exports = {
  ...
  computeSummary,
  getLeaderboard,
  displayRowsFromLedgerAndSignals,
};
