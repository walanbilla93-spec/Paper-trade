...
function signalFromLeader(leader, settings) {
  ...
  try {
    ...
    try {
      const _rUnit = (num(leader.entry, null) != null && num(leader.sl, null) != null)
        ? Math.abs(num(leader.entry, 0) - num(leader.sl, 0)) : null;
...
function updateExistingSignals(signals, priceMap) {
  ...
  for (const s of signals) {
    ...
    if ((s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && isDeadHourLK(now) && !s.tradeId && !s.liveOrderId) {
      s.paperState = 'EXPIRED';
      s.displayState = 'EXPIRED';
      s.status = 'EXPIRED';
      s.positionStatus = 'NONE';
...
async function scanOnce() {
  ...
  try {
    ...
    // fix75: PAPER EQUITY SELF-SNAPSHOT. The balance_history store (equity curve, fix49) was only
    // ever fed by the FRONTEND polling the real Bybit wallet — in paper mode nothing wrote to it, so
    // the equity chart was structurally empty for the mode the system actually runs in. Backend now
    // self-records paper equity hourly: V4_PAPER_START_BALANCE (env, default 1000) + permanent net
    // P&L (fix52 append-only accumulator — decay-proof, so the curve can never un-earn a resolved
    // trade). Only in paper modes; live mode keeps the wallet-poll as sole author so the two writers
    // can never interleave real and synthetic equity in one store.
    try {
      if (String(settings.botMode || '') !== 'LIVE_REAL_BYBIT') {
        const _nowEq = Date.now();
        if (!scanOnce._lastPaperEquityAt || _nowEq - scanOnce._lastPaperEquityAt >= 3600 * 1000) {
          scanOnce._lastPaperEquityAt = _nowEq;
          const _base = Math.max(0, Number(process.env.V4_PAPER_START_BALANCE || '1000'));
          const _perm = getPermanentStats();
          const _eq = Number((_base + num(_perm.allTimeNet, 0)).toFixed(4));
          recordBalanceSnapshot({ walletBalance: _eq, availableBalance: _eq, unrealisedPnl: 0, equity: _eq });
        }
      }
    } catch (_ePE) { /* diagnostic feature — never block the scan */ }
