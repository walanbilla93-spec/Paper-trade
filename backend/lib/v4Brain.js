...
const V4_VERSION = '4.6.8.48x'; // fix48x: ONE-LINE BUG — config.getSettings() in convictionScore threw ReferenceError (config is destructured, not a named var) → try/catch silently set contextMult=1.0/_ctxDiag=null every signal → all 48v/48w context fields null. Fix: getSettings(). Carries all prior fixes.
