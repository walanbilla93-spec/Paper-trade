...
const V4_VERSION = '4.6.8.49e'; // fix49a: contextWeighting default→OFF (was ghost-towning all markets with bearish breadth; 13-22% score penalty pushed every signal below minScoreToTrade=70). Penalty curve needs recalibration — re-enable via CONTEXT_WEIGHTING=true. Carries 48z SELL-unblock + zombie sweep.
...
function updateExistingSignals(signals, priceMap) {
  ...
  for (const s of signals) {
    ...
    // fix49e: LIVE STATE AUTHORITY — Bybit is the single source of truth in live mode.
    // If this signal has a linked Bybit trade, its state is driven by Bybit, and ALL paper
    // logic below (expiry, revalidation, missed-move, paper TP/SL) is bypassed for this signal.
    ...
    if (_live) {
      ...
      if (_live.kind === 'WAITING') {
        if (s.paperState !== 'WAITING_ENTRY') {
          ...
          s.status = 'DETECTED';
          s.positionStatus = 'NONE';
          s.stateReason = `${V4_VERSION}: LIVE — Bybit order resting/unfilled (authority lock)`;
          appendSignalDiagnostic('LIVE_LOCK_WAITING', s, { prevState, price, tradeId: s.tradeId }, settings);
          changed = true;
        }
        continue; // do not let paper expiry abandon a live resting order
      }
      if (_live.kind === 'CLOSED') {
