...
function calibratedScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  // fix-regime-floor: if regime contributed nothing (CHOP/conflict/unknown),
  // cap score at 72 regardless of other components. Prevents score-78 signals
  // with zero regime edge from reaching minScoreToTrade=45 gate as "good" signals.
  if (components.regimeAlignment === 0) score = Math.min(score, 72);
  ...
  try {
    ...
    for (const t of universe) {
      try {
        ...
        // fix-skip: log scan-loop rejections to v4_candidate_rejections so
        // /api/v4/rejections and exportAlertsCSV() can surface skip reasons.
        // Previously only ingestCandidate() wrote here — scan rejections were invisible.
        if (leader.rejected && leader.rejectReasons && leader.rejectReasons.length) {
          try {
            ...
            const _row = {
              ...
              score: leader.score,
              rr: leader.math?.rr || 0,
              slPct: slDistancePct(leader),
              btcRegime: leader.btcRegime || '',
              reasons: leader.rejectReasons,
              warnings: leader.rejectWarnings || [],
              source: `scan-${V4_VERSION}`,
              plan: leader.plan || {},
              math: leader.math || {},
            };
