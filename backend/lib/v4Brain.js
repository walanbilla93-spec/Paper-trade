...
const MIN_RR = Math.max(1.2, Number(process.env.V4_MIN_RR || '1.6'));
const MIN_SELL_RR = Math.max(1.2, Number(process.env.V4_MIN_SELL_RR || '1.7'));
...
const MAX_TP_ATR = Math.max(1.0, Number(process.env.V4_MAX_TP_ATR || '3.0')); // FIX20: raised 2.4→3.0; allows bigger TP without changing SL risk
const RISK_WINDOW_HOURS = Math.max(1, Number(process.env.V4_RISK_WINDOW_HOURS || '12')); // FIX20b: was 6h — midnight block after 21:xx losses
const MAX_CONSECUTIVE_LOSSES = Math.max(1, parseInt(process.env.V4_MAX_CONSECUTIVE_LOSSES || '5', 10));
const MAX_SESSION_LOSS_USDT = Math.max(0, Number(process.env.V4_MAX_SESSION_LOSS_USDT || '8.0')); // FIX20b: was 5.0 — too easy to hit with 3x0.33 losses
...
function currentSettings() {
  ...
  return {
    ...
    minEntryTimingScore: Math.max(35, Math.min(100, num(s.minEntryTimingScore, 85))),
    v4MinBuyEntryTimingScore: Math.max(35, Math.min(100, num(s.v4MinBuyEntryTimingScore, 80))), // FIX20: was 85; too strict, killing all BUY signals
    v4MinSellEntryTimingScore: Math.max(35, Math.min(100, num(s.v4MinSellEntryTimingScore, 92))),
    ...
    v4WaitingExpiryMinutes: Math.max(10, Math.min(60, num(s.v4WaitingExpiryMinutes, EXPIRY_MS / 60000))),
...
function minRRFor(side, settings = currentSettings()) {
  return Math.max(1.2, num(side === 'SELL' ? settings.v4MinSellRR : settings.v4MinRR, side === 'SELL' ? MIN_SELL_RR : MIN_RR));
}
...
function minScoreForSide(side, settings = currentSettings()) {
  const fallback = num(settings.minScoreToTrade, 75); // FIX23: lowered 84→75 to increase signal volume
...
function minEntryTimingForSide(side, settings = currentSettings()) {
  const fallback = num(settings.minEntryTimingScore, 85);
  return String(side || '').toUpperCase() === 'SELL'
    ? Math.max(35, Math.min(100, num(settings.v4MinSellEntryTimingScore, fallback)))
    : Math.max(35, Math.min(100, num(settings.v4MinBuyEntryTimingScore, fallback)));
}
...
function calibratedScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  if (targetDistanceAtr <= maxTargetAtr) {
    ...
    penalties += 14;
    risks.push(`target too far vs ATR (${targetDistanceAtr.toFixed(2)}x)`);
  } else {
    penalties += 7;
    risks.push(`target stretched vs ATR (${targetDistanceAtr.toFixed(2)}x)`);
  }

  if (stopDistanceAtr > 2.4) {
    penalties += 8;
    risks.push(`stop too wide vs ATR (${stopDistanceAtr.toFixed(2)}x)`);
  }
  ...
  if (math.netTpUSDT < minNetTp) {
    penalties += 12;
    risks.push(`net TP ${math.netTpUSDT} below ${minNetTp}`);
  }
  ...
  if (!hasStructure) score = Math.min(score, 62); // FIX8: was 50; prevented any signal passing minBuyScore even when lowered
...
function computeEntryTiming(ctx, plan, side, atr, settings = currentSettings()) {
  ...
  const reason = !inZone
    ...
    : directional
      ? `ENTRY_CONFIRMED: tight zone + directional reaction (${num(settings.v4EntryReactionBps, ENTRY_REACTION_BPS)}bps/min ATR-aware)`
      : `WAITING_REACTION: tight zone touched, waiting for directional reaction (${num(settings.v4EntryReactionBps, ENTRY_REACTION_BPS)}bps/min ATR-aware)`;
...
function deriveEntryTimingScore(leader, timing = {}) {
  ...
  let score = 72;
  if (entryDistanceAtr <= 0.25) score = 92;
  else if (entryDistanceAtr <= 0.50) score = 82;
  else if (entryDistanceAtr <= 1.00) score = 68;
  else if (entryDistanceAtr <= 1.80) score = 54;
  else score = 38;
  if (timing.inZone) score += 5;
  if (timing.directional) score += 8;
...
function signalEntryTiming(s, price, atr, now = Date.now(), settings = currentSettings()) {
...
function refreshSignalPlan(existing, candidate, reason = 'REFRESHED_WAITING_SETUP') {
  ...
  Object.assign(existing, candidate, preserve, {
    ...
    updatedAt: Date.now(),
    stateReason: `V4.6.8.19: ${reason}; refreshed waiting setup with newer/better planner data`,
  });
...
function btcRegimeDecision(leader, settings, btcUnknownException = false) {
  ...
  const regime = String(leader.btcRegime || 'unknown').toLowerCase();
  const side = String(leader.side || '').toUpperCase();
  const wanted = side === 'BUY' ? 'bull' : side === 'SELL' ? 'bear' : '';
  const opposite = side === 'BUY' ? 'bear' : side === 'SELL' ? 'bull' : '';
  const reasons = [];
  const warnings = [];
  ...
  if (regime === 'mixed' || regime === 'neutral' || regime === 'chop') {
    warnings.push('BTC_REGIME_MIXED_OR_NEUTRAL');
    if (mode === 'strict' || (settings.rejectMixedBtcRegime && mode !== 'neutral_allowed')) reasons.push('BTC_REGIME_MIXED');
    return { reasons, warnings };
  }

  // FIX22: Hard block opposite-regime trades in direction_match mode too.
  // Previously BTC_BULL_AGAINST_SHORT was only a warning in direction_match — SELL signals fired into pumps.
  if (regime === opposite) {
    reasons.push(side === 'BUY' ? 'BTC_BEAR_AGAINST_LONG' : 'BTC_BULL_AGAINST_SHORT');
  }
  if (mode === 'strict' && wanted && regime !== wanted) reasons.push(`BTC_NOT_${wanted.toUpperCase()}_FOR_${side}`);
  return { reasons, warnings };
}
...
function globalRiskBlockReason(signals = [], settings = currentSettings()) {
  ...
  if (consecutiveLosses >= maxConsecutive) {
    // FIX22: Auto-release after 2hrs — don't hold rest of session permanently.
    const blockHoldMs = Math.max(1, Number(process.env.V4_CONSEC_LOSS_HOLD_HOURS || '2')) * 60 * 60 * 1000;
...
function shouldReject(leader, settings, signals = []) {
  ...
  const minRR = minRRFor(leader.side, settings);
  ...
  const entryTimingScore = deriveEntryTimingScore(leader, leader?.entryTiming || {});
  const minEntryTimingScore = minEntryTimingForSide(leader.side, settings);
  ...
  if (entryTimingScore < minEntryTimingScore) reasons.push(`ENTRY_TIMING_LT_${minEntryTimingScore}`);
  // FIX20b: ETS=100 is anti-predictive (15.4% WR in journal). Block on both sides.
  // Override via env: V4_MAX_BUY_ENTRY_TIMING_SCORE / V4_MAX_SELL_ENTRY_TIMING_SCORE
  const maxEtsBuy  = Math.max(0, parseInt(process.env.V4_MAX_BUY_ENTRY_TIMING_SCORE  || process.env.V4_MAX_ENTRY_TIMING_SCORE || '97', 10));
  const maxEtsSell = Math.max(0, parseInt(process.env.V4_MAX_SELL_ENTRY_TIMING_SCORE || process.env.V4_MAX_ENTRY_TIMING_SCORE || '97', 10));
  const maxEts = leader.side === 'BUY' ? maxEtsBuy : maxEtsSell;
  if (maxEts > 0 && entryTimingScore > maxEts) reasons.push(`ENTRY_TIMING_GT_${maxEts}_ANTIPREDICTIVE`);
  ...
  // FIX21: High-score signals (>=85) get a wider TP ATR allowance (up to 5.0x).
  // HYPE SELL score=87 was blocked 10+ times by TP_TOO_FAR_8.9ATR_GT_3.
  // Score>=85 indicates high-confidence setup — wider TP is valid, not noise.
  const scoreTpBonus = leader.score >= 85 ? 2.0 : leader.score >= 80 ? 1.0 : 0;
  const effectiveMaxTpAtr = Math.min(8.0, maxTargetAtr + scoreTpBonus);
  if (targetAtr && targetAtr > effectiveMaxTpAtr * 1.10) reasons.push(`TP_TOO_FAR_${targetAtr.toFixed(2)}ATR_GT_${effectiveMaxTpAtr}`);
  ...
  const gate = { ok: reasons.length === 0, reasons: [...new Set(reasons)], warnings: [...new Set(warnings)], minNetTpInfo, minNetRR, netRr: num(leader.math.netRr, 0), entryTimingScore, minEntryTimingScore, minRR, minScore, slPct, minSlPct };
  appendDiagnosticJournal({
    at: Date.now(),
    iso: new Date().toISOString(),
...
function signalFromLeader(leader, settings) {
  ...
  const entryTimingScore = deriveEntryTimingScore(leader, timing);
  const minEntryTimingScore = num(settings.minEntryTimingScore, 85);
  const minScore = minScoreForSide(leader.side, settings); // FIX10: was undefined in this scope; caused undefined in diagnostic gateSnapshot
  ...
  return {
    id,
    source: 'backend-planner-v4.6.8.19',
    ...
    stateReason: state === 'PAPER_ACTIVE' ? `V4.6.8.19: ${timing.reason}` : `V4.6.8.19: ${timing.reason}`,
    reasons: leader.reasons,
    risks: leader.risks,
    entryTiming: timing,
    ...
    diagnostic: {
      ...
      gateSnapshot: { minScoreToTrade: settings.minScoreToTrade, minScore, v4MinBuyScore: settings.v4MinBuyScore, v4MinSellScore: settings.v4MinSellScore, v4AllowSells: settings.v4AllowSells, minEntryTimingScore, v4MinBuyEntryTimingScore: settings.v4MinBuyEntryTimingScore, v4MinSellEntryTimingScore: settings.v4MinSellEntryTimingScore, minRR: minRRFor(leader.side, settings), minNetTp: minNetTpFor(settings, leader.math), minNetRR: minNetRRFor(settings), maxTpAtr: maxTpAtrFor(settings), btcRegimeMode: settings.btcRegimeMode },
...
function getSignals() {
  ...
  const normalized = data.map(s => {
    ...
    if ((s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && now - num(s.createdAt || s.updatedAt, now) > num(s.expireMs, EXPIRY_MS)) {
      changed = true;
      return { ...s, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', positionStatus: 'NONE', updatedAt: now, closedAt: now, stateReason: 'V4.6.8.19: stale waiting setup expired' };
    }
    return s;
  });
...
function saveLedger(rows) {
  ...
  const prepared = (rows || []).map(r => {
    ...
    if ((state === 'WAITING_ENTRY' || state === 'WAITING_REACTION') && age > num(r.expireMs, EXPIRY_MS)) {
      return { ...r, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', result: 'EXPIRED', updatedAt: now, closedAt: now, stateReason: r.stateReason || 'V4.6.8.19: stale waiting setup expired by ledger cleanup' };
    }
    return r;
  });
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') {
      if (age > num(s.expireMs, expiryMsFor(settings))) {
        ...
        s.closedAt = now;
        s.stateReason = 'V4.6.8.19: setup expired before entry confirmation';
      ...
      if (timing.inZone && timing.directional) {
        ...
        s.entryHit = true;
        s.openedAt = now;
        s.stateReason = 'V4.6.8.19: ' + timing.reason;
...
function ingestCandidate(payload = {}) {
  ...
  const minRR = minRRFor(side, settings);
  ...
  const minEntryTimingScore = minEntryTimingForSide(side, settings);
  if (entryTimingScore < minEntryTimingScore) reasons.push(`ENTRY_TIMING_LT_${minEntryTimingScore}`);
  ...
  const scoreTpBonusIngest = score >= 85 ? 2.0 : score >= 80 ? 1.0 : 0;
  const effectiveMaxTpAtrIngest = Math.min(8.0, maxTargetAtr + scoreTpBonusIngest);
  if (targetDistanceAtr && targetDistanceAtr > effectiveMaxTpAtrIngest * 1.10) reasons.push(`TP_TOO_FAR_${targetDistanceAtr.toFixed(2)}ATR_GT_${effectiveMaxTpAtrIngest}`);
  ...
  if (settings.rejectMomentumConflict && side === 'BUY' && /short momentum down|bearish divergence|sell[-\s]?accel/i.test(text)) reasons.push('MOMENTUM_OR_VOLUME_CONFLICT_LONG');
  if (settings.rejectMomentumConflict && side === 'SELL' && /short momentum up|bullish divergence|buy[-\s]?accel/i.test(text)) reasons.push('MOMENTUM_OR_VOLUME_CONFLICT_SHORT');
  ...
  const sig = {
    ...
    source: 'frontend-candidate-v4.6.8.19',
    plannerVersion: V4_VERSION,
    sym: symbol,
    symbol,
    side,
    ...
    entryHit: false,
    stateReason: 'V4.6.8.19: READY setup captured as WAITING_ENTRY; activation requires tight entry zone + directional reaction',
...
function freshJournal() {
  ...
  store.write('legacy_browser_signals', { at: Date.now(), total: 0, note: 'cleared by v4.6.8.19 freshJournal' });
  snapshot = { ...snapshot, ts: Date.now(), signals: [], visibleSignals: 0, ledger: [], summary: computeLedgerSummary([]) };
  store.write('v4_snapshot', snapshot);
  return { ok: true, cleared: true, source: 'v4.6.8.19 freshJournal' };
}
