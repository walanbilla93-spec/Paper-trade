...
const marketPolicy = require('./marketPermission');
const paperOrders = require('./paperOrderLifecycle');
const observations = require('./rejectedObservations');
const patch24Identity = require('./patch24Identity');
function observeDecision24(s,type,reason,at=Date.now(),market={}) {
  const settings=currentSettings();
  return observations.register({...s,observationContext:{patchVersion:patch24Identity.patchVersion,
    sourceHash:patch24Identity.sourceHash,configHash:patch24Identity.configHash(settings),
    scorePolicy:{minInclusive:40,maxExclusive:80},timingThreshold:Number(process.env.V4_TIMING_BLOCK_THRESHOLD||44),
    marketPermission:s.marketPermission||currentMarketPermission(at)}},type,reason,at,market);
}
function currentMarketPermission(now = Date.now()) {
  const capture = getSentinelForCapture();
  let live=null;try{live=sentinel.getSentinel();}catch(_e){}
  const options={now,
    buyMin:Math.max(0,Math.min(50,Number(process.env.V4_BUY_MIN_BREADTH || '50'))),
    sellMax:Math.max(50,Math.min(100,Number(process.env.V4_SELL_MAX_BREADTH || '50')))};
  const current=marketPolicy.resolve({...options,sent:live});
  // A valid breadth reading has authority even when the composite/BTC label is UNKNOWN.
  return current.usable?current:marketPolicy.resolve({...options,sent:capture.sent});
}
function reconcileScoreAdmission(leader, gate) {
  const reasons = (leader.rejectReasons || gate.reasons || []).filter(r=>!/^SCORE_(BAND|LT)_/.test(String(r)));
  const pass = passesScoreBand(leader.score);
  if (!pass) reasons.push('SCORE_BAND_OUT_' + leader.score);
  leader.scoreAdmission={raw:leader.euphGuard?.from ?? leader.bottomGuard?.from ?? leader.score,
    adjusted:leader.euphAdxPromote?.from ?? leader.euphGuard?.to ?? leader.score,
    final:leader.score,policy:{enabled:SCORE_BAND_ENABLED,minInclusive:SCORE_BAND_LO,maxExclusive:SCORE_BAND_HI},pass};
  leader.rejectReasons=reasons;leader.rejected=reasons.length>0;
  gate.reasons=reasons;gate.ok=!leader.rejected;gate.scoreAdmission=leader.scoreAdmission;
  return gate;
}
...
const directionMode = require('./directionMode');
const metaFilter = require('./metaFilter'); // fixMETA: meta-labeling gate (whether to TAKE, not which way) // fixWIRE: switchable direction engine + extension gate // fix44f: squeeze/trap/BOS/CHoCH/divergence + fix46c: Elliott Waves
...
// fixOOM: 5000 rows x ~9-12KB/row (hotfix9's own measurement) = up to 60MB held permanently in
// the hotfix38 in-memory ledger cache, on a container whose Node heap ceiling is ~258MB. Combined
// with the kline cache leak this is what tipped the process over. 1500 rows (~18MB) still covers
// weeks of history at current signal rates. Raise with V4_MAX_LEDGER if you move to a bigger plan.
const MAX_LEDGER = Math.max(500, Math.min(20000, parseInt(process.env.V4_MAX_LEDGER || '1500', 10)));
...
// fix48y set both of these to ZERO. That single change is the largest single source of the
// paper-vs-real gap: paper booked every trade with no cost while Bybit charged on every fill.
//
// MEASURED (same calendar window Jul 27 - Aug 1, paper ledger vs Bybit ledger):
//   paper  WR 44.7%  PF 1.45  P&L +2.48U
//   real   WR 32.1%  PF 0.67  P&L -3.56U
// Merged real ledger, 182 closed positions (Jul 17 - Aug 1): WR 37.9%, PF 0.82.
//
// Bybit USDT perps: maker 0.020%, taker 0.055%. A bracket order pays TAKER on the stop-out
// leg essentially always, and paper's own exit model books TP/SL at the exact level, so the
// realistic round trip is maker-in + taker-out = 0.075%. Default set accordingly.
// Slippage default is a conservative 2bp per side on top; illiquid alts are worse.
//
// These are DEFAULTS now, not zeros. Set V4_FEE_RATE=0 to restore fix48y's behaviour, but
...
// fixCONFLICTSPLIT (P1, plan Part 2.4 / step 4): OVERRIDE (admit a counter-regime trade) and CONFLICT
// (reject a trade that fights a fresh coin reversal) both keyed off PCT_STRONG=3. Admitting on weak
// evidence is dangerous; BLOCKING on weak evidence is cheap insurance. So the conflict/reject bar is
// now separate and LOWER by default (2 = 15m + 1 fast TF), letting a fresh-but-not-yet-"strong"
// reversal veto an entry without being allowed to admit a counter-regime trade on the same thin
// evidence. Override still requires the full PCT_STRONG. Set equal to restore old symmetric behaviour.
const PCT_CONFLICT_STRONG  = Math.max(2, Math.min(4, Number(process.env.V4_PCT_CONFLICT_STRONG || '2')));
...
// ============================ fixPARTIALLOCK (hotfix 41, 4.6.9.15) v1 ===========================
// User witnessed the SAME shape 4 times in one session on REAL money: a position reaches ~1R
// favorable, then reverses and gives it all back before the bot's own TP/SL ever fires. Measured
// directly from tonight's 8 real Bybit closes (mfeR = peak favorable excursion in R, from the
// fixMFEALL capture hotfix37 added): every single trade reached POSITIVE mfeR (0.39R-2.66R) — so
// entry timing is NOT the disease (ruled out with the data, not asserted) — the disease is that
// NOTHING protects a favorable excursion once it happens. Of the 4 "wins" tonight, 3 (ERA/COTI/AIO)
// only became wins because the user manually intervened; without those saves the real tally is
// 1W/7L. DEXE round-tripped an entire R in a single candle with zero time to react.
//
// V1 SCOPE, DELIBERATE: this locks the stop to BREAKEVEN once a position reaches +1R — it does NOT
// yet physically bank partial size on Bybit. A true partial-close needs a brand-new reduce-only
// live order call this codebase has never used before (setTradingStopSafe only ever modifies an
// existing TP/SL, never places a new order) — writing that blind, untestable against the real API
// from this sandbox, at the end of an already long live-money session, is a worse risk than shipping
// breakeven-only tonight and building partial-size banking as a fast, carefully-tested follow-up.
// Breakeven alone already eliminates the exact failure mode witnessed 4 times: a real winner
// round-tripping through the ORIGINAL stop into a full loss. Worst case after this fix is ~$0
// (minus fees), not a full R loss.
//
// Both-sides symmetric, fires once per position (partialLockDone flag), only ever tightens (never
// loosens an already-better stop from some other mechanism). Reuses the EXACT rAtExit formula and
// executor.setTradingStopSafe call fix48i-TRAIL already proved live — not new, unproven mechanics
// for the live-stop-move piece.
//
// BUG FOUND AND FIXED IN THE SAME PASS (not hypothetical — traced in code): netSlUSDT is computed
// ONCE at signal birth (calculatePaperMath, ~L1748) from the ORIGINAL stop distance and NEVER
// recalculated. If a stop is later moved (this fix, or the pre-existing fix48i-TRAIL) and price
// later hits the NEW level, the close would incorrectly book the STALE original-distance loss
// instead of the correct amount for the new distance — meaning a "breakeven" stop would have
// silently still booked the full original loss. This fix recomputes netSlUSDT using the same
// formula and the position size already fixed at birth (position doesn't change, only slRaw does).
// NOTE, flagged for a future look, NOT changed here (scope discipline): fix48i-TRAIL has the same
// latent gap — it also moves s.sl without recomputing netSlUSDT. It's fired rarely in practice
// (regime-flip-triggered only) so the exposure has been small, but it is the same bug.
//
// Reversible: V4_PARTIAL_LOCK_ENABLED=false.
// ── fixNOPARTIAL: PARTIAL CLOSE / BREAKEVEN LOCK NOW OFF BY DEFAULT ──────────────────
// MEASURED on 4,918 trend signals across 25 pairs, 8 weeks, path-simulated bar-by-bar with
// conservative intrabar ordering (adverse level assumed hit BEFORE favourable in the same bar):
//
//   variant                    OOS WR    PF     Net P&L
//   NO partial (TP/SL only)     42.3%   1.27    +70.23 U   <-- BEST P&L, every test
//   partial 50% @ 0.5R          72.5%   1.46    +61.29 U
//   partial 50% @ 1.0R          55.2%   1.30    +62.04 U   <-- was the shipped default
//   partial 50% @ 1.5R          46.6%   1.27    +65.80 U
//
// No partial variant beat plain TP/SL on total P&L in ANY test — all signals or extension-gated,
// in-sample or out-of-sample. Partial close is a variance-reduction tool (it converts ~half of
// losses into breakeven scratches, which is why WR jumps to 72%), NOT a profit tool. It costs
// 6-13% of total return. The fraction (33/50/75%) barely matters — identical WR, P&L only falls
// as you bank more. The TRIGGER level is the real lever.
//
// It also caused a REPORTING bug: a breakeven stop-out books positive realizedPnl but sets
// paperState=SL_HIT, so 70 of 123 "LOSS" rows in the 04/08 ledger were actually PROFITABLE
// (true W/L 181/53 = 77% WR, not the reported 112/123 = 47.7%). That corrupted the journal AND
// sentinelBrain's rolling win rate, which gates whether SELL is allowed in a bull regime.
// Turning the feature off removes the source of that mislabelling entirely.
//
// Re-enable with V4_PARTIAL_LOCK_ENABLED=true (0.5R is the best trigger if you ever want the
// smoother equity curve — NOT the 1.0R that was shipped, which is the worst of both).
const PARTIAL_LOCK_ENABLED = String(process.env.V4_PARTIAL_LOCK_ENABLED || 'false').toLowerCase() === 'true';
const PARTIAL_LOCK_TRIGGER_R = Number(process.env.V4_PARTIAL_LOCK_TRIGGER_R || '1.0');
// hotfix42 (fixPARTIALCLOSE): what fraction of the position actually gets banked at the trigger.
// User's confirmed spec: close 50% at 1R, breakeven on the rest.
const PARTIAL_LOCK_FRACTION = Math.min(0.9, Math.max(0.1, Number(process.env.V4_PARTIAL_LOCK_FRACTION || '0.5')));
// Recompute netSlUSDT for a MOVED stop, using the position size fixed at birth (unchanged) and the
// NEW stop distance — mirrors calculatePaperMath's own formula exactly (~L1748) so a breakeven stop
// correctly books ~$0 (just the fee/slippage cost, currently 0 per fix48y) instead of the stale
// original-distance loss.
function recomputeNetSlForMovedStop(s, newSl) {
  const entry = num(s.entry, 0);
  const position = num(s.position, 0);
  if (!entry || !position) return num(s.netSlUSDT, 0); // fail-open: can't recompute, keep existing
  const slRaw = Math.abs(entry - newSl) / entry;
  const grossSl = position * slRaw;
  const cost = position * (FEE_RATE + SLIPPAGE_RATE) * 2;
  return Number((-(grossSl + cost)).toFixed(4));
}
let _pctStats = null;
function _pctReset() { _pctStats = { fresh: 0, fetched: 0, capped: 0, strong: 0, f4leaders: 0, overrides: 0, conflicts: 0, scoreBypass: 0, breadthFightShadow: 0, pctBandBlocked: 0, blocks: {} }; }
...
async function coinTrendEngine(symbol) {
  try {
    ...
    // fixREPAINT: coinTrendEngine fed RAW arrays (forming last bar included) into trendFromCloses,
    // calcADX, calcVWAP and the 15m SMA9/21 cross — the single biggest contributor to the fast-trend
    // read flickering exactly in the 5-15min reversal window the plan flags. trendFromCloses now drops
    // the forming bar itself; here we also hand calcADX/calcVWAP/cross only closed 15m bars so every
    // input to coinTrend.strength agrees on "closed bars only".
    const k15c = closedOnly(k15);
    ...
    const vwap15 = calcVWAP(k15c, 20);
    const lastClose15 = k15c.length ? k15c[k15c.length - 1].close : 0;
    ...
    let cross = null;
    const c15 = k15c.map(c => c.close); // fixREPAINT: closed bars — cross no longer forms on a live wick
    if (c15.length >= 22) {
    ...
    const data = { bias, strength, t1, t3, t5, t15, cross, adx15: calcADX(k15c, 14), vwap15, vwapVote };
...
async function applyPerCoinTrend(leader, gate) {
  ...
  // fixBAND (hotfix37): the admission gate now emits SCORE_BAND_* instead of SCORE_LT_*.
  // fork-A's blanket score WAIVER is deliberately NOT extended to it — waiving a band that survived a
  // within-day permutation test would re-open the exact sub-40 admission hole hotfix37 exists to close,
  // so fork-A is now dormant by design (SCORE_LT_ can no longer be emitted). fixEUPHADX's promote IS
  // extended, because it does not waive anything: it restores a score the EUPH dampener lowered, and
  // that restored score is then re-tested against the band before the reason is dropped.
  const bandBlocked     = leader.rejected && reasons.some(r => /^SCORE_BAND_/.test(String(r)));
  // fixPCTBANDSHADOW (hotfix40): fork-A's score-gate bypass matches SCORE_LT_* and has been dormant
  // since fixBAND (hotfix37) replaced that reason system with SCORE_BAND_* — a strong continuation-
  // entry leader currently gets no rescue from a band-only rejection. Before deciding whether to
  // rewire the bypass, gather the same evidence fork-A itself was built from: how often this actually
  // happens, and what the leader looked like when it did. Record-only, mirrors the existing
  // breadthFightShadow pattern exactly — leader.rejected/gate.ok/rejectReasons are NEVER touched here.
  const bandBlockedCandidate = PCT_SHADOW_ENABLED && bandBlocked && leader.continuationEntry === side;
  ...
  const euphScoreBlocked = EUPH_GUARD_ENABLED && !!leader.euphGuard && (scoreBlocked || bandBlocked);
  ...
  if (!regimeBlocked && !admitting && !bypassCandidate && !breadthFightBlocked && !euphScoreBlocked && !bandBlockedCandidate) return; // decision doesn't hinge on the coin trend — skip the fetch

  const ct = await coinTrendEngine(leader.symbol || leader.sym);
  leader.coinTrend = ct; // stamped for diag/logging always
  // fixCONFLICTSPLIT: gate on the LOWER of the two thresholds so a strength-2 fresh reversal can still
  // reach the conflict branch below. Every OVERRIDE / promote / bypass branch re-checks the full
  // PCT_STRONG itself, so lowering this outer gate cannot loosen any admit path.
  const _minStrength = Math.min(PCT_STRONG, PCT_CONFLICT_STRONG);
  if (!ct || ct.strength < _minStrength) return;
  const ctStrongEnoughToAdmit = ct.strength >= PCT_STRONG; // guards every admit/override/promote branch
  ...
  if (EUPH_ADX_PROMOTE_ENABLED && ctStrongEnoughToAdmit && leader.euphGuard && withSide && ct.adx15 >= EUPH_ADX_MIN) {
    ...
    if (leader.rejected) {
      // fixBAND: drop the score reason ONLY if the RESTORED score actually clears the band. Restoring
      // can also push a score ABOVE the 80 ceiling (the 0W/7L exhaustion end) — that must stay blocked.
      const _bandNowOk = passesScoreBand(leader.score);
      const nonScoreE = reasons.filter(r => {
        const _t = String(r);
        if (/^SCORE_LT_/.test(_t)) return false;
        if (/^SCORE_BAND_/.test(_t)) return !_bandNowOk;
        return true;
      });
  ...
  reconcileScoreAdmission(leader, gate);
  ...
  if (bandBlockedCandidate && withSide) {
    const bandReason = reasons.find(r => /^SCORE_BAND_/.test(String(r))) || null;
    const nonBand = reasons.filter(r => !/^SCORE_BAND_/.test(String(r)));
    // Only worth logging when the band was the LAST barrier — exactly fork-A's own "only waive when
    // score is the sole blocker" rule, applied here to counting rather than acting, so the eventual
    // decision is judged on the same clean cases fork-A itself would ever be able to rescue.
    const bandWasSoleBlocker = nonBand.length === 0
      || nonBand.every(r => /^BTC_.*_BLOCKS_(BUY|SELL)$/.test(String(r)));
    if (bandWasSoleBlocker) {
      const _p = leader.plan || {};
      leader.pctBandShadow = {
        side, entry: _p.entry, sl: _p.sl, tp1: _p.tp1,
        rr: num(leader.math && leader.math.rr, null), score: leader.score,
        bandReason, coinBias: ct.bias, coinStrength: ct.strength,
        t1: ct.t1, t3: ct.t3, t5: ct.t5, t15: ct.t15, cross: ct.cross || null, at: Date.now()
      };
      if (_pctStats) _pctStats.pctBandBlocked++;
      // fixGUARDLOG's throttle is reused here on purpose — this is exactly the shape (a repeat
      // condition on the same symbol across scan ticks) that caused tonight's earlier log-spam
      // problem, and a genuinely strong-trending coin can sit band-blocked for many ticks in a row.
      try {
        if (shouldLogGuard('PCT_BAND_SHADOW', leader.symbol)) {
          addLog('PCT_BAND_SHADOW',
            `${leader.symbol}: ${side} would-be entry=${_p.entry} sl=${_p.sl} tp=${_p.tp1} rr=${num(leader.math && leader.math.rr, 0).toFixed(2)} score=${leader.score} — blocked by ${bandReason}, coin trend ${ct.bias} str${ct.strength} (1m=${ct.t1} 3m=${ct.t3} 5m=${ct.t5} 15m=${ct.t15}${ct.cross ? ' ' + ct.cross : ''})`
          );
        }
      } catch (_l) {}
    }
  ...
  if (bypassCandidate && withSide && ctStrongEnoughToAdmit) {
  ...
  if (regimeBlocked2 && withSide && ctStrongEnoughToAdmit) {
    ...
    if (PCT_CONFLICT_ENABLED) {
      // fixCONFLICTSPLIT: reject a would-be entry that fights the coin trend at the LOWER conflict bar
      // (PCT_CONFLICT_STRONG, default 2) — a fresh reversal only 5-15min old that hasn't yet built full
      // PCT_STRONG agreement can now veto the entry. Admitting still needs the full bar (guarded above).
      leader.rejected = true;
      leader.rejectReasons = [...reasons, `PERCOIN_TREND_CONFLICT_${side}_str${ct.strength}`];
      gate.ok = false;
      // fixGUARDLOG: guard still fires and still rejects every tick — only the repeated identical
      // log line is throttled.
      try { if (shouldLogGuard('PERCOIN_CONFLICT', leader.symbol)) addLog('PERCOIN_CONFLICT', `${leader.symbol}: ${side} rejected — fights coin trend ${ct.bias} str${ct.strength} (1m=${ct.t1} 3m=${ct.t3} 5m=${ct.t5} 15m=${ct.t15})`); } catch (_l) {}
    }
  }
}
...
// ================================ fixGUARDLOG (hotfix 39, 4.6.9.13) =============================
// User caught it directly from Northflank logs: BOTTOM_GUARD and PERCOIN_CONFLICT were logging a
// fresh line for the SAME coin every single scan tick (~20-40s) for HOURS while its condition never
// changed — EULUSDT and AEHRUSDT both showed dozens of near-identical lines stacked back to back.
// This is unconditional per-candidate logging, exactly the class of thing the existing
// _sweepSuppressLogged comment above already warns about ("the trade log is a 1000-entry ring and
// an unthrottled sweep could evict everything else in it") — BOTTOM_GUARD/PERCOIN_CONFLICT just
// never got the same treatment.
// NOTE what this does NOT change: the underlying guard logic (score dampening / rejection) still
// runs and re-evaluates EVERY tick, exactly as before — a coin leaving the toxic zone the very next
// tick is still caught instantly. Only the LOG LINE is throttled, because the guard's real effect
// (score/rejection) doesn't need a disk write to take effect; the log is purely a human-visibility
// aid, and re-writing an identical verdict every 20-40s adds nothing a person can use.
// TIME-based (not the sweep pattern's log-once-ever Set), because these conditions genuinely change
// over hours — a coin can leave the bottom-breadth zone and return later, and that return deserves
// a fresh log line, not permanent silence. Cooldown default 5min: long enough to kill the observed
// spam (~10-15x per 5min down to 1x), short enough that an active investigation still sees recent
// state within a few minutes, not hours.
const GUARD_LOG_COOLDOWN_MS = Math.max(30 * 1000, Number(process.env.V4_GUARD_LOG_COOLDOWN_MS || '300000'));
const _guardLogLastAt = new Map(); // key: `${type}:${symbol}` -> last-logged timestamp
function shouldLogGuard(type, symbol) {
  const key = `${type}:${symbol}`;
  const now = Date.now();
  const last = _guardLogLastAt.get(key);
  if (last && (now - last) < GUARD_LOG_COOLDOWN_MS) return false;
  if (_guardLogLastAt.size > 2000) _guardLogLastAt.clear(); // same safety-valve pattern as _sweepSuppressLogged
  _guardLogLastAt.set(key, now);
  return true;
}
let snapshot = {
  ok: true,
...
// ── fixREPAINT (P0-1): CLOSED-CANDLE DISCIPLINE ────────────────────────────────────────────────
// Bybit's /v5/market/kline returns the CURRENT, STILL-FORMING candle as the newest element. After
// fetchKline's ascending sort that bar sits at index length-1. Every indicator in this file that
// reads a raw fetchKline array has therefore been reading a partially-built bar:
//   • trendFromCloses (SMA 9/21/50)  — the label flips mid-candle and flips back = the "trend lag"
//     and whipsaw the charts kept showing; it is repaint, not lag.
//   • volRatio (volNow / avg20)      — a 5m bar 30s old holds ~10% of its eventual volume AND is
//     itself inside the 20-bar average, so volRatio reads systematically LOW. This is why the
//     ledger is full of volRatio 0.03-0.25 and why the `volume` score component is 0 on nearly
//     every birth: the volume signal has been effectively dead, not merely weak.
//   • calcADX / calcVWAP / the 15m SMA9-21 cross inside coinTrendEngine — same contamination.
// structureDetector.analyzeStructure ALREADY does this correctly (k5.slice(0,-1)), so the codebase
// was internally inconsistent: structure saw closed bars, trend/volume/ADX/VWAP did not. Backtests
// run on closed bars only, which is a direct mechanical source of backtest/live divergence.
// A forming bar is still the right input for the ENTRY-TIMING path (live tick price, zone touch,
// directional reaction) — that path is untouched here on purpose.
// Reversible: V4_USE_CLOSED_CANDLES=false restores the old contaminated behaviour exactly.
const USE_CLOSED_CANDLES = String(process.env.V4_USE_CLOSED_CANDLES || 'true').toLowerCase() === 'true';
function closedOnly(candles) {
  if (!USE_CLOSED_CANDLES) return Array.isArray(candles) ? candles : [];
  if (!Array.isArray(candles) || candles.length < 2) return Array.isArray(candles) ? candles : [];
  return candles.slice(0, -1);
}

// fixSMASHORT (P1-4): sma() silently averages FEWER than `len` values when the series is short —
// slice(-50) on a 30-bar array returns 30 bars and avg() happily returns a number. That made
// trendFromCloses' `sma(closes,50) || slow` fallback unreachable (a number is never falsy), so a
// 30-49 bar series was labelled using a "50 SMA" that was really a 30-49 SMA. Strict variant
// returns null so the caller can decide honestly.
function smaStrict(values, len) {
...
function trendFromCloses(closes) {
  ...
  // fixREPAINT: drop the still-forming last close so the SMA label stops flipping mid-candle.
  const c = closedOnly(closes);
  if (c.length < 30) return 'unknown';
  const price = c[c.length - 1];
  const fast = sma(c, 9);
  const slow = sma(c, 21);
  const base = smaStrict(c, 50) || slow; // fixSMASHORT: honest 50-SMA-or-fallback (was silently a short SMA)
...
function detectSwings(candles, lr = 3) { // FIX7: was lr=2; too strict, missed many valid swing highs/lows // anchor-detectSwings
  const highs = [], lows = [];
...
async function fetchTickers() {
  ...
  return list
    ...
    .map(x => ({
      symbol: x.symbol,
      sourceAt: Number(data.time) || null, receivedAt:Date.now(), priceSource:'BYBIT_REST_TICKER',
...
// fixOOM: hard LRU cap. Sized for the working set actually needed in one scan cycle, not
// for the whole universe — a 530-pair scan only needs the entries it is currently walking.
// ~800 entries x ~40KB = ~32MB steady state instead of ~85MB and climbing.
// Tune with V4_KLINE_CACHE_MAX; raising it above ~1200 on a 512MB container risks the crash again.
const KLINE_CACHE_MAX = Math.max(100, Math.min(5000, parseInt(process.env.V4_KLINE_CACHE_MAX || '800', 10)));
...
async function fetchKline(symbol, interval = '5', limit = 100) {
  ...
  if (KLINE_CACHE_MS > 0) {
    // ── fixOOM: THE CACHE NEVER EVICTED. This is the heap-out-of-memory crash. ──────────
    // Old code: `if (size > 3000) { delete entries older than KLINE_CACHE_MS (12s) }`.
    // BOTH conditions were unreachable in practice:
    //   (a) 530 pairs x ~4 intervals = ~2120 keys — never exceeds the 3000 cap, so the
    //       prune block never ran at all; and
    //   (b) even if it had, every key is re-set with a FRESH `at` every 12s scan cycle, so
    //       nothing is ever older than the cutoff and the loop would delete zero entries.
    // Result: a permanently-growing Map holding ~2120 x (100-200 candles x 7 numbers)
    // ≈ 85MB+ of live objects that GC can never reclaim, inside a 512MB container whose
    // Node heap ceiling is ~258MB. The log shows the death spiral exactly:
    //   "300482 ms: Mark-Compact (reduce) 234.8 (258.0) -> 234.7 (258.5) MB"
    // — a 300-SECOND garbage collection that freed 0.1MB, then FATAL heap limit.
    //
    // Fix: true LRU with a HARD cap. Map preserves insertion order, so deleting the key
    // before re-setting moves it to the back; entries are then evicted from the front
    // (least recently used) whenever the cap is exceeded — unconditionally, not age-gated.
    if (_klineCache.has(key)) _klineCache.delete(key);   // refresh LRU position
    _klineCache.set(key, { at: Date.now(), data: out });
    const cap = KLINE_CACHE_MAX;
    while (_klineCache.size > cap) {
      const oldest = _klineCache.keys().next();
      if (oldest.done) break;
      _klineCache.delete(oldest.value);
    }
  }
  return out;
}
...
function calculatePaperMath(plan, settings) {
  ...
  // ── fixSIZECAP: THE 0.5U-0.75U LOSSES — a default mismatch, not a design flaw ────────────
  // The canonical setting (line ~1427, currentSettings()) defaults v4RiskPerTradeUsdt to 0.25.
  // THIS function had its own separate default of 0. Whenever `settings` here is a raw/partial
  // object that skipped the normalizer — a real, findable case: every oversized loss traces to
  // UAIUSDT/HFTUSDT/1000RATSUSDT/SSPCUSDT, all signals that came through a retry/rebuild path —
  // v4RiskPerTradeUsdt was undefined, num(undefined, 0) = 0, the `if (riskPerTrade > 0)` guard
  // below went FALSE, and sizing silently fell back to FIXED_NOTIONAL = margin*leverage (50U),
  // with NO regard for how tight the stop was. A 1.37% stop (UAIUSDT) on a 50U position risks
  // 0.68U — 2.75x the 0.25 target — which is exactly the -0.743U loss observed.
  // Fix: this function's own default now matches the canonical one, so it is self-defending
  // regardless of which caller forgot to route settings through the normalizer. This does not
  // fix whichever upstream path drops the field — that is still worth finding — but it closes
  // the actual money-losing hole immediately and unconditionally, at the one place all sizing
  // must pass through.
  const riskPerTrade = Math.max(0, num(settings.v4RiskPerTradeUsdt, 0.25));
...
function convictionScore(ctx, decision, plan, math, btcRegime, levels, marketPermission = currentMarketPermission()) {
  ...
  // Patch 2.4: BTC context is diagnostic; neutral multiplier comes from the authority.
  const btcR = String(btcRegime.regime || 'UNKNOWN').toUpperCase();
  const regimeStrength = 0;
  let regimeMult = sideAllowance.scoreRegimeMultiplier;
  const regimeNote = sideAllowance.reason;
  const _contestedBull = false;
  if (localRegime === 'LOCAL_CHOP') maxTargetAtr = Math.min(maxTargetAtr, 2.2);
...
// fixWIRE: local ATR (Wilder-ish EMA of true range) from closed candles. Self-contained so
// the extension gate never depends on a field that may not be populated yet.
function atrFromCandles(k, price, period = 24) {
  try {
    if (!Array.isArray(k) || k.length < period + 2) return Math.max(Number(price) * 0.006 || 0, 0);
    const trs = [];
    for (let i = Math.max(1, k.length - (period * 3)); i < k.length; i++) {
      const h = Number(k[i].high), l = Number(k[i].low), pc = Number(k[i - 1].close);
      if (![h, l, pc].every(Number.isFinite)) continue;
      trs.push(Math.max(h - l, Math.abs(h - pc), Math.abs(l - pc)));
    }
    if (!trs.length) return Math.max(Number(price) * 0.006 || 0, 0);
    const tail = trs.slice(-period);
    const v = tail.reduce((a, b) => a + b, 0) / tail.length;
    return Number.isFinite(v) && v > 0 ? v : Math.max(Number(price) * 0.006 || 0, 0);
  } catch (_e) { return Math.max(Number(price) * 0.006 || 0, 0); }
}

// ── fixWIRE: EXTENSION FEATURES ───────────────────────────────────────────────────────
// Computes the inputs the extension gate needs, from the SAME k5 array the rest of the
// plan builder already uses. All look-back only — no future bars touched.
//   ext50 : (log(price) - log(SMA50)) / sigma, SIGNED so POSITIVE = extended in trade dir
//   room  : distance to the prior 48-bar swing extreme, expressed in units of the TP distance
//   atrPct: percentile rank of current ATR within the last 288 bars (0..1)
// Returns nulls when there is not enough history — the gate fails OPEN on null by design.
function computeExtensionFeatures(k5, price, side, tpDistance) {
  try {
    if (!Array.isArray(k5) || k5.length < 60) return { ext50: null, room: null, atrPct: null };
    const closes = k5.map(c => Number(c.close)).filter(Number.isFinite);
    if (closes.length < 60) return { ext50: null, room: null, atrPct: null };
    const dir = side === 'BUY' ? 1 : -1;
    const px = Number(price) > 0 ? Number(price) : closes[closes.length - 1];

    // SMA50
    const last50 = closes.slice(-50);
    const sma50 = last50.reduce((a, b) => a + b, 0) / last50.length;
    if (!(sma50 > 0) || !(px > 0)) return { ext50: null, room: null, atrPct: null };

    // sigma = per-bar log-return stdev over the same window
    const rets = [];
    for (let i = closes.length - 60; i < closes.length; i++) {
      if (i > 0 && closes[i] > 0 && closes[i - 1] > 0) rets.push(Math.log(closes[i] / closes[i - 1]));
    }
    if (rets.length < 20) return { ext50: null, room: null, atrPct: null };
    const mu = rets.reduce((a, b) => a + b, 0) / rets.length;
    const varr = rets.reduce((a, b) => a + (b - mu) * (b - mu), 0) / (rets.length - 1);
    const sigma = Math.sqrt(Math.max(varr, 0));
    if (!(sigma > 0)) return { ext50: null, room: null, atrPct: null };

    const ext50 = dir * (Math.log(px) - Math.log(sma50)) / sigma;

    // room to prior 48-bar swing extreme, in units of the TP distance
    let room = null;
    const win = k5.slice(-49, -1);
    if (win.length >= 20 && Number(tpDistance) > 0) {
      ...
      const raw = side === 'BUY' ? (hi - px) : (px - lo);
      if (Number.isFinite(raw)) room = raw / Number(tpDistance);
    }

    // ATR percentile over last 288 bars
    let atrPct = null;
    if (k5.length >= 120) {
      const trs = [];
      for (let i = Math.max(1, k5.length - 288); i < k5.length; i++) {
        const h = Number(k5[i].high), l = Number(k5[i].low), pc = Number(k5[i - 1].close);
        if (![h, l, pc].every(Number.isFinite)) continue;
        trs.push(Math.max(h - l, Math.abs(h - pc), Math.abs(l - pc)));
      }
      if (trs.length >= 60) {
        const cur = trs.slice(-24).reduce((a, b) => a + b, 0) / Math.min(24, trs.length);
        const below = trs.filter(v => v <= cur).length;
        atrPct = below / trs.length;
      }
    }
    return {
      ext50: Number.isFinite(ext50) ? ext50 : null,
      room: Number.isFinite(room) ? room : null,
      atrPct: Number.isFinite(atrPct) ? atrPct : null,
    };
  } catch (_e) {
    return { ext50: null, room: null, atrPct: null };
  }
}


// ── fixMETA: LIVE FEATURE BUILDER ──────────────────────────────────────────────────────
// Must reproduce the TRAINING features exactly. Any name mismatch or scaling difference makes
// the model score noise, so the names below are copied verbatim from the trained artifact's
// feature list and every value uses the same formula as meta_features.py.
//
// Features the live bot genuinely cannot reproduce (cross-sectional peer stats computed across
// all 50 pairs at the same instant, and the strategy's own rolling win rate) are deliberately
// LEFT UNDEFINED rather than faked with a plausible-looking constant. Undefined follows the
// model's trained default branch, which is the correct, honest behaviour for a missing input —
// inventing a value would shift the score in an unvalidated direction.
// The ablation study showed the model still delivers PF ~1.40 vs 1.21 baseline with those
// features entirely removed, so this degrades gracefully rather than collapsing.
function buildMetaFeatures(ctx, k5, side, extFeat, price) {
  const out = {};
  try {
    const s = side === 'BUY' ? 1 : -1;
    const closes = (k5 || []).map(c => Number(c.close)).filter(Number.isFinite);
    const highs  = (k5 || []).map(c => Number(c.high)).filter(Number.isFinite);
    const lows   = (k5 || []).map(c => Number(c.low)).filter(Number.isFinite);
    // fixMETAFEED: the old `if (closes.length < 210) return out;` bailed out on EVERY live call
    // because ctx.k5 was fetched with limit=100. An empty feature object scores 0.609 — above the
    // 0.47 threshold — so 100% of signals passed and the filter did nothing, silently.
    // (Observed live: evaluated=75, passed=75, blocked=0.)
    // Now: 130 is the hard floor (enough for every feature except the SMA200 family), and the
    // SMA200-dependent features are individually skipped when history is short instead of
    // discarding all 28. Partial features are correct behaviour; an empty object is not.
    // Floor of 60 (not 130): a newly-listed pair with short history should still get the
    // features it CAN support rather than an empty object, because an empty object scores
    // 0.609 and silently auto-passes. Feature families that need more history are each
    // guarded individually below.
    if (closes.length < 60) return out;
    const n = closes.length;
    const has200 = closes.length >= 210;
    const has50 = closes.length >= 60;
    const px = Number(price) > 0 ? Number(price) : closes[n - 1];
    const lc = x => Math.log(x);

    const mean = a => a.reduce((x, y) => x + y, 0) / a.length;
    const sd = a => { const m = mean(a); return Math.sqrt(a.reduce((x, y) => x + (y - m) * (y - m), 0) / Math.max(1, a.length - 1)); };
    const smaN = k => mean(closes.slice(-k));

    // per-bar log-return sigma (same window as training: span-ish 120 -> use last 120)
    const rets = [];
    for (let i = Math.max(1, n - 120); i < n; i++) if (closes[i] > 0 && closes[i - 1] > 0) rets.push(Math.log(closes[i] / closes[i - 1]));
    const sigma = sd(rets);
    if (!(sigma > 0)) return out;

    const s21 = smaN(21), s50 = smaN(50);
    out.side_num = s;
    out.is_buy = s === 1 ? 1 : 0;
    out.ext21 = s * (lc(px) - lc(s21)) / sigma;
    out.ext50 = Number.isFinite(extFeat && extFeat.ext50) ? extFeat.ext50 : s * (lc(px) - lc(s50)) / sigma;
    if (has200) out.ext200 = s * (lc(px) - lc(smaN(200))) / sigma;
    ...
    const atrAbs = mean(atrArr.slice(-24)) / px;
    out.atr_abs = atrAbs;
    const atrSeries = [];
    for (let j = 24; j <= atrArr.length; j++) atrSeries.push(mean(atrArr.slice(j - 24, j)) / px);
    out.atr_pct = Number.isFinite(extFeat && extFeat.atrPct) ? extFeat.atrPct
                 : atrSeries.filter(v => v <= atrAbs).length / Math.max(1, atrSeries.length);

    const rShort = rets.slice(-12), rLong = rets.slice(-96);
    const volS = sd(rShort), volL = sd(rLong);
    out.volratio = volL > 0 ? volS / volL : undefined;
    const volWin = [];
    for (let j = 12; j <= rets.length; j++) volWin.push(sd(rets.slice(j - 12, j)));
    out.volofvol = volS > 0 ? sd(volWin.slice(-48)) / volS : undefined;

    const rngArr = [];
    for (let i = n - Math.min(288, n); i < n; i++) rngArr.push((highs[i] - lows[i]) / closes[i]);
    const curRng = (highs[n - 1] - lows[n - 1]) / closes[n - 1];
    out.rngpct = rngArr.filter(v => v <= curRng).length / Math.max(1, rngArr.length);

    // RSI(14) Wilder
    let up = 0, dn = 0;
    for (let i = n - 14; i < n; i++) { const d = closes[i] - closes[i - 1]; if (d > 0) up += d; else dn -= d; }
    up /= 14; dn /= 14;
    const rsi = dn > 0 ? 100 - 100 / (1 + up / dn) : 100;
    out.rsi = rsi;
    out.rsi_side = s === 1 ? rsi : 100 - rsi;
    let up3 = 0, dn3 = 0;
    for (let i = n - 17; i < n - 3; i++) { const d = closes[i] - closes[i - 1]; if (d > 0) up3 += d; else dn3 -= d; }
    up3 /= 14; dn3 /= 14;
    const rsiPrev = dn3 > 0 ? 100 - 100 / (1 + up3 / dn3) : 100;
    out.rsi_slope = s * (rsi - rsiPrev);

    const mom = k => s * (lc(closes[n - 1]) - lc(closes[n - 1 - k])) / (sigma * Math.sqrt(k));
    out.mom1 = mom(1); out.mom3 = mom(3); out.mom6 = mom(6); out.mom12 = mom(12);
    if (n > 49) out.mom48 = mom(48);

    const hi = highs[n - 1], lo = lows[n - 1], cl = closes[n - 1];
    const rng = Math.max(hi - lo, 1e-12);
    out.clp = s === 1 ? (cl - lo) / rng : (hi - cl) / rng;

    let rl = 1;
    for (let i = n - 1; i > 1; i--) {
      const d1 = Math.sign(closes[i] - closes[i - 1]), d0 = Math.sign(closes[i - 1] - closes[i - 2]);
      if (d1 !== 0 && d1 === d0) rl++; else break;
    }
    out.run_len = rl;

    const prevHi = Math.max(...highs.slice(-49, -1));
    const prevLo = Math.min(...lows.slice(-49, -1));
    const tpDist = 2 * 3.0 * (sigma * px);
    out.room = Number.isFinite(extFeat && extFeat.room) ? extFeat.room : ((s === 1 ? prevHi - px : px - prevLo) / tpDist);
    if (has200) {
      const hi200 = Math.max(...highs.slice(-201, -1));
      const lo200 = Math.min(...lows.slice(-201, -1));
      out.room200 = (s === 1 ? hi200 - px : px - lo200) / tpDist;
    }

    out.sma_stack = s * ((s21 - s50) / px) / sigma;
    out.trend_edge = Number(ctx && ctx.trendEdge);
    out.w5c = Number(ctx && ctx.w5Count) || 0;
    out.magmin = Number(ctx && ctx.magMin);

    const d = new Date();
    out.hour = d.getUTCHours();
    out.dow = (d.getUTCDay() + 6) % 7;   // pandas dayofweek: Monday=0

    // xs_* and roll_wr_* intentionally omitted — see note above.
  } catch (_e) { /* partial features are fine; missing ones take the model's default branch */ }
  // strip non-finite so metaFilter treats them as missing rather than 0
...
function buildStructurePlan(ctx, settings, btcRegime) {
  ...
  // ── fixWIRE: DIRECTION ENGINE + EXTENSION GATE (actually on the hot path) ────────────
  // DIRECTION_MODE=trend  (default) restores sideFromTrends from the 70%-WR build, with
  //                       Elliott demoted back to VETO-only (its role before fix48).
  // DIRECTION_MODE=fade   reproduces 4.6.9.x behaviour (W5 steers) — for A/B only.
  // DIRECTION_MODE=hybrid trend leads; a fade is allowed only on a strong, unopposed W5.
  //
  // Then the EXTENSION GATE — the single strongest pre-execution filter found in the
  // 8-week / 7,968-signal study. Five candidate features all measured ONE thing
  // (correlations 0.87-0.95): how far price already ran in the trade's direction.
  // Worst quintile (ext50 > 6.3) ran WR 35.6% and lost money outright.
  //   baseline   WR 41.2% / 41.0% (IS/OOS)  PF 1.20 / 1.22
  //   gated      WR 43.8% / 43.0%           PF 1.33 / 1.29
  // Monotone across all five quintiles, and IS/OOS AGREE — unlike every earlier filter.
  //
  ...
  if (_dirMode === 'fade') {
    decision = determineDirection(trend5, trend15, trend1h, elliottAnalysis, structure, ctx.coinTrend);
  } else {
    decision = directionMode.decideDirection({
      trend5, trend15, trend1h,
      rsi: num(rsi, 50),
      momentumPct: num(momentumPct, 0),
      volRatio: num(volRatio, 1),
      elliottAnalysis,
    });
  }
  ...
  if (decision.side === 'NEU') return { ok: false, reason: 'NO_DIRECTION', leader: { symbol, score: 0, side: 'NEU', dirMode: _dirMode, dirReasons: decision.reasons || [] } };

  // Extension gate. Applied to EVERY mode (an over-extended fade is just as bad as an
  // over-extended trend entry). tpDistance uses the ATR-based target scale already in ctx;
  // if it is not yet known at this point, `room` simply comes back null and is skipped.
  {
    // ctx.atr is NOT populated at this point in the pipeline (verified — it is derived later
    // from `levels`). Compute ATR locally from the same closed-candle array so `room` is a
    // real number rather than silently null.
    const _atr = atrFromCandles(k5c, price);
    const _tpDist = _atr > 0 ? _atr * 2 : 0;   // TP scale ~2 ATR; only sets `room`'s units
    const _extFeat = computeExtensionFeatures(k5c, price, decision.side, _tpDist);
    const _gate = directionMode.extensionGate({
      ext50: _extFeat.ext50, room: _extFeat.room, atrPct: _extFeat.atrPct,
    });
    ctx.extFeatures = _extFeat;   // surfaced for the ledger/diagnostics
    ctx.extGate = _gate;
    if (!_gate.pass) {
      return { ok: false, reason: 'EXT_GATE_BLOCKED', leader: {
        symbol, score: 0, side: 'NEU', dirMode: _dirMode,
        extGateReason: _gate.reason, ext50: _extFeat.ext50, room: _extFeat.room, atrPct: _extFeat.atrPct,
      } };
    }

    // ── fixMETA: META-LABELING GATE ───────────────────────────────────────────────────
    // The direction engine above says WHICH WAY. This says WHETHER TO TAKE IT AT ALL.
    // 150-tree GBM, trained on FEE-ADJUSTED labels (a trade only counts as good if it nets
    // positive after 7.5bp), validated walk-forward across 5 test months with a 6h purge and
    // embargo — every test month was scored by a model that had only ever seen earlier months.
    //   @7.5bp fees:  baseline PF 1.03 (4/5 months green)  ->  filtered PF 1.46 (5/5 green)
    //   win rate ~41% -> ~49%, keeping ~15-20% of signals.
    // Fails OPEN: any load or inference error allows the trade and logs the reason. A broken
    // model must never silently stop the bot from trading.
    try {
      const _mf = metaFilter.shouldTake(buildMetaFeatures(ctx, k5c, decision.side, _extFeat, price));
      ctx.metaP = _mf.p;
      ctx.metaPass = _mf.pass;
      if (!_mf.pass) {
        return { ok: false, reason: 'META_BLOCKED', leader: {
          symbol, score: 0, side: 'NEU', dirMode: _dirMode,
          metaP: _mf.p, metaThreshold: _mf.threshold, metaReason: _mf.reason,
        } };
      }
    } catch (_e) {
      // fail open, never block trading on a filter bug
      ctx.metaP = null; ctx.metaPass = true;
    }
  }
  ...
  if (side === 'BUY') {
    ...
    const _pullBuyRaw = (PCT_PULLBACK_ENABLED && ctx.coinTrend && ctx.coinTrend.strength >= PCT_STRONG && ctx.coinTrend.bias === 'up') ? PCT_PULLBACK_ATR : 0.6;
    // Patch 2.4.4: planner/executor geometry parity. A newly-born unplaced limit must not
    // start beyond the executor's own missed-move envelope (v4MissedMoveAtr, default 0.5 ATR).
    // This does not loosen the parity threshold; it constrains plan construction to it.
    const _pullBuy = Math.min(_pullBuyRaw, Math.max(0, num(settings.v4MissedMoveAtr, 0.5)));
    ...
    const _isContinuationBuy = _pullBuyRaw !== 0.6; // classify from strategy's raw pullback request, before parity clamp
    ...
    const _pullSellRaw = (PCT_PULLBACK_ENABLED && ctx.coinTrend && ctx.coinTrend.strength >= PCT_STRONG && ctx.coinTrend.bias === 'down') ? PCT_PULLBACK_ATR : 0.6;
    const _pullSell = Math.min(_pullSellRaw, Math.max(0, num(settings.v4MissedMoveAtr, 0.5)));
    ...
    const _isContinuationSell = _pullSellRaw !== 0.6;
  ...
  // fixINVERT (Aug 1): everything above this line — structure detection, support/resistance,
  // entry zone, stop distance, target distance — is UNCHANGED, exactly as the brain has always
  // computed it. This block only flips WHICH SIDE gets traded, at the last possible moment,
  // mirroring the already-computed SL/TP distances around the same entry price so risk sizing
  // and RR are preserved exactly — a SELL at this entry with the same stop distance and the
  // same target distance, just on the other side. Everything downstream (RR gates, scoring,
  // entry-confirmation timing) runs on the flipped, self-consistent side. Default OFF.
  // Reversible: V4_INVERT_SIGNALS=false (default) is a complete no-op, byte-identical to before
  // this fix. Scope note: this covers the structure-based planner (buildPlan), which is the
  // majority path (TRAP/DIVERGENCE/BOS ≈ 95% of real trades this session). Contrarian-flip
  // children and any entry path that does not route through buildPlan are NOT yet covered —
  // flagged for a fast follow-up once this is confirmed live, not silently claimed as full coverage.
  if (String(process.env.V4_INVERT_SIGNALS || 'false').toLowerCase() === 'true') {
    const _invSlDist = Math.abs(entry - sl);
    const _invTpDist = Math.abs(tp1 - entry);
    const _invSide = side === 'BUY' ? 'SELL' : 'BUY';
    const _invSl = _invSide === 'BUY' ? entry - _invSlDist : entry + _invSlDist;
    const _invTp = _invSide === 'BUY' ? entry + _invTpDist : entry - _invTpDist;
    slSource += ' [V4_INVERT_SIGNALS: mirrored, was ' + side + ']';
    tpSource += ' [V4_INVERT_SIGNALS: mirrored, was ' + side + ']';
    entrySource += ' [V4_INVERT_SIGNALS active]';
    side = _invSide;
    sl = _invSl;
    tp1 = _invTp;
  }
    ...
    try {
      ...
      if (_bp && _bp.pctile != null) {
        ...
        if (_inToxicZone) {
          ...
          // fixGUARDLOG: score dampening still applies to every candidate every tick — only the
          // repeated identical log line is throttled.
          if (shouldLogGuard('BOTTOM_GUARD', leader.symbol)) addLog('BOTTOM_GUARD', `${leader.symbol}: ${_sideU} in breadth range-${_sideU === 'SELL' ? 'BOTTOM' : 'TOP'} zone (pctile=${_bp.pctile}, breadth=${_bp.cur} in [${_bp.lo}..${_bp.hi}] over ${_bp.windowMin}m) — score ${_before}→${leader.score}`);
        }
      }
      ...
      if (_bp && _bp.cur != null) {
        ...
        if (_inEuphZone) {
          ...
          leader.euphGuard = { side: _sideU, from: _before, to: leader.score, mult: EUPH_CONV_MULT,
                               ...
                               btcRsi: _bp.btcRsi };
          // fixGUARDLOG: same twin condition as BOTTOM_GUARD above, found in the same review pass —
          // identical unthrottled-per-tick shape, same helper.
          if (shouldLogGuard('EUPH_GUARD', leader.symbol)) addLog('EUPH_GUARD', `${leader.symbol}: ${_sideU} at absolute breadth ${_sideU === 'BUY' ? 'euphoria' : 'capitulation'} (bull_pct=${_bp.cur}, ${_sideU === 'BUY' ? '>=' + EUPH_BREADTH_HI : '<=' + EUPH_BREADTH_LO}) — score ${_before}→${leader.score}`);
        }
...
function recordWithhold24(s,reason,at,price){
  // 2.4.2: compact episode summary only. Repeated per-tick sample arrays were forensic noise
  // and dominated multi-day exports. Keep first/last/min/max/count — enough to prove persistence.
  const episodes=s.withheldEpisodes||(s.withheldEpisodes=[]);
  let e=episodes[episodes.length-1];
  if(!e||e.reason!==reason){e={reason,firstAt:at,lastAt:at,count:0,firstPrice:price,lastPrice:price,minPrice:price,maxPrice:price};episodes.push(e);require('./executionParity').appendEvent(s,'ORDER_WITHHELD',at,{reason,episode:episodes.length});}
...
function timingDiagnostics24(s,price,atr,timing){
  const leader={...s,price,plan:{entry:s.entry}};
  const total=deriveEntryTimingScore(leader,timing),base=deriveEntryTimingScore(leader,{});
  const threshold=Math.max(1,Number(process.env.V4_TIMING_BLOCK_THRESHOLD||'44'));
  const tolerance=entryToleranceAbs(s.entry,atr,currentSettings());
  // Exact breakpoints of the piecewise proximity function, plus both endpoints.
  const points=[0,tolerance,...[0.0015,0.003,0.005,0.01].flatMap(f=>[s.entry*f-1e-10,s.entry*f,s.entry*f+1e-10])].filter(x=>x>=0&&x<=tolerance);
  const reaction=entryReactionAbs(s.entry,atr,currentSettings())*(s.hadFirstTouch?.5:1);
...
function signalEntryTiming(s, price, atr, now, settings = currentSettings()) {
  ...
  const favorableDistance = s.side === 'BUY' ? price-entry : entry-price;
  const missedMove = !inZone && atr>0 && favorableDistance>atr*num(settings.v4MissedMoveAtr,0.5);
  const birthValue=s.priceAtBirth??s.price;
  const birthPrice=birthValue!=null&&Number(birthValue)>0?Number(birthValue):NaN;
  s.missedMoveDiagnostics={distanceFromEntryAtr:atr>0?favorableDistance/atr:null,
    birthDistanceAtr:atr>0 && Number.isFinite(birthPrice)?(s.side==='BUY'?birthPrice-entry:entry-birthPrice)/atr:null,
    movementSinceBirthAtr:atr>0 && Number.isFinite(birthPrice)?(s.side==='BUY'?price-birthPrice:birthPrice-price)/atr:null,
    classification:missedMove?'UNPLACED_PRICE_BEYOND_LIMIT':'NOT_MISSED', detectionLatency:'NOT_MEASURED'};
  ...
  if (!inZone) {
    ...
    return { inZone, directional: false, tolerance, reaction, missedMove, expiredWindow: false, reason: missedMove ? 'ORDER_WITHHELD: unplaced price beyond planned limit; detection latency not established' : 'WAITING_ENTRY: price outside tight entry zone' };
  }
...
// fixSIDELOCK (plan Part 2.1): loss cooldown was strictly SAME-SIDE — a fresh SL on the SELL side
// left the BUY side free to re-enter the same symbol immediately (and vice-versa), so a chopping
// pair could bleed alternating-direction losses that no cooldown ever caught. Default OFF to keep
// behaviour identical until backtested on the 50-pair set; enable with V4_SYMBOL_LOSS_BOTH_SIDES=true
// to make ANY recent loss on a symbol lock BOTH directions for the same cooldown window.
const SYMBOL_LOSS_BOTH_SIDES = String(process.env.V4_SYMBOL_LOSS_BOTH_SIDES || 'false').toLowerCase() === 'true';
function symbolCooldownReason(signals, symbol, side, now = Date.now(), settings = currentSettings()) {
  ...
  // fixSIDELOCK: loss rows respect _lossSideFilter (either-side when V4_SYMBOL_LOSS_BOTH_SIDES=true).
  const lossRows = SYMBOL_LOSS_BOTH_SIDES
    ? combinedLedgerSignalRows(signals).filter(s => s && (s.sym === symbol || s.symbol === symbol))
    : rows;
...
function refreshSignalPlan(existing, candidate, reason = 'REFRESHED_WAITING_SETUP') {
  if(existing?.executionPosition?.intent?.status==='ACCEPTED'||existing?.tradeId||existing?.liveOrderId)return false;
...
function shouldReject(leader, settings, signals = []) {
  ...
  if(!marketPolicy.allows(permission,_sideUp)) reasons.push('BREADTH_SIDE_BLOCKED_'+_sideUp);
  warnings.push(permission.reason);
  ...
  // fixBAND: score quality band is now the admission rule; the RSI zone is recorded, not enforced.
  if (!passesScoreBand(leader.score)) reasons.push(scoreBandReason(leader.score));
  leader.rsiGateShadow = passesRsiDirectionGate(leader.side, leader.rsi) ? 'PASS' : 'FAIL';
  if (!RSI_GATE_SHADOW_ONLY && !passesRsiDirectionGate(leader.side, leader.rsi)) reasons.push(`RSI_GATE_${leader.side}_${leader.rsi}`);
...
function signalFromLeader(leader, settings) {
  ...
  try {
    ...
    // fixBAND (hotfix37): the admission stamp now records BOTH the band that actually admitted the
    // signal and the (now shadow-only) RSI verdict, so the caged trial can score them against each
    // other on real closes without hand reconstruction.
    _birthDiag.scoreAtBirth = num(leader.score, null);
    _birthDiag.scoreBandAtBirth = passesScoreBand(leader.score) ? 'IN' : 'OUT';
    _birthDiag.rsiGateShadowAtBirth = leader.rsiGateShadow
      || (passesRsiDirectionGate(leader.side, leader.rsi) ? 'PASS' : 'FAIL');
    try {
      addLog('SCORE_BAND_PASS', `${leader.symbol} ${leader.side} — score=${_birthDiag.scoreAtBirth} band=${SCORE_BAND_LO}-${SCORE_BAND_HI} | rsi=${_birthDiag.rsiAtEntry} rsiShadow=${_birthDiag.rsiGateShadowAtBirth}`, {
        symbol: leader.symbol, side: leader.side, score: _birthDiag.scoreAtBirth,
        rsi: _birthDiag.rsiAtEntry, rsiGateShadow: _birthDiag.rsiGateShadowAtBirth,
      });
  ...
  return {
    ...
    score: leader.score,
    priceAtBirth:price,forensicTiming:{...(leader.forensicTiming||{}),candidateBornAt:now},scoreAdmission:leader.scoreAdmission||null,marketPermission:leader.marketPermission||currentMarketPermission(),
...
function saveSignals(signals) {
  for(const s of signals||[]) {
    if(s.entryHit)observations.link(s);
    else if(FINAL_STATES.has(s.paperState))observeDecision24(s,s.paperState,s.stateReason||s.paperState,s.closedAt||Date.now());
  }
  observations.flush();
...
// Unified entry-decision engine. Patch 2.4.6 promotes this function to the actual PAPER placement
// authority after the phase trace demonstrated repeated WOULD_PLACE / NOT_LIVE divergence.
// It remains a pure function and the REAL-BYBIT execution path is intentionally unchanged.
// A true missed move, knife block, max-open condition, sentinel blind state, breadth-side block,
// or absent first-touch/confirmation still returns wouldPlace=false.
...
function normalizeLedgerTrade(s) {
  ...
  return ledgerObservability.clone({
    ...
    key,
    planKey,
    patchVersion:'PATCH-2.4.6',scoreAdmission:s.scoreAdmission||null,marketPermission:s.marketPermission||null,
...
// Patch 2.4.4: generation fence. Manual destructive resets increment this so an
// already-running scan cannot repersist the pre-clear signal/ledger snapshot after the clear.
let _stateMutationEpoch = 0;
...
function saveLedger(rows) {
  for(const row of rows||[])if(!row.entryHit&&FINAL_STATES.has(row.paperState))observeDecision24(row,row.paperState,row.stateReason||row.paperState,row.closedAt||Date.now());
  observations.flush();
  const now = Date.now();
  ...
  // fixLEDGERCACHE: skip the disk write when nothing that matters actually changed since the last
  // write — a structural-signature match is a provable no-op, never a guess. Any real state change
...
function walkOutcome(row, candles) {
  ...
  for (const c of candles) {
    if (!filled) {
      // ── fixHONESTPAPER (B): FILL REQUIRES TRADE-THROUGH, NOT A TOUCH ─────────────────
      // A resting limit order is NOT filled because price kissed your level — you are at the
      // back of the queue at that price. You are reliably filled only once price trades
      // THROUGH your level, clearing the book ahead of you. Paper's touch-fill is optimistic
      // in exactly the cases that matter: the level that gets touched once and reverses is a
      // free winner in paper and a no-fill in reality.
      // Buffer is expressed in bp of price; V4_PAPER_FILL_BUFFER_BP=0 restores touch-fill.
      const _fillBufBp = Math.max(0, Number(process.env.V4_PAPER_FILL_BUFFER_BP ?? '1'));
      const _buf = row.entry * (_fillBufBp / 1e4);
      const touchedEntry = side === 'BUY' ? (c.low <= row.entry - _buf) : (c.high >= row.entry + _buf);
...
// ================================ fixBAND (hotfix 37, 4.6.9.11) ================================
// SCORE QUALITY BAND — replaces passesRsiDirectionGate() as the candidate ADMISSION rule.
//
// WHY, and why this reverses a standing rule. The long-standing "score is dead weight (r=-0.07 over
// 537 trades)" finding was measured with a LINEAR correlation on a relationship that is not linear.
// Measured again on the 167 resolved signals of the 07/25-07/31 export, bucketed rather than
// correlated:
//     score <40   -> 27% WR (n=41)
//     score 40-80 -> 60% WR (n=119)
//     score 80+   ->  0% WR (n=7)
// Bad at BOTH ends, good in the middle — the two tails cancel, which is exactly how a U-shaped
// signal reads as r~0 under a linear test. The old rule was not wrong about the number, it was
// wrong about the test.
//
// Validation actually run before shipping (not asserted):
//   * leave-one-day-out: the band beats the outside band on 6 of 6 days that carry trades on both
//     sides (67/0, 64/33, 62/17, 38/0, 58/40, 38/23) — it is not one day carrying the result.
//   * WITHIN-DAY stratified permutation (20k shuffles, outcomes permuted inside each day so a single
//     bad session cannot manufacture the gap): observed 36.7pp, p=0.0032.
//   * threshold sensitivity: floors 35/40/45 and ceilings 75/80/85 all land in the same place
//     (WR 56-61%, net +20.9 to +28.9) — a plateau, not a knife-edge, so this is not curve-fit to a
//     single cut point.
// Kept trades = 119/167 (71%), so this is a quality filter, not a volume choke.
//
// WHAT THIS REPLACES. hotfix33 deleted the score floor entirely and made the RSI zone the sole
// admission rule. On the same export the RSI zones do not separate at all (BUY buckets 30-40 43%,
// 40-50 50%, 50-60 41%, 60-70 65%) and the practical effect of the deletion was a composition shift:
// pre-hotfix33 only 16/134 resolved trades scored <40; post-hotfix33 it was 25/33. The bleed
// (post-deploy BUY 4W/21L) tracks that shift. RSI is NOT deleted here — it is demoted to a shadow
// stamp (rsiGateShadow / rsiGateShadowAtBirth) so it keeps accumulating evidence for a later re-test
// with more real closes, exactly as hotfix34 flagged the BUY 55-70 zone as under-sampled.
//
// Both sides symmetric by construction — the band is side-agnostic, it reads one number.
// Reversible: set SCORE_BAND_ENABLED=false to restore hotfix33 behaviour (RSI gate sole rule);
// LO/HI are named constants, not env vars, per the standing hardcode-config rule.
const SCORE_BAND_ENABLED = true;
const SCORE_BAND_LO = 40;   // inclusive floor
const SCORE_BAND_HI = 80;   // EXCLUSIVE ceiling — 80+ measured 0W/7L (exhaustion end)
const RSI_GATE_SHADOW_ONLY = true; // hotfix33's gate kept as evidence, no longer admits/rejects
function passesScoreBand(score) {
  if (!SCORE_BAND_ENABLED) return true;
  if (score === null || score === undefined) return true; // fail-open, same stance as the RSI gate
  const v = Number(score);
  if (!Number.isFinite(v)) return true;
  return v >= SCORE_BAND_LO && v < SCORE_BAND_HI;
}
// ================================ fixARM (hotfix 37, 4.6.9.11) =================================
// CANCEL A RESTING LIMIT THAT IS ABOUT TO BE FILLED BY A KNIFE.
//
// fixKNIFEGUARD (hotfix24) checks the 1m candle body ONCE, at the moment the retest limit is PLACED.
// After that the GTC limit rests on Bybit and can fill minutes or hours later with nothing
// re-checking momentum. That is the structural hole behind the fill-and-die pattern: a passive limit
// at a pullback zone only fills when price moves AGAINST the trade into it, so fills are biased
// toward pullbacks that keep going. Supporting evidence from the 07/25-07/31 export (65 rows that
// carry excursion data): fills that happened within 30s of signal birth ran 50% WR vs 76% for fills
// 30s+ later, and the median LOSING trade only ever reached +0.27R in its favour (43% never passed
// +0.25R) — i.e. losses are mostly immediately-wrong fills, not give-backs. Directionally clear,
// but n is small and the within-day permutation on the fill-speed split is p=0.19, so this is
// [Likely], NOT proven — it is judged by the caged live session, not claimed as validated.
//
// WHAT IT DOES: restingOrderWatch() already re-runs the SL-breach and re-validation checks every 10s
// for the small set of signals carrying a resting order. This adds a third check on the same pass —
// the SAME knife test fixKNIFEGUARD uses at placement — and on a hit cancels the order through the
// SAME flagTradeForCancel()/STALE_CANCELLED path the re-validation ABANDON branch already uses. No
// new state, no new terminal state, no cancel-and-replace loop, and no new network calls beyond one
// cached fetchKline per resting order per 10s tick (bounded by maxOpenTrades, and the whole function
// early-returns to zero calls when nothing is resting).
//
// DELIBERATE SCOPE LIMIT: this does NOT re-arm. A cancelled setup dies; a fresh signal for the same
// symbol can be born later through the normal path. Re-arming would need cancel/fill race handling
// on a live account and is not worth the risk before the mechanism has proven it fires at all.
//
// NOTE ON THE CANDLE (real discrepancy, deliberately NOT changed here): fetchKline sorts ascending,
// so the LAST element is the still-FORMING bar. fixKNIFEGUARD reads that last element even though
// its comment says "last completed" — which makes it systematically weak in the first seconds of a
// minute. fixARM reads index -2, the genuinely closed bar, which is the correct read for a cancel
// decision. fixKNIFEGUARD is left alone to keep this diff scoped; flagged for a later pass.
//
// Both sides symmetric (bearish body vs BUY, bullish body vs SELL — same test, mirrored).
// Reversible: ARM_CANCEL_ENABLED=false restores hotfix36 behaviour exactly.
const ARM_CANCEL_ENABLED = true;

// ============================== fixMFEALL (hotfix 37, 4.6.9.11) ================================
// Excursion capture (mfeR/maeR) only ever ran on the PAPER path. Both live paths skip it: the
// unifiedBrain seam `continue`s before any paper logic for every order-bearing signal, and the
// liveStateAuthority ACTIVE branch calls initActiveDiagnostics once and then `continue`s, so
// trackActiveDiagnostics (which lives further down inside the paper PAPER_ACTIVE block) never runs
// for a real position. Result: mfeR stayed 0 or absent on live trades — only 65 of 167 resolved
// rows in the last export carried it, and the missing ones were exactly the real-money trades the
// entry-model work needs. This is pure observability: no gate reads mfeR/maeR anywhere.
function syncLiveDiagnostics(s, market, price, now, settings) {
  try {
    if (!s) return;
    const st = String(s.paperState || '').toUpperCase();
    const isOpen = (st === 'PAPER_ACTIVE' || st === 'ACTIVE' || st === 'LIVE_OPEN');
    if (isOpen) {
      const px = num(price, 0) || num(s.entry, 0);
      if (!px) return;
      if (!s.diag || !num(s.diag.riskAbs, 0)) {
        const _atrForDiag = num(s.planner && s.planner.keyLevels && s.planner.keyLevels.atr, 0)
                         || num(s.atr, 0) || Math.abs(num(s.entry) - num(s.sl));
        initActiveDiagnostics(s, px, _atrForDiag, now, settings);
      } else {
        trackActiveDiagnostics(s, market || {}, px, now);
      }
      return;
    }
    if (FINAL_STATES.has(st) && s.diag && num(s.diag.riskAbs, 0) && !s.diag._finalized) {
      finalizeActiveDiagnostics(s, now);
      s.diag._finalized = true;
    }
  } catch (_e) { /* observability only — never break a live tick */ }
}

function scoreBandReason(score) {
  const v = Number(score);
  const shown = Number.isFinite(v) ? Math.round(v) : 'NA';
  return `SCORE_BAND_${shown}_OUTSIDE_${SCORE_BAND_LO}_${SCORE_BAND_HI}`;
}
...
function reconcileOrphanedLedgerRows(ledger, signals) {
  ...
  const rows = (ledger || []).map(r => {
    ...
    if (ACTIVE_STATES.has(st) && rid && !liveIds.has(rid)) {
      if(r.executionPosition?.remainingQty>0){
        // An absent signal is not evidence that a filled position expired.
        signals.push({...r,paperState:'PAPER_ACTIVE',displayState:'PAPER_ACTIVE',positionStatus:'PAPER'});
        return r;
      }
      if(!r.tradeId&&!r.liveOrderId)paperOrders.cancel(r.executionPosition,now,'LOST_ACTIVE_WINDOW');
      reconciledCount++;
      return {
        ...r,
        ...
        history:[...(r.history||[]),{at:now,state:'EXPIRED',reason:'LOST_ACTIVE_WINDOW'}],
      };
    }
    return r;
  });
...
function activeFinalFromMarket(s, market, price) {
  ...
  const _slSlipBp = Math.max(0, Number(process.env.V4_PAPER_SL_SLIP_BP ?? '3'));
  const _slipped = side === 'BUY' ? sl * (1 - _slSlipBp / 1e4) : sl * (1 + _slSlipBp / 1e4);
  const _tpSlipBp = Math.max(0, Number(process.env.V4_PAPER_TP_SLIP_BP ?? '0'));
  const _tpFill = side === 'BUY' ? tp * (1 - _tpSlipBp / 1e4) : tp * (1 + _tpSlipBp / 1e4);

  // ── fixEXITWALK (P0-3): WALK EVERY FETCHED CANDLE, OLDEST FIRST, NOT JUST THE NEWEST ONE ──────
  // market.recentCandles (see enrichOpenSignalMarketMap) now carries every 1m candle fetched since
  // this signal's last check, not only the most-recently-closed one. Walking them in chronological
  // order and returning the FIRST touch reproduces what a real resting order would have hit, even
  // if a scan cycle overran and several candles closed between checks. Falls back to the single
  // high/low snapshot (old behaviour) if recentCandles is absent — e.g. other/legacy call sites.
  // Reversible: V4_EXIT_WALK_ALL_CANDLES=false (set upstream; recentCandles is then just [last]).
  const candles = Array.isArray(market.recentCandles) && market.recentCandles.length
    ? market.recentCandles
    : (market.high || market.low ? [{ ts: candleTs, high: market.high, low: market.low }] : []);

  for (const c of candles) {
    const cTs = num(c.ts, candleTs);
    // Same guard as before: a candle that STARTED before this signal was activated can't be used
    // for its whole range (pre-entry price action would falsely trigger TP/SL) — fall back to the
    // tick price only for that one candle.
    const rangeStartedBeforeEntry = cTs && openedAt && openedAt > cTs && openedAt < cTs + candleIntervalMs;
    const rangeUsable = !rangeStartedBeforeEntry;
    const high = rangeUsable ? num(c.high, price) : price;
    const low = rangeUsable ? num(c.low, price) : price;
    ...
    if (tpHit && slHit) return { state: 'SL_HIT', status: 'LOSS', reason: `${V4_VERSION}: SL and TP touched same range; conservative SL first-hit rule`, exitPx: _slipped, rangeUsable, slSlipBp: _slSlipBp, resolvedAtTs: cTs };
    if (slHit) return { state: 'SL_HIT', status: 'LOSS', reason: `${V4_VERSION}: SL hit`, exitPx: _slipped, rangeUsable, slSlipBp: _slSlipBp, resolvedAtTs: cTs };
    return { state: 'TP_HIT', status: 'WIN', reason: `${V4_VERSION}: TP hit`, exitPx: _tpFill, rangeUsable, tpSlipBp: _tpSlipBp, resolvedAtTs: cTs };
  }
  return null;
}
...
function flagTradeForCancel(s, why) {
  if(s&&!s.tradeId&&!s.liveOrderId)paperOrders.cancel(s.executionPosition,Date.now(),why);
...
function updateExistingSignals(signals, priceMap) {
  ...
  for (const s of signals) {
    ...
    const latePaperOrder=!s.tradeId&&!s.liveOrderId&&s.executionPosition?.cancelState==='ACKNOWLEDGED'&&s.executionPosition.status!=='CLOSED'&&s.executionPosition.intent.requestedQty-s.executionPosition.filledQty>1e-12&&s.executionPosition.cancelAcknowledgedAt!=null;
    ...
    s.lastPriceSourceAt=q24.valid?q24.at:null;s.lastPriceReceivedAt=q24.valid?q24.receivedAt:null;
    if(!s.tradeId && !s.liveOrderId && (paperOrders.working(s.executionPosition)||latePaperOrder)) {
      const p=s.executionPosition;
      // The accepted order owns its side and bracket, regardless of later planner refreshes.
      s.side=p.intent.side;
      if(!p.filledQty){s.entry=p.intent.plannedEntry;s.plannedEntry=p.intent.plannedEntry;s.sl=p.intent.sl;s.tp1=p.intent.tp;}
      const fill=paperOrders.tick(p,market,now,FEE_RATE);
      if(latePaperOrder&&!fill.filled&&_finalNow)continue;
      s.executionIntent=p.intent;
      s.pendingFillDiagnostic={at:now,reason:fill.reason||'FILLED',sourceAt:q24.at};
      if(fill.filled){
    ...
    if (require('./unifiedBrain').flagEnabled()) {
      ...
      if (_ub.handled) { syncLiveDiagnostics(s, market, price, now, settings); changed = true; continue; } // fixMFEALL
    }
    // === end UNIFIED BRAIN =====================================================================
    ...
    if (_live) {
      if (_live.kind === 'ACTIVE') {
        ...
        syncLiveDiagnostics(s, market, price, now, settings); // fixMFEALL: excursion tracking for a real position
    ...
    if (!price) continue;
    if(!s.entryHit && !s.tradeId && !s.liveOrderId && !q24.valid){
      s.pendingFillDiagnostic={at:now,reason:'STALE_OR_MISSING_PRICE'};changed=true;continue;
    }
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION' || s.paperState === 'CANDIDATE') {
      ...
      s.entryTiming=timing;
      s.timingDiagnostics=timingDiagnostics24(s,price,atr,timing);
      // Patch 2.4.6: one entry authority for PAPER.
      // decideLiveEntry() used to be shadow-only while the real paper path still had an extra
      // outer `timing.inZone && timing.directional` gate. That produced the observed contradiction:
      // unified=WOULD_PLACE, actual=NOT_LIVE, no accepted paper intent. On a genuine retest,
      // hadFirstTouch already proves the first zone visit happened and price left; the correct
      // live-parity behavior is to REST a limit at the structural entry and wait for a later fresh
      // touch, not wait for a directional move that occurs only after the entry opportunity.
      //
      // Promotion is PAPER-ONLY. Real-Bybit placement remains on its existing guarded path.
      const _paperMode246 = !(settings.botMode === 'LIVE_REAL_BYBIT' && settings.tradingEnabled);
      const _paperActiveCount246 = signals.filter(x => x && x.id !== s.id &&
        (x.paperState === 'PAPER_ACTIVE' || paperOrders.working(x.executionPosition))).length;
      const _paperUnifiedEligibility246 = _paperMode246 && passesScoreBand(s.score)
        ? decideLiveEntry(s, market, timing, settings, atr, _paperActiveCount246, scanPermission)
        : null;

      // Keep Phase-1 observability. In paper mode, "actual" now also counts an acknowledged
      // paper working order, so WOULD_PLACE/NOT_LIVE no longer falsely reports a divergence
      // merely because the paper order is correctly waiting for a later touch.
      try {
        const _p1Verdict = _paperUnifiedEligibility246 ||
          decideLiveEntry(s, market, timing, settings, atr, _p1ActiveCount, scanPermission);
        const _p1ActuallyLive = !!(s.liveOrderId || s.tradeId || paperOrders.working(s.executionPosition));
        phase1ShadowTrace(s, _p1Verdict, _p1ActuallyLive);
      } catch (_p1e) { /* observability only — never let it touch the real tick loop */ }
      ...
      if (timing.missedMove && !s.tradeId && !s.liveOrderId) {
          const parity = require('./executionParity');
          s.forensicTiming={...(s.forensicTiming||{}),intentDecisionAt:now};
          if(s.forensicTiming.candidateBornAt)s.forensicTiming.birthToIntentMs=now-s.forensicTiming.candidateBornAt;
          if(s.forensicTiming.scanStartedAt)s.forensicTiming.scanToIntentMs=now-s.forensicTiming.scanStartedAt;

          // Patch 2.4.5: MISSED_MOVE is a transient placement veto, NOT an order lifecycle.
          // Before this patch we stored the WITHHELD object in executionIntent. That made a
          // momentary "price ran past the unplaced limit" episode look like authoritative order
          // state even after price later returned to an executable zone. It also polluted the
          // lifecycle evidence with a permanent WITHHELD intent although no order ever existed.
          //
          // Keep the forensic observation, but never let it occupy executionIntent. A later
          // fresh, non-missed, in-zone directional confirmation is free to create a brand-new
          // ACCEPTED intent. That accepted order still must ACK first and can fill ONLY on a
          // later fresh LAST-price touch through paperOrderLifecycle — no same-tick/chase fill.
          const _withheldObservation = parity.createIntent(
            s, now,
            { missedMove:true, hadFirstTouch:!!s.hadFirstTouch, qty:num(s.position)/num(s.entry) }
          );
          s.lastWithheldIntent = _withheldObservation;
          if (s.executionIntent?.status === 'WITHHELD' &&
              s.executionIntent?.reason === 'MISSED_MOVE_LIVE_PARITY' &&
              !s.executionPosition) {
            s.executionIntent = null;
          }
          s.paperState = 'WAITING_ENTRY'; s.displayState = 'WAITING_ENTRY'; s.status = 'DETECTED'; s.positionStatus = 'NONE'; s.entryHit = false;
          s.stateReason = `${V4_VERSION}: transient missed-move hold; no order exists; awaiting executable retrace`;
          recordWithhold24(s,'MISSED_MOVE_LIVE_PARITY',now,price);
          observeDecision24(s,'ORDER_WITHHELD','MISSED_MOVE_LIVE_PARITY',now,market);
          changed = true;
          continue;
        }

      // Patch 2.4.5 migration/self-heal: a pre-patch transient MISSED_MOVE hold must not remain
      // authoritative once the current market state is no longer missed. Preserve it as forensic
      // history and clear only the non-order-bearing WITHHELD placeholder. ACCEPTED intents and
      // any executionPosition are never touched here.
      if (!timing.missedMove &&
          s.executionIntent?.status === 'WITHHELD' &&
          s.executionIntent?.reason === 'MISSED_MOVE_LIVE_PARITY' &&
          !s.executionPosition && !s.tradeId && !s.liveOrderId) {
        s.lastWithheldIntent = s.executionIntent;
        s.executionIntent = null;
        s.executionReason = 'PATCH-2.4.6: transient missed-move hold cleared after executable retrace';
        changed = true;
      }

      const _paperUnifiedReady246 = !!(_paperUnifiedEligibility246 && _paperUnifiedEligibility246.wouldPlace);
      const _legacyTimingReady246 = timing.inZone && timing.directional;
      if (_legacyTimingReady246 || (_paperMode246 && _paperUnifiedReady246)) {
        ...
        if (_activationTimingScore >= 100) {
          ...
          observeDecision24(s,'TIMING_HOLD','ENTRY_TIMING_TOO_HIGH',now,market);
          s.paperState = 'WAITING_ENTRY';
          s.displayState = 'WAITING_ENTRY';
          s.status = 'DETECTED';
          s.positionStatus = 'NONE';
        ...
        // Patch 2.4.6: reuse the SAME verdict that opened this paper placement path.
        // No second entry authority / retest rule is allowed here.
        const _eligibility = _paperUnifiedEligibility246 ||
          (passesScoreBand(s.score)
            ? decideLiveEntry(s, market, timing, settings, atr, _activeCount, scanPermission)
            : {wouldPlace:false,reason:scoreBandReason(s.score)});
        ...
        if (_intent.status !== 'ACCEPTED') {
          ...
          parity.appendEvent(s,'ORDER_REJECTED',now,{reason:_intent.reason,observedPrice:price}); observeDecision24(s,'ORDER_REJECTED',_intent.reason,now,market); changed=true; continue;
        }
        const _position = parity.newPosition(_intent);
    ...
    if (s.paperState === 'PAPER_ACTIVE') {
      ...
      // fixPARTIALLOCK v2 (hotfix42): check BEFORE the normal TP/SL evaluation so a stop that
      // reaches breakeven this same tick is already in place when activeFinalFromMarket reads s.sl
      // below. v1 (hotfix41) only moved the stop; user correctly pushed back that breakeven alone
      // never banks anything — a round-trip back to breakeven leaves nothing to show for a real 1R
      // move. v2 actually banks PARTIAL_LOCK_FRACTION (default 50%) of the position, on paper always
      // and on Bybit for real when a live position exists (executor.placeReduceOnlyClose, hotfix42 —
      // reuses the exact order-placement pattern already firing real orders tonight, side inverted +
      // reduceOnly:true so Bybit itself refuses to let a bad qty/side open a new position).
      if (PARTIAL_LOCK_ENABLED && !s.partialLockDone && price) {
        const _plEntry = num(s.entry);
        const _plSlDist = Math.abs(_plEntry - num(s.sl)) || 1e-9;
        const _plRNow = String(s.side).toUpperCase() === 'BUY'
          ? (price - _plEntry) / _plSlDist
          : (_plEntry - price) / _plSlDist;
        if (_plRNow >= PARTIAL_LOCK_TRIGGER_R) {
          const isBuy = String(s.side).toUpperCase() === 'BUY';
          const newSl = roundPrice(_plEntry); // breakeven = entry price, exactly
          const currentlyTighter = isBuy ? (num(s.sl) >= newSl) : (num(s.sl) <= newSl);
          if (!currentlyTighter) {
            const oldSl = s.sl;
            // Capture the FULL-SIZE dollar-per-R BEFORE anything is halved — this is what banks the
            // real, current-price profit on the closed fraction, not an idealized exact-1R number.
            const dollarPerR = Math.abs(num(s.netSlUSDT, 0));
            const fraction = PARTIAL_LOCK_FRACTION;
            const partialPnl = Number((fraction * dollarPerR * _plRNow).toFixed(4));
            s.sl = newSl;
            // The position is now (1-fraction) of its original size — both the eventual TP payout
            // AND the breakeven-stop payout must scale down to match, or a full-size number would
            // be booked against a partial-size position.
            s.netTpUSDT = Number((num(s.netTpUSDT, 0) * (1 - fraction)).toFixed(4));
            ...
            try {
              addLog('PARTIAL_LOCK', `${s.symbol} ${s.side}: +${_plRNow.toFixed(2)}R reached — banked ${(fraction*100).toFixed(0)}% (+${partialPnl}U), stop moved to breakeven on the rest (${oldSl}->${newSl})`);
            } catch (_l) {}
            if (s.tradeId) {
              // Real money: attempt the reduce-only partial close AND move the real stop. These are
              // independent outcomes on purpose — if the close order fails (rejected, below exchange
              // minimum, network error), the breakeven stop move still goes through, so the live
              // position is never LESS protected than the v1 behaviour even on a partial failure.
              (async () => {
                try {
                  const _trades = store.read('trades', {});
                  const _trade = _trades[s.tradeId];
                  const _liveQty = num(_trade && _trade.qty, 0);
                  if (_liveQty > 0) {
                    const closeResult = await require('./executor').placeReduceOnlyClose({
                      symbol: s.symbol, side: s.side, qty: _liveQty, fraction, signalId: s.id,
                    });
                    if (!closeResult.ok) {
                      s.partialCloseLiveFailed = true; // surfaced in diag/ledger — visible, not silent
                      addLog('WARN', `partial-lock: LIVE partial close failed for ${s.symbol} (${closeResult.error || 'unknown'}) — paper still banked ${fraction*100}% for tracking, but Bybit position is full size; breakeven stop still applied to protect it`);
                    } else {
                      addLog('PARTIAL_LOCK_LIVE', `${s.symbol}: real Bybit partial close filled, qty=${closeResult.qty}`, { orderId: closeResult.orderId });
                    }
                  } else {
                    s.partialCloseLiveFailed = true;
                    addLog('WARN', `partial-lock: no live qty found for ${s.symbol} tradeId=${s.tradeId} — skipped real partial close, stop still moved to breakeven`);
                  }
                } catch (e) {
                  s.partialCloseLiveFailed = true;
                  addLog('WARN', `partial-lock: live partial close threw for ${s.symbol}: ${e.message}`);
                }
                // Reuses fix48i-TRAIL's exact live-stop-move call — already proven in production.
                try {
                  const stopResult = await require('./executor').setTradingStopSafe({ symbol: s.symbol, tp1: s.tp1, sl: newSl, side: s.side, bybitSide: s.side === 'BUY' ? 'Buy' : 'Sell' });
                  if (!stopResult.ok) addLog('WARN', `partial-lock: live SL move to breakeven failed for ${s.symbol}: ${stopResult.error || JSON.stringify(stopResult.data || {})}`);
                } catch (e) { addLog('WARN', `partial-lock: live SL move threw for ${s.symbol}: ${e.message}`); }
              })();
            }
            changed = true;
          }
      ...
      // fixEXITWALK: record how far the walk actually got, win/loss or not, so next scan's
      // enrichOpenSignalMarketMap only fetches candles AFTER this point — never re-walks the same
      // range, and never has a gap to skip even if this cycle or a future one overruns LOOP_MS.
      // Falls back to `now` when no candle data was available at all (fetch failure this cycle).
      {
        const _candles = Array.isArray(market.recentCandles) ? market.recentCandles : [];
        const _lastCandleTs = _candles.length ? num(_candles[_candles.length - 1].ts, 0) : 0;
        s.lastResolutionCheckTs = final?.resolvedAtTs || _lastCandleTs || now;
      }
      if (final) {
        finalizeActiveDiagnostics(s, now);
  ...
  for(const row of signals){
    if(row.entryHit)observations.link(row);
    else if(FINAL_STATES.has(row.paperState))observeDecision24(row,row.paperState,row.stateReason||row.paperState,row.closedAt||now,priceMap.get(row.sym||row.symbol)||{});
  }
  observations.flush();
  return changed;
}
...
async function enrichOpenSignalMarketMap(signals, basePriceMap) {
  ...
  // fixCANDLECAP (Aug 1): the 30-symbol real-candle budget below was filled in raw array
  // order with no priority. ACTIVE_STATES includes WAITING_ENTRY/WAITING_REACTION/CANDIDATE/
  // FROZEN alongside PAPER_ACTIVE/LIVE_* — on a busy scan (signals list runs up to 120) that
  // pool routinely exceeds 30, and a genuinely open position (real paper P&L, sometimes real
  // money) could silently miss the cut. When that happens, activeFinalFromMarket()'s TP/SL
  // check falls through to market.high||market.low, which for an un-enriched symbol resolves
  // to the raw ticker's highPrice24h/lowPrice24h (fetchTickers, ~L1250) — the 24-HOUR range,
  // not the current 1m candle. That is a materially wider band that can register a false
  // TP/SL touch unrelated to the trade's actual live path. Root-caused Aug 1 while
  // investigating the paper-vs-real TP-reach gap (paper 50.1%, real unaided ~19%); a direct
  // duration-of-resolution check did NOT show the smoking-gun signature (a cluster of
  // near-instant paper WINs) this bug's worst case would predict, so it is shipped as a
  // confirmed, real, worth-fixing-regardless defect — NOT claimed as the sole explanation
  // for the gap. Fix: genuinely open positions (PAPER_ACTIVE/LIVE_OPEN/LIVE_RESTING/
  // LIVE_PENDING) are now guaranteed a slot before any WAITING/CANDIDATE/FROZEN watch-list
  // symbol competes for the remaining budget. Total fetch volume is UNCHANGED (still capped
  // at 30) — this only reorders who gets the real data first. Reversible:
  // V4_CANDLECAP_PRIORITY_ENABLED=false restores the old unordered behaviour.
  //
  // fixEXITWALK (P0-3, machan's cross-referenced-timestamps finding, 2026-08-11): the Aug 1 fix
  // above only re-ordered a still-hard 30-symbol cap and — separately, unconditionally, even for
  // the 30 that DO get real data — activeFinalFromMarket() only ever looked at candles[length-1]:
  // the SINGLE most-recently-closed 1m candle. That is correct only if every candle is checked
  // exactly once, which requires the scan to run at least once per minute without fail. LOOP_MS
  // is 20s, but this file's own changelog documents repeated real incidents of a scan cycle
  // taking 90-300+ seconds under load (kline fetch storms, AI proxy calls, sentinel watchdog
  // timeouts). Any cycle that overruns 60s can let a whole 1m candle close, get silently skipped
  // by the "last candle only" check, and never be re-examined once a later candle becomes "last".
  // A stop-out wick that recovers before the NEXT scan is then never recorded as a loss — paper
  // rides the position to its eventual TP and books a win a real exchange stop-market could never
  // have allowed. Independently confirmed 2026-08-11: a candle-by-candle replay against real
  // market data (every candle walked, none skipped) resolved ledger trades at ~26% WR; paper's
  // own polling-based bookkeeping showed ~60% WR on the same signals.
  // Fix, two parts:
  //  (1) Genuinely open positions (PAPER_ACTIVE/LIVE_OPEN/LIVE_RESTING/LIVE_PENDING) are now
  //      UNCAPPED — they no longer share the 30-slot budget with WAITING/CANDIDATE symbols at
  //      all; every open position gets real candle data every single scan, full stop.
  //  (2) Each open position now fetches EVERY 1m candle since its own last resolution check
  //      (tracked per-signal as s.lastResolutionCheckTs), not just the newest one — so a scan
  //      that overran cannot cause a candle, and the wick inside it, to go unchecked.
  // Reversible: V4_CANDLECAP_PRIORITY_ENABLED=false (part 1) / V4_EXIT_WALK_ALL_CANDLES=false
  // (part 2, restores old single-last-candle behaviour).
  const CANDLECAP_PRIORITY_ENABLED = String(process.env.V4_CANDLECAP_PRIORITY_ENABLED || 'true').toLowerCase() === 'true';
  const EXIT_WALK_ALL_CANDLES = String(process.env.V4_EXIT_WALK_ALL_CANDLES || 'true').toLowerCase() === 'true';
  const EXIT_WALK_MAX_CANDLES = Math.max(3, Number(process.env.V4_EXIT_WALK_MAX_CANDLES || '240')); // 4h safety cap
  const OPEN_STATE_PRIORITY = new Set(['PAPER_ACTIVE', 'LIVE_OPEN', 'LIVE_RESTING', 'LIVE_PENDING']);
  const openPriority = open.filter(s => OPEN_STATE_PRIORITY.has(s.paperState));
  const openOther = open.filter(s => !OPEN_STATE_PRIORITY.has(s.paperState));
  // fixEXITWALK part 1: open positions are no longer part of the capped pool — every one of them
  // gets a real-data slot below, unconditionally. CANDLECAP_PRIORITY_ENABLED=false restores the
  // old shared-pool-with-priority-ordering behaviour exactly (openPriority folded back into the
  // capped list) for anyone who needs to revert under infra pressure.
  const orderedOpen = CANDLECAP_PRIORITY_ENABLED ? openOther : open;
  const uncappedOpenSymbols = CANDLECAP_PRIORITY_ENABLED
    ? [...new Set(openPriority.map(s => String(s.sym || s.symbol || '').toUpperCase()).filter(Boolean))]
    : [];
  // fixEXITWALK review-2: a symbol can legitimately appear in BOTH pools (e.g. an active BUY
  // plus a separate waiting SELL on the same coin). Without deduping, Promise.all below would
  // fetch it twice — once here with the small 3-candle waiting budget, once in the uncapped loop
  // with the full gap-covering lookback — and whichever resolves LAST wins the merged.set(),
  // non-deterministically. That could silently overwrite a correct full walk with a 3-candle one
  // and reopen exactly the bug this fix exists to close. Excluded here; the uncapped loop always
  // owns it, with the correct oversized lookback and rangeUsable-safe candle set.
  const uncappedSet = new Set(uncappedOpenSymbols);
  const symbols = [...new Set(orderedOpen
    .map(s => String(s.sym || s.symbol || '').toUpperCase())
    .filter(Boolean))].filter(sym => !uncappedSet.has(sym)).slice(0, 30);
  // fixEXITWALK part 2: per-symbol candle lookback sized to cover the longest gap since any of
  // that symbol's open signals were last checked (fresh signal → small default lookback so we
  // don't over-fetch on a brand-new position; a signal that survived a slow/overrun cycle gets
  // exactly enough extra candles to cover the gap, capped at EXIT_WALK_MAX_CANDLES as a safety
  // ceiling against a signal whose lastResolutionCheckTs went stale for an unrelated reason).
  const now7395 = Date.now();
  const lookbackFor = new Map();
  for (const s of openPriority) {
    const sym = String(s.sym || s.symbol || '').toUpperCase();
    if (!sym) continue;
    const since = num(s.lastResolutionCheckTs, num(s.openedAt, now7395 - 5 * 60000));
    const gapMin = Math.max(1, Math.ceil((now7395 - since) / 60000)) + 2; // +2 candle buffer
    const need = EXIT_WALK_ALL_CANDLES ? Math.min(EXIT_WALK_MAX_CANDLES, Math.max(3, gapMin)) : 3;
    lookbackFor.set(sym, Math.max(lookbackFor.get(sym) || 0, need));
  }
  ...
  await Promise.all([...symbols, ...uncappedOpenSymbols].map(async symbol => {
    try {
      const need = lookbackFor.get(symbol) || 3;
      const candles = await fetchKline(symbol, '1', need);
      ...
      const merged = {
        ...
        candle: last,
        // fixEXITWALK: full chronological candle set for this symbol's fetch window, so the
        // caller can walk every candle instead of trusting only the newest one.
...
async function enrich(ticker) {
  const [k5, k15, k60] = await Promise.all([
    fetchKline(ticker.symbol, '5', 220),   // fixMETAFEED: was 100 — buildMetaFeatures needs >=210 for SMA200/room200/ext200
  ...
  // fixREPAINT (volRatio): volNow was the FORMING 5m bar (a fraction of its eventual volume) and that
  // same partial bar also sat inside the 20-bar average — so volRatio read systematically low and the
  // `volume` score component was ~0 on almost every birth in the ledger. Use the last CLOSED bar as
  // "current" and average the 20 closed bars before it. Result is a real, comparable volume ratio.
  const k5c = closedOnly(k5);
  const volNow = num(k5c[k5c.length - 1]?.volume);
  const volAvg = avg(k5c.slice(-20).map(c => c.volume));
...
async function autoRetryRejections(currentSnapshot, settings) {
  ...
  for (const r of deduped) {
    ...
    // fixBAND (hotfix37): score band replaces the RSI zone at this second enforcement point too.
    if (!passesScoreBand(payload?.score)) {
      addLog('RETRY_SKIP', `${r.symbol} ${r.side} outside score band: score=${payload?.score} (band ${SCORE_BAND_LO}-${SCORE_BAND_HI})`);
      continue;
    }
    if (!RSI_GATE_SHADOW_ONLY && !passesRsiDirectionGate(payload?.side, payload?.rsi)) {
...
// ── fixOOM: HEAP WATCHDOG ────────────────────────────────────────────────────────────────
// The crash gave no warning in the app log — the first sign was a FATAL from V8 itself after a
// 300-second GC. This logs heap pressure every scan so the next leak is visible while there is
// still time to act, and names the two caches most likely responsible.
let _lastHeapWarnAt = 0;
function heapWatchdog() {
  try {
    const mu = process.memoryUsage();
    const usedMb = mu.heapUsed / 1048576;
    const totalMb = mu.heapTotal / 1048576;
    const limitMb = (require('v8').getHeapStatistics().heap_size_limit) / 1048576;
    const pct = limitMb > 0 ? (usedMb / limitMb) * 100 : 0;
    if (pct >= 75 && Date.now() - _lastHeapWarnAt > 60000) {
      _lastHeapWarnAt = Date.now();
      addLog('HEAP_PRESSURE', `heap ${usedMb.toFixed(0)}/${limitMb.toFixed(0)}MB (${pct.toFixed(0)}%) · klineCache=${_klineCache.size}/${KLINE_CACHE_MAX} · ledgerCache=${_ledgerCache ? _ledgerCache.length : 0}/${MAX_LEDGER}`);
      console.warn(`[v4] HEAP_PRESSURE ${pct.toFixed(0)}% used=${usedMb.toFixed(0)}MB limit=${limitMb.toFixed(0)}MB klineCache=${_klineCache.size} ledgerCache=${_ledgerCache ? _ledgerCache.length : 0}`);
    }
    return { usedMb, totalMb, limitMb, pct };
...
async function scanOnce() {
  ...
  inFlight = true;
  heapWatchdog(); // fixOOM: surface heap pressure before V8 kills the process
  const startedAt = Date.now();
  const scanMutationEpoch = _stateMutationEpoch;
  _pctReset();
  try {
    const settings = currentSettings();
    ...
    for (const t of universe) {
      try {
        const _symbolScanStartedAt = Date.now();
        const ctx = _ctxMap.get(t.symbol) || await enrich(t);
        const _contextReadyAt = Date.now(); // reuse the parallel-prefetched ctx; fall back if a pair failed
        ...
        if (!built.ok) {
          // 2.4.2 compact logging: pre-plan rejects are scanner noise, not execution candidates.
        ...
        leader.forensicTiming={scanStartedAt:startedAt,symbolScanStartedAt:_symbolScanStartedAt,contextReadyAt:_contextReadyAt,planBuiltAt:Date.now()};
  ...
  const gate = shouldReject(leader, settings, signals);
        ...
        reconcileScoreAdmission(leader,gate);
        // 2.4.2: do not persist every per-pair admission decision every sweep. Actual candidate/execution lifecycle is recorded below.
        const _isRejected = !gate.ok;
        if (_isRejected) {
          // 2.4.2 compact logging: rejected pre-admission leaders remain UI scan results only; no durable research row.
        ...
        if (gate.ok && openSignalCount(signals) < settings.maxOpenTrades) {
          ...
          // High-quality structure signals (trap, CHoCH confirmed) can trade any hour
            ...
            if (dupReason === 'DUPLICATE_SYMBOL_SIDE_WAITING') {
              ...
              observeDecision24(sig,'CANDIDATE_BORN','ADMITTED_SIGNAL',sig.createdAt,priceMap.get(leader.symbol)||{});
              signals.unshift(sig);
    ...
    // Patch 2.4.4: if Clear Ledger/New Session mutated durable state while this async scan
    // was in flight, discard the scan's stale local `signals` copy. Without this fence the scan
    // could write old rows back seconds after a successful durable zero-row verification.
    if (scanMutationEpoch !== _stateMutationEpoch) {
      addLog('V4_SCAN_DISCARDED_AFTER_STATE_RESET', 'Discarded in-flight scan after state mutation', { startedAt, scanMutationEpoch, currentEpoch:_stateMutationEpoch });
      return getSnapshot();
    }
    ...
    try {
      if (_pctStats && (_pctStats.fresh > 0 || _pctStats.strong > 0)) {
        ...
        addLog('PCT_SCAN', `fresh=${_pctStats.fresh} fetched=${_pctStats.fetched}${_pctStats.capped ? ` capped=${_pctStats.capped}` : ''} strong=${_pctStats.strong} f4leaders=${_pctStats.f4leaders} overrides=${_pctStats.overrides} scoreBypass=${_pctStats.scoreBypass} pctBandBlocked=${_pctStats.pctBandBlocked} breadthFightShadow=${_pctStats.breadthFightShadow} conflicts=${_pctStats.conflicts} moverLane=${_moverLaneSymbols.size} | strongBlocks: ${b}`);
      }
    } catch (_ps) {}
  }
}
...
function ingestCandidate(payload = {}) {
  ...
  const marketPermission=currentMarketPermission();
  if(!marketPolicy.allows(marketPermission,side))reasons.push('BREADTH_SIDE_BLOCKED_'+side);
  warnings.push(marketPermission.reason);
  ...
  if (isDeadHourLK()) reasons.push(`DEAD_HOUR_NO_NEW_SIGNALS_${deadHourLabel().replace(/\s+/g, '')}`);
  // fixBAND (hotfix37): third and last enforcement point — same band, same fail-open stance.
  // IMPORTANT: `score` above defaults to 0 when the payload carries NO score field at all, and 0 is
  // outside the band — so gate on the raw presence of a score input, not on the defaulted number,
  // or an external submitter that never sends a score would be rejected 100% of the time instead of
  // failing open the way the RSI gate it replaces did.
  if (scoreInputs.length && !passesScoreBand(score)) reasons.push(scoreBandReason(score));
  if (!RSI_GATE_SHADOW_ONLY && !passesRsiDirectionGate(side, payload.rsi)) reasons.push(`RSI_GATE_${side}_${payload.rsi}`);
  ...
  // 2.4.2: pre-admission payload rejects are not durable candidate-forensic rows.
  if (rejected) {
...
async function restingOrderWatch() {
  ...
  try {
    ...
    for (const s of watching) {
      ...
      // 3. fixARM (hotfix37) — re-run fixKNIFEGUARD's own test against the resting order. Placement
      // checked momentum once; this checks it for as long as the order is exposed. Fail-open: any
      // missing candle/ATR leaves the order resting exactly as before.
      if (!ARM_CANCEL_ENABLED || KNIFE_BODY_ATR <= 0) continue;
      // ATR source mirrors the knife check in updateExistingSignals (~L5982): a signal object has NO
      // top-level .atr — it lives under planner.keyLevels.atr, with the SL distance as the fallback.
      const armAtr = num(s.planner && s.planner.keyLevels && s.planner.keyLevels.atr, 0)
                  || num(s.diag && s.diag.atrAtEntry, 0)
                  || Math.abs(num(s.entry) - num(s.sl));
      if (armAtr <= 0) continue;
      let armCandle = null;
      try {
        const armK = await fetchKline(s.symbol, '1', 3);
        // index -2 = last CLOSED bar (the last element is the still-forming one after the asc sort)
        armCandle = (armK && armK.length >= 2) ? armK[armK.length - 2] : null;
      } catch (_ae) { armCandle = null; }
      if (!armCandle) continue;
      const armBody = num(armCandle.close) - num(armCandle.open);
      const armBodyAtr = Math.abs(armBody) / armAtr;
      const armKnife = (side === 'BUY' && armBody < 0 && armBodyAtr >= KNIFE_BODY_ATR)
                    || (side === 'SELL' && armBody > 0 && armBodyAtr >= KNIFE_BODY_ATR);
      if (!armKnife) continue;
      const armReason = `last closed 1m candle ${armBody < 0 ? 'bearish' : 'bullish'} ${armBodyAtr.toFixed(2)}ATR against ${side}`;
      s.paperState = 'STALE_CANCELLED'; s.status = 'STALE_CANCELLED'; s.displayState = 'STALE_CANCELLED';
      s.positionStatus = 'NONE'; s.closedAt = now;
      s.stateReason = `${V4_VERSION}: ARM_CANCEL — resting limit pulled before a knife fill (${armReason})`;
      s.history = [...(s.history || []), { at: now, state: 'STALE_CANCELLED', reason: s.stateReason, price }];
      appendSignalDiagnostic('SIGNAL_ARM_CANCELLED', s, { price, armReason, armBodyAtr, source: 'restingOrderWatch' }, currentSettings());
      addLog('ARM_CANCEL', `${s.symbol} ${side} resting limit cancelled — ${armReason}`, {
        symbol: s.symbol, side, bodyAtr: Number(armBodyAtr.toFixed(3)), threshold: KNIFE_BODY_ATR,
        signalId: s.id, orderId: s.liveOrderId,
      });
      flagTradeForCancel(s, 'ARM_KNIFE_PRE_FILL');
      changed++;
    }
    if (!changed) return;
    saveSignals(signals);
    ...
    snapshot = { ...snapshot, ok: true, ts: now, version: V4_VERSION, signals, visibleSignals: signals.length, ledger: ledger.slice(0, RECENT_LEDGER_LIMIT), summary: { ...computeLedgerSummary(ledger), visible: computeSummary(signals), source: 'v4_paper_ledger' } };
    store.write('v4_snapshot', snapshot);
    addLog('RESTING_WATCH', `${changed} resting order(s) cancelled before fill (structure break, revalidation abandon, or fixARM knife)`);
  } catch (e) {
...
function cancelSignal({ id = '', signalId = '', symbol = '', side = '', reason = 'USER_CANCELLED' } = {}) {
  ...
  signals = signals.map(s => {
    ...
    if(!s.tradeId&&!s.liveOrderId&&s.executionPosition){
      paperOrders.cancel(s.executionPosition,now,reason);
      if(s.executionPosition.remainingQty>0){
        s.paperState='PAPER_ACTIVE';s.displayState='PAPER_ACTIVE';s.status='ACTIVE';s.positionStatus='PAPER';
        s.stateReason='Remainder cancelled; filled position remains open';
        changed=true;cancelled.push(s);return s;
      }
    }
    const next = {
      ...s,
...
function clearSignals({ archive = true, clearLedger = false } = {}) {
  ...
  _stateMutationEpoch++; // invalidate any scan that started before this destructive clear
...
function startNewSession({archive=true,reason='NEW_SESSION'}={}) {
  ...
  _stateMutationEpoch++; // prevent an older in-flight scan from restoring the prior session view
  if(archive && signals.length){
    const sessions=getSessions();
...
module.exports = {
  refreshSignalPlan, reconcileOrphanedLedgerRows,
  buildStructurePlan, convictionScore, signalFromLeader, applyPerCoinTrend, btcRegimeDecision,
  ...
  // proves fixOOM-kline loaded; MAX_LEDGER_ACTIVE lets /health confirm the cap value in effect.
  ...
  getConfigFlags: () => ({ FIXCONFIRM_ENTRY_MARKET, patchVersion:'PATCH-2.4.6', sourceHash:patch24Identity.sourceHash, sourceHashes:patch24Identity.sourceHashes, marketPolicy:marketPolicy.VERSION, scorePolicy:{minInclusive:40,maxExclusive:80}, timingBlockThreshold:Number(process.env.V4_TIMING_BLOCK_THRESHOLD||44), timingUniversalZoneBypass:true, maxPriceAgeMs:paperOrders.MAX_PRICE_AGE_MS, fixRetestEnabled: true, restingWatchMs: RESTING_WATCH_MS, ledgerObservabilitySchema: LEDGER_OBSERVABILITY_SCHEMA, rejectedObservationSchema:observations.SCHEMA }), // fixVERIFY (07/26): lets /health report the live value of entry-placement flags directly from the running process, so a deploy can be confirmed correct with zero live trades — needed because these hooks only fire when tradingEnabled=true, so logs alone can't verify a dry/warmup run.
