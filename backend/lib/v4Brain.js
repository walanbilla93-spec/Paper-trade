...
function currentSettings() {
  ...
  return {
    ...s,
    version: V4_VERSION,
    minScoreToTrade: num(s.minScoreToTrade, 65),
...
function minScoreForSide(side, settings = currentSettings()) {
  ...
  // fix25: fundamental score scale (0-100). Default 45 = A-tier + regime match.
  const fallback = Math.max(0, Math.min(100, num(settings.minScoreToTrade, 45)));
  if (settings.v4UseSideMinScores !== true) return fallback;
...
function minEntryTimingForSide(side, settings = currentSettings()) {
  const fallback = num(settings.minScoreToTrade, 65);
  return String(side || '').toUpperCase() === 'SELL'
...
function btcRegimeDecision(leader, settings, btcUnknownException = false) {
  ...
  if (regime === 'CHOP') {
    ...
    if (mode !== 'off') reasons.push('BTC_CHOP_BLOCKS_TRADE');
    return { reasons, warnings };
  }

  // Hard blocks — regime directly opposed to side
  if (side === 'BUY' && regime === 'BEAR_TREND') {
    reasons.push('BTC_BEAR_TREND_BLOCKS_BUY');
    return { reasons, warnings };
  }
  if (side === 'SELL' && regime === 'BULL_TREND') {
    reasons.push('BTC_BULL_TREND_BLOCKS_SELL');
    return { reasons, warnings };
  }

  // Soft warnings for partial misalignment
  if (side === 'BUY' && (regime === 'BEAR_RANGE')) warnings.push('BTC_BEAR_RANGE_WEAK_FOR_BUY');
  if (side === 'SELL' && (regime === 'BULL_RANGE')) warnings.push('BTC_BULL_RANGE_WEAK_FOR_SELL');

  // Strict mode: require exact alignment
  if (mode === 'strict') {
    if (side === 'BUY' && !['BULL_TREND','BULL_RANGE','BREAKOUT'].includes(regime)) reasons.push(`BTC_${regime}_NOT_BULLISH_FOR_BUY`);
    if (side === 'SELL' && !['BEAR_TREND','BEAR_RANGE','BREAKOUT'].includes(regime)) reasons.push(`BTC_${regime}_NOT_BEARISH_FOR_SELL`);
  }

  // Mixed/neutral (old labels from legacy data)
  if (['MIXED','NEUTRAL'].includes(regime)) {
    warnings.push('BTC_REGIME_MIXED_OR_NEUTRAL');
    if (mode === 'strict' || settings.rejectMixedBtcRegime) reasons.push('BTC_REGIME_MIXED');
  }
...
function shouldReject(leader, settings, signals = []) {
  ...
  // fix24-b: entry timing via new two-gate system (no entryTimingScore field)
  const timing = leader?.entryTiming || {};
  const zoneTouchScore = num(timing.zoneTouchScore, 0);
  const reactionScore = num(timing.reactionScore, 0);
  // Minimum zone touch: price must be within 1.0% of entry (zoneTouchScore >= 6)
  const minZoneTouch = 6;
  // NOT_STALE gate: signal must have zone proximity (fix24 design requirement)
  if (zoneTouchScore < minZoneTouch) reasons.push('NOT_IN_ENTRY_ZONE');
  ...
  const gate = {
    ...
    // fix24-b: new timing fields replace entryTimingScore
