...
function activeFinalFromMarket(s, market, price) {
  ...
  // ── fixHONESTPAPER (C): STOP-OUTS SLIP, TAKE-PROFITS DO NOT ───────────────────────────
  // Paper booked BOTH exits at the exact level. That is right for a TP (a resting limit that
  // the market comes to) and wrong for an SL (a stop-market that fires INTO the move that
  // triggered it). The real ledger shows exactly this asymmetry — avg win +0.40 vs
  // avg loss -0.2833 against a bracket that was supposed to be symmetric in R.
  // Slipping only the loss side is the conservative and realistic choice.
  // V4_PAPER_SL_SLIP_BP=0 restores the old exact-level fill.
  ...
  // ── fixTPSLIP (P0-2, plan Part 4 step 1): TP FILL WAS ALWAYS THE EXACT LEVEL, SL CARRIED SLIP.
  // That asymmetry structurally tilts paper's OWN internal math in its favour on EVERY resolved
  // trade, independent of which trades reach Bybit — the plan's "biggest single reason paper still
  // wins even with live paused." A resting TP limit is genuinely kinder than a stop-market, so TP
  // slip DEFAULTS TO 0 (V4_PAPER_TP_SLIP_BP=0) to keep historical before/after comparable; set it to
  // a small value (e.g. 1) to model queue/latency on the limit and make paper honest end-to-end.
  // TP slip is always ADVERSE (fills slightly WORSE than the level), never better.
  ...
  if (tpHit && slHit) return { state: 'SL_HIT', status: 'LOSS', reason: `${V4_VERSION}: SL and TP touched same range; conservative SL first-hit rule`, exitPx: _slipped, rangeUsable, slSlipBp: _slSlipBp };
  if (slHit) return { state: 'SL_HIT', status: 'LOSS', reason: `${V4_VERSION}: SL hit`, exitPx: _slipped, rangeUsable, slSlipBp: _slSlipBp };
  return { state: 'TP_HIT', status: 'WIN', reason: `${V4_VERSION}: TP hit`, exitPx: _tpFill, rangeUsable, tpSlipBp: _tpSlipBp };
}
...
async function enrichOpenSignalMarketMap(signals, basePriceMap) {
  ...
  const orderedOpen = CANDLECAP_PRIORITY_ENABLED
    ? [...open.filter(s => OPEN_STATE_PRIORITY.has(s.paperState)), ...open.filter(s => !OPEN_STATE_PRIORITY.has(s.paperState))]
    : open;
