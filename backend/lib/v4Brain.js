...
function ingestCandidate(payload = {}) {
  ...
  const _isFrontend = _src.includes('frontend') || _src.includes('search-modal') || _src.includes('candidate');
  // Allow backend-retry path — that's backend-originated, just re-processed
  const _isRetry = _src.includes('retry') || _src.includes('backend-retry');
  if (_isFrontend && !_isRetry) {
    return { ok: false, rejected: true, reasons: ['FRONTEND_SIGNALS_DISABLED'], warnings: [],
      note: 'fix47: All signals generated by backend scanner only. Frontend submission disabled.' };
  }
  const settings = currentSettings();
