...
const V4_VERSION = '4.6.8.47'; // fix47: dead hours removed, RR cap removed (floor 2.0 only), frontend signals blocked, sentinel observer-only
...
const MAX_SCORE_IF_ENTRY_FAR_ATR = Math.max(60, Number(process.env.V4_MAX_SCORE_IF_ENTRY_FAR_ATR || '70')); // fix47d: lowered 78→70 — entry>1ATR past price = missed move, score must reflect lower conviction
const MAX_SCORE_IF_NO_FULL_TREND = Math.max(60, Number(process.env.V4_MAX_SCORE_IF_NO_FULL_TREND || '70')); // fix47d: lowered 78→70 — less than 3 TF trend alignment must score below v4MinSellScore gate (72)
...
function calibratedScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  // fix46c: Elliott Wave score boost — data-driven, same logic as frontend scanner
  if (ctx.elliottAnalysis?.signal) {
    const ew = ctx.elliottAnalysis.signal;
    if (ew.type === 'W5_EXHAUSTION') {
      const boost = ew.confidence === 'HIGH' ? 12 : 8; // 3 TF vs 2 TF
      components.structure = (components.structure || 0) + boost;
      confluence.push(`W5 exhaustion ${ew.w5Count}TF — ${ew.reason}`);
    } else if (ew.type === 'W3_CONTINUATION') {
      components.structure = (components.structure || 0) + 6;
      confluence.push(`W3 continuation ${ew.w3Count}TF — strongest wave`);
    }
  }
  // Also boost single-TF W5 as a warning/moderate signal
  const _w5any = [ctx.elliottAnalysis?.waves?.['5m'], ctx.elliottAnalysis?.waves?.['15m'], ctx.elliottAnalysis?.waves?.['1h']]
    .filter(w => w && w.label === 'W5' && w.phase === 'IMPULSE').length;
  if (_w5any === 1 && !ctx.elliottAnalysis?.signal) {
    components.structure = (components.structure || 0) + 3;
    confluence.push('W5 detected on 1TF — watch for exhaustion');
  }
...
function buildStructurePlan(ctx, settings, btcRegime) {
  ...
  // fix46c: Elliott Wave analysis across 3 timeframes using closed candles
  // W5 exhaustion = end of move → trade reversal. W3 = middle of move → continuation.
  ...
  ctx.elliottAnalysis = elliottAnalysis; // passed into calibratedScore via ctx
  ...
  if (structure.dominant === 'TRAP' && structure.trap) {
  ...
  } else if (elliottAnalysis.signal?.type === 'W5_EXHAUSTION' && elliottAnalysis.signal.w5Count >= 2) {
    // fix46c: W5 on 2+ timeframes = highest-conviction exhaustion trade
    // W5 top in uptrend → SELL | W5 bottom in downtrend → BUY
    structureOverride = { side: elliottAnalysis.signal.tradeSide, reason: elliottAnalysis.signal.reason };
    ...
    // Squeeze release: trade in direction of last CLOSED candle (k5c already excludes open candle)
  ...
  } else if (elliottAnalysis.signal?.type === 'W3_CONTINUATION' && elliottAnalysis.signal.w3Count >= 2) {
    // W3 on 2+ TFs = strongest wave, trade continuation
    structureOverride = { side: elliottAnalysis.signal.tradeSide, reason: elliottAnalysis.signal.reason };
...
function refreshSignalPlan(existing, candidate, reason = 'REFRESHED_WAITING_SETUP') {
  ...
  Object.assign(existing, candidate, preserve, {
    ...
    updatedAt: Date.now(),
    stateReason: `V4.6.8.47: ${reason}; refreshed waiting setup with newer/better planner data`,
  });
...
function shouldReject(leader, settings, signals = []) {
  ...
  // fix47: RR ceiling removed — floor 2.0 is the only gate.
  // Data showed RR cap at 3.0 killed 64 signals including score 94. Market determines TP.
  // Traders don't cap reward — let price reach the level or get invalidated naturally.
...
function signalFromLeader(leader, settings) {
  ...
  return {
    ...
    stateReason: state === 'PAPER_ACTIVE' ? `V4.6.8.47: ${timing.reason}` : `V4.6.8.47: ${timing.reason}`,
    reasons: leader.reasons,
    risks: leader.risks,
    entryTiming: timing,
...
function getSignals() {
  ...
  const normalized = data.map(s => {
    ...
    if ((s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && now - num(s.createdAt || s.updatedAt, now) > num(s.expireMs, EXPIRY_MS)) {
      changed = true;
      return { ...s, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED', positionStatus: 'NONE', updatedAt: now, closedAt: now, stateReason: 'V4.6.8.47: stale waiting setup expired' };
    }
    return s;
  });
  ...
  for (const s of signals) {
    ...
    if (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') {
      if (age > num(s.expireMs, expiryMsFor(settings))) {
        ...
        s.closedAt = now;
        s.stateReason = 'V4.6.8.47: setup expired before entry confirmation';
      ...
      if (timing.inZone && timing.directional) {
        ...
        s.entryHit = true;
        s.openedAt = now;
        s.stateReason = 'V4.6.8.47: ' + timing.reason;
...
function isDeadHour() { return false; } // kept for compat — always returns false
...
function ingestCandidate(payload = {}) {
  ...
  // fix47: RR ceiling removed from ingestCandidate — floor only (minRRFor handles it above)
  // No _ingRrCap — consistent with shouldReject
  ...
  const sig = {
    ...
    entryHit: false,
    stateReason: 'V4.6.8.47: READY setup captured as WAITING_ENTRY; activation requires tight entry zone + directional reaction',
...
function freshJournal() {
  ...
  store.write('legacy_browser_signals', { at: Date.now(), total: 0, note: 'cleared by v4.6.8.47 freshJournal' });
  snapshot = { ...snapshot, ts: Date.now(), signals: [], visibleSignals: 0, ledger: [], summary: computeLedgerSummary([]) };
  store.write('v4_snapshot', snapshot);
  return { ok: true, cleared: true, source: 'v4.6.8.47 freshJournal' };
}
