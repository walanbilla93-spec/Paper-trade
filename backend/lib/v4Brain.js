...
const structureDetector = require('./structureDetector'); // fix44f: squeeze/trap/BOS/CHoCH/divergence
...
const ENTRY_CONFIRMATION_WINDOW_MS = Math.max(1000, Number(process.env.V4_ENTRY_CONFIRMATION_WINDOW_SECONDS || '180') * 1000); // fix43b: raised 90→180s; 90s = only 4 scan ticks at 20s cycle, not enough to confirm direction
...
const V4_VERSION = '4.6.8.43e'; // fix43e: trendFromCloses reverted 0.995→0.998; BULL_RANGE SELL blocked; RR floor 2.0 restored; dead hours +UTC11,21; sc_entry inverted (zone-touch=12, breakout=5)
...
function currentSettings() {
  ...
  return {
    ...
    v4SymbolLossCooldownMinutes: Math.max(15, Math.min(240, num(s.v4SymbolLossCooldownMinutes, COOLDOWN_MS / 60000))), // fix43d: clamp was 180 but SYMBOL_LOSS_LOCKOUT_MS default is 240 — silently capping at 3h not 4h
...
function minRRFor(side, settings = currentSettings(), score = null) {
  // fix43e: restore RR floor 2.0 — fix43 removed it causing V44 regression
  // V44 data: RR 1.8-2.0 = 21.7% WR, -5.306U across 23 trades. Clear bleed.
  // RR 2.0-2.5 remains the proven sweet spot at 59.5% WR across 131 trades.
...
function calibratedScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  // fix38g: dynamic RR ceiling — CHOP regime only (too risky to change globally)
  // In CHOP, TP targets are rarely reached. Cap at 2.2 ATR to keep setups realistic.
  ...
  // fix44f: structure detection score boost — reward signals that have confirmed market structure
  // These are the highest-conviction setups the system can identify
  if (ctx.structure) {
    const _dom = ctx.structure.dominant || 'NONE';
    if (_dom === 'TRAP') {
      components.structure = (components.structure || 0) + 8;
      confluence.push('trap confirmed — fade signal');
    } else if (_dom === 'CHOCH_CONFIRMED') {
      components.structure = (components.structure || 0) + 10;
      confluence.push('CHoCH confirmed on 5m+15m — structure shifted');
    } else if (_dom === 'CHOCH') {
      components.structure = (components.structure || 0) + 5;
      confluence.push('CHoCH detected — potential reversal');
    } else if (_dom === 'SQUEEZE_RELEASE') {
      components.structure = (components.structure || 0) + 7;
      confluence.push('squeeze release — volatility expanding');
    } else if (_dom === 'DIVERGENCE') {
      components.structure = (components.structure || 0) + 6;
      confluence.push(`${ctx.structure.divergence?.type || ''} divergence detected`);
    } else if (_dom === 'BOS') {
      components.structure = (components.structure || 0) + 4;
      confluence.push('BOS confirmed — structure intact');
    }
  }
  ...
  // fix38g: dynamic RR ceiling — moved here so btcR and localRegime are defined
  if (btcR === 'CHOP' || localRegime === 'LOCAL_CHOP') {
    maxTargetAtr = Math.min(maxTargetAtr, 2.2);
  }
  ...
  // fix44f: merge with structure detection boost (set earlier) — don't overwrite
  components.structure = (components.structure || 0) + (hasStructure ? 10 : 0);
  ...
  // fix43e: inverted entry score values — data: sc_entry=12 had 28.6% WR, sc_entry=5 had 51% WR
  // Root cause: tight entries (≤0.25 ATR) were being rewarded as "precise" but are actually
  // momentum-chasing entries with tight SL (avg 0.62%) that get stopped out on noise.
  // Zone-touch entries (0.5-1.0 ATR) wait for price to pull back — better fills, wider SL, 51% WR.
  // Swapped score VALUES only — thresholds and logic unchanged.
  if (entryDistanceAtr <= 0.25) components.entry = 5;   // was 12 — tight/breakout entry, demoted
  else if (entryDistanceAtr <= 0.50) components.entry = 9;  // unchanged — mid range
  else if (entryDistanceAtr <= 1.00) components.entry = 12; // was 5  — zone touch, promoted
  else if (entryDistanceAtr <= 1.80) components.entry = 2;  // unchanged
  else components.entry = 0;                                 // unchanged
  ...
  // fix39: softened SL% penalty (was -15/-8, causing gate to be unreachable combined with other fixes)
  const slPctCalc = plan.entry ? Math.abs(plan.entry - plan.sl) / Math.abs(plan.entry) * 100 : 0;
  if (slPctCalc > 0 && slPctCalc < 0.35) { penalties += 8; risks.push(`SL too tight (${slPctCalc.toFixed(2)}%) — noise stop`); }
  else if (slPctCalc > 0 && slPctCalc < 0.50) { penalties += 4; risks.push(`SL tight (${slPctCalc.toFixed(2)}%) — stop risk`); }
  ...
  // fix46b: structure signals (TRAP, CHOCH_CONFIRMED, DIVERGENCE) bypass risk cap
  // A trap signal LOOKS risky (price near swing high/low) but IS the trade — cap would suppress it
  const _isStructureSig = ctx.structure && ['TRAP','CHOCH_CONFIRMED','DIVERGENCE'].includes(ctx.structure.dominant);
  if (risks.length && !_isStructureSig) score = Math.min(score, MAX_SCORE_WITH_RISK);
...
function buildStructurePlan(ctx, settings, btcRegime) {
  ...
  // fix44f: run structure analysis on available candle data
  // Detects squeeze, BOS, CHoCH, trap, divergence — same logic as frontend scanner
  // If a dominant structure signal exists, it overrides the trend-based side decision
  const structure = structureDetector.analyzeStructure(k5, Array.isArray(k15) ? k15 : [], trend5);

  // Structure-driven side override — high conviction signals take precedence
  let structureOverride = null;
  if (structure.dominant === 'TRAP' && structure.trap) {
    // Trap confirmed: trade the OPPOSITE direction of the fake breakout
    structureOverride = { side: structure.trap.tradeSide, reason: `${structure.trap.type.toUpperCase()} — fade the trap` };
  } else if (structure.dominant === 'CHOCH_CONFIRMED' && structure.choch) {
    // CHoCH confirmed on both 5m+15m: trade the new direction
    structureOverride = { side: structure.choch.type === 'bull' ? 'BUY' : 'SELL', reason: `CHoCH_CONFIRMED_${structure.choch.type.toUpperCase()} — structure shifted` };
  } else if (structure.dominant === 'SQUEEZE_RELEASE') {
    // Squeeze release: trade in direction of last CLOSED candle (k5c excludes open candle)
    const k5c = k5.length > 1 ? k5.slice(0, -1) : k5;
    const relCandle = k5c[k5c.length - 1];
    const relSide = relCandle && relCandle.close > relCandle.open ? 'BUY' : 'SELL';
    structureOverride = { side: relSide, reason: 'SQUEEZE_RELEASE — volatility expanding' };
  } else if (structure.dominant === 'DIVERGENCE' && structure.divergence) {
    // RSI divergence: trade the divergence direction
    structureOverride = { side: structure.divergence.tradeSide, reason: `${structure.divergence.type.toUpperCase()}_DIVERGENCE strength=${structure.divergence.strength}` };
  }

  // Use structure override if it exists and doesn't conflict with BTC macro regime
  let effectiveSide = decision.side;
  let structureSignal = null;
  if (structureOverride) {
    const _macroBtcR = String(btcRegime?.regime || '').toUpperCase();
    const _macroConflict =
      (structureOverride.side === 'BUY'  && _macroBtcR === 'BEAR_TREND') ||
      (structureOverride.side === 'SELL' && _macroBtcR === 'BULL_TREND');
    if (!_macroConflict) {
      effectiveSide = structureOverride.side;
      structureSignal = structureOverride;
    }
  }

  // fix46b: mark structure-driven side changes as contrarianFlip so shouldReject gates
  // (SELL_SIDE_DISABLED, PLANNER_RR_DEADZONE) and retry matching don't block them
  const _structureFlipped = structureSignal && structureOverride?.side !== decision.side;

  // fix44f: attempt plan build with effectiveSide; fallback to decision.side if structure levels missing
  let side = effectiveSide !== 'NEU' ? effectiveSide : decision.side;
  const _structureSideDiffers = structureSignal && side !== decision.side;
  ...
  if (side === 'BUY') {
    if (!support || !resistance) {
      // fix44f: fallback — if structure override caused this side, try original decision.side
    ...
    if (!support || !resistance) {
      // fix46: mirror BUY fallback — if structure override caused SELL but no resistance, try decision.side
  ...
  // fix44f: attach structure data to ctx so calibratedScore can use it
  ...
  if (_shouldFlip && support > 0 && resistance > 0) {
    ...
    if (_sellRr >= 1.8 && _sellTp < _sellEntry && _sellSl > _sellEntry) {
      leader.side = 'SELL';
      ...
      addLog('CONTRARIAN_FLIP', `${symbol}: BUY→SELL flip (${_flipBtcR}+${_flipLocalR}) entry=${leader.entry} sl=${leader.sl} tp=${leader.tp1} rr=${leader.math.rr.toFixed(2)}`);
    }
...
function shouldReject(leader, settings, signals = []) {
  ...
  // NOTE: SENTINEL_SIDE_BLOCK and sentinel score/RR gates are intentionally disabled.
  // Use warnings above to track conflict data without blocking signals.
  ...
  const userMinRR = minRRFor(leader.side, settings, leader.score);
  ...
  // fix43: RR cap now score-aware. score>=85 = high conviction, allow up to 3.5.
  // Data: 64 kills from RR_GT_3_CAPPED including 94-score signals. Low-score signals still capped at 3.0.
  const _rrCap = leader.score >= 85 ? 3.5 : 3.0;
  if (leader.math.rr > _rrCap) reasons.push(`RR_GT_${_rrCap}_CAPPED`);

  // fix44f: RR 2.50-2.74 bleeds -1.39U on planner signals (44% WR, 3 sessions confirmed)
  // Retry signals exempt — they have fresh data and 59% WR even at higher RR
  // Contrarian flip signals also exempt — structure-driven, different geometry
  // fix46b: also exempt structure override signals (trap/CHoCH have directional confirmation)
  const _isPlanner = String(leader.source || '').includes('planner');
  if (_isPlanner && !leader.contrarianFlip && !leader.structureSignal && leader.math.rr > 2.49 && leader.math.rr < 2.75) {
    reasons.push('PLANNER_RR_DEADZONE_2.5-2.75');
  }
  ...
  // data-driven: SELL in STRONG_BULL with RR>2.5 = losses (bull run resumes, stops out SELL).
  // Wins concentrated at RR 2.0-2.5. Cap SELL RR at 2.5 in bull regimes. Not a hard block — just filters stretched targets.
  ...
  if (_btcRForSellBlock === 'BULL_RANGE' && leader.side === 'SELL') reasons.push('BULL_RANGE_SELL_BLOCKED');

  // fix43b: use leader.btcRegime (regimeBrain, 3-TF) not sentRegime (sentinelBrain, disabled/stale)
  // sentRegime was UNKNOWN when sentinel is disabled → _isBullRegime=false → SELL cap never fired
  ...
  if (_isBullRegime && leader.side === 'SELL' && leader.math.rr > 2.5) reasons.push('SELL_RR_TOO_HIGH_IN_BULL');
...
const HARD_REJECT_PREFIXES = [
  ...
  'DEAD_HOUR_NO_NEW_ENTRIES',   // time-based — entry time has passed
    ...
    try {
      ...
      if (built.ok && built.leader && (
        ...
        payload = {
          ...
          score: Math.max(r.score, built.leader.score || 0), // keep higher score
        };
      } else {
...
// fix43e: added UTC 11 (LK 17:00, 31% WR) and UTC 21 (LK 03:00, 21% WR)
// fix44d: added UTC 23 (LK 05:30, 33% WR), UTC 12 (LK 18:00, 29% WR), UTC 14 (LK 20:00, 29% WR)
// fix44d-patch: removed UTC 01 (LK 06:30 — LK06 is 75% WR, wrong to block)
// fix44d-patch: removed UTC 19 (LK 00:30 — no data evidence)
const BAD_SCAN_HOURS_UTC = new Set([11, 12, 14, 21, 23]);
function isDeadHour() {
  return BAD_SCAN_HOURS_UTC.has(new Date().getUTCHours());
}
...
async function scanOnce() {
  ...
  try {
    ...
    // fix44f: dead hour block removed — structureDetector handles signal quality
    // Log for diagnostics only — scan continues normally
    const _deadHour = isDeadHour();
    if (_deadHour) {
      addLog('STRUCTURE_HOUR', `High-structure hour (${new Date().getUTCHours()}:xx UTC) — scanning for trap/CHoCH/squeeze setups`);
    }
...
function ingestCandidate(payload = {}) {
  ...
  // fix35-obs22: frontend-candidate reload flood — reject same sym+side if captured within 15min
  // Page reload re-submits all frontend candidates; cooldown prevents duplicate capture
  const isFrontendCandidate = String(payload.source || '').includes('frontend-candidate');
  if (isFrontendCandidate) {
    const recentCapture = signals.find(s =>
      (s.sym === symbol || s.symbol === symbol) &&
      s.side === side &&
      num(s.createdAt, 0) > 0 &&
      (Date.now() - num(s.createdAt, 0)) < 15 * 60 * 1000
    );
    if (recentCapture) {
      return { ok: false, rejected: true, reasons: ['FRONTEND_CANDIDATE_RELOAD_COOLDOWN_15MIN'], warnings: [], note: 'Same frontend candidate submitted within 15min — reload suppressed' };
    }
  }
  ...
  // fix-rr-cap: apply same RR>3.0 cap to frontend candidates
  // fix43b: mirror shouldReject() score-aware RR cap — was hardcoded 3.0 with no score awareness
  const _ingRrCap = score >= 85 ? 3.5 : 3.0;
  if (rr > _ingRrCap) reasons.push(`RR_GT_${_ingRrCap}_CAPPED`);
  ...
  const sig = {
    ...
    source: payload.source || `frontend-candidate-${V4_VERSION}`, // fix42: use caller's source tag — retry path was stamping backend signals as frontend
    plannerVersion: V4_VERSION,
    sym: symbol,
    symbol,
    side,
...
function start() {
  ...
  try {
    ...
    const cleaned = raw.map(s => {
      if (!s || !ACTIVE_STATES.has(s.paperState)) return s;
      const createdAt = num(s.createdAt, 0);
      const updatedAt = num(s.updatedAt, 0);
      const tsRaw = createdAt || updatedAt; // 0 if both missing
