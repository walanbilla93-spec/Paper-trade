...
function normalizeLedgerTrade(s) {
  ...
  return ledgerObservability.clone({
    ...
    key,
    planKey,
    patchVersion:'PATCH-2.4.4',scoreAdmission:s.scoreAdmission||null,marketPermission:s.marketPermission||null,
...
module.exports = {
  ...
  getConfigFlags: () => ({ FIXCONFIRM_ENTRY_MARKET, patchVersion:'PATCH-2.4.4', sourceHash:patch24Identity.sourceHash, sourceHashes:patch24Identity.sourceHashes, marketPolicy:marketPolicy.VERSION, scorePolicy:{minInclusive:40,maxExclusive:80}, timingBlockThreshold:Number(process.env.V4_TIMING_BLOCK_THRESHOLD||44), timingUniversalZoneBypass:true, maxPriceAgeMs:paperOrders.MAX_PRICE_AGE_MS, fixRetestEnabled: true, restingWatchMs: RESTING_WATCH_MS, ledgerObservabilitySchema: LEDGER_OBSERVABILITY_SCHEMA, rejectedObservationSchema:observations.SCHEMA }), // fixVERIFY (07/26): lets /health report the live value of entry-placement flags directly from the running process, so a deploy can be confirmed correct with zero live trades — needed because these hooks only fire when tradingEnabled=true, so logs alone can't verify a dry/warmup run.
