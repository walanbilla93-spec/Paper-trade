...
const V4_VERSION = '4.6.8.48l'; // fix48l: ALL THREE waiting-expiry paths (2618/2644/3612) now keyed to immutable signalBirthMs (48k only fixed one; CRV zombie slipped through the other two). Backend-persisted alerts added. No brain/score/RR/sentinel/direction logic changes.
...
function recordBackendAlert(type, s, extra = {}) {
  try {
    if (!BACKEND_ALERT_TYPES.has(type)) return;
    const alert = {
      at: Date.now(),
      ...
      score: s?.score,
      rr: s?.rr || s?.rr1,
      state: s?.paperState || s?.status || '',
      reason: s?.stateReason || extra?.reason || '',
      pnlUSDT: s?.paperState === 'TP_HIT' ? num(s?.netTpUSDT) : s?.paperState === 'SL_HIT' ? num(s?.netSlUSDT) : null
    };
    let arr = [];
    try { arr = store.read('v4_alerts', []); } catch (_e) { arr = []; }
    if (!Array.isArray(arr)) arr = [];
    arr.unshift(alert);
    store.write('v4_alerts', arr.slice(0, MAX_BACKEND_ALERTS));
  } catch (_e) {}
}

function appendSignalDiagnostic(type, s, extra = {}, settings = currentSettings()) {
  recordBackendAlert(type, s, extra); // fix48l: persist alert-worthy transitions to backend store
  try {
...
module.exports = {
  ...
  getSnapshot,
  getSignals,
  getBackendAlerts: (limit = 100) => { // fix48l: persisted alerts for polling
    try { const a = store.read('v4_alerts', []); return Array.isArray(a) ? a.slice(0, limit) : []; }
    catch (_e) { return []; }
  },
  getLedger,
