...
const V4_VERSION = '4.6.8.48p'; // fix48p: regimeBrain mislabel fixes. (1) RSI-extreme (<22/>78) no longer blindly→CHOP: when structure/trend confirm, →BEAR_TREND/BULL_TREND (RSI 9 crash was labeled CHOP→BTC_CHOP_BLOCKS_ALL→100min NO_DIRECTION silence). (2) EMA spread guarded vs insufficient-candle blowup (ema50 SMA-fallback gave 80-106% spreads); untrusted spread→emasTight=false. (3) logs BTC_REGIME_FIX on correction. Carries 48o funding + 48n2 conflict guard.
...
function minScoreForSide(side, settings = currentSettings()) {
  ...
  const NEW_SCALE_THRESHOLD_CEIL = Math.max(50, Number(process.env.V4_SCORE_THRESHOLD_CEIL || '68'));
...
function convictionScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  const qualityMult = familyMult * entryMult * regimeMult * rrMult * rsiMult * volMult;
  ...
  components._mult = { familyMult: +familyMult.toFixed(3), entryMult: +entryMult.toFixed(3), regimeMult: +regimeMult.toFixed(3), rrMult: +rrMult.toFixed(3), rsiMult: +rsiMult.toFixed(3), volMult: +volMult.toFixed(3), regimeStrength: +regimeStrength.toFixed(3), qualityMult: +qualityMult.toFixed(3) };
...
function initActiveDiagnostics(s, entryPrice, atr, now, settings) {
  ...
  // fix48m-instr: rsiAtEntry was null ~70% of the time because s.rsi isn't always set at activation.
  // Fall back to the scoreInfo rsi captured at signal birth.
  s.diag.rsiAtEntry = num(s.rsi, num(s.scoreInfo?.rsi, null));
  ...
  // fix48m-instr: CAPTURE MARKET-SENTINEL CONTEXT AT ENTRY.
  // breadth (vol_bull_pct) was the hidden directional edge we only found by reconstructing it
  // from the sentinel timeline by timestamp. Record it on the trade row directly so every future
  // edge-hunt has clean, per-trade context — no archaeology. Recording only; no gating here.
  try {
    const _sent = (typeof sentinel?.getSentinel === 'function') ? (sentinel.getSentinel() || {}) : {};
    s.diag.breadthBullPctAtEntry = num(_sent?.components?.vol?.bull_pct,
                                       num(_sent?.vol?.bull_pct, null));
    s.diag.breadthLabelAtEntry   = _sent?.components?.vol?.label || _sent?.vol?.label || null;
    s.diag.fundingRateAtEntry    = num(_sent?.components?.fund?.avg_rate,
                                       num(_sent?.fund?.avg_rate, null));
    s.diag.fundingDangerAtEntry  = _sent?.components?.fund?.danger || _sent?.fund?.danger || null;
    s.diag.sentinelRegimeAtEntry = _sent?.market_regime || null;
    s.diag.sentinelCompositeAtEntry = num(_sent?.composite_score, null);
    s.diag.sentinelAllowedAtEntry = Array.isArray(_sent?.allowed_sides) ? _sent.allowed_sides.join('|') : null;
  } catch (_e) {
