...
let snapshot = {
  ok: true,
  version: '4.0.0-backend-paper-engine',
  ts: 0,
  entries: [],
  signals: [],
  summary: {},
...
function buildStructurePlan(ctx, settings, btcRegime) {
  ...
  let score = 45 + decision.edge;
  const confluence = [...decision.reasons];
  const risks = [];
  if (trend5 === side.toLowerCase() || trend15 === side.toLowerCase()) score += 0; // no-op for readability
  if (side === 'BUY' && trend5 === 'bull' && trend15 !== 'bear') score += 8;
  if (side === 'SELL' && trend5 === 'bear' && trend15 !== 'bull') score += 8;
  if (trend1h === (side === 'BUY' ? 'bull' : 'bear')) score += 5;
  if (volRatio > 1.3) score += 4;
  if (math.rr >= 2) score += 6;
  if (Math.abs(price - entry) <= atr * ENTRY_TOLERANCE_ATR) score += 4;
  if (ticker.turnover24h > 50000000) score += 3;

  if (btcRegime.regime === 'unknown') risks.push('BTC regime unknown');
  if (side === 'BUY' && btcRegime.regime === 'bear') risks.push('BTC bearish against long');
  if (side === 'SELL' && btcRegime.regime === 'bull') risks.push('BTC bullish against short');
  if (side === 'BUY' && rsi > 78) risks.push('RSI overbought on long');
  if (side === 'SELL' && rsi < 22) risks.push('RSI oversold on short');
  if (math.rr < MIN_RR) risks.push(`RR ${math.rr} below ${MIN_RR}`);
  if (math.netTpUSDT < MIN_NET_TP) risks.push(`net TP ${math.netTpUSDT} below ${MIN_NET_TP}`);
  ...
  const leader = {
    ...
    side,
    score,
    tier: score >= 85 ? 'A+' : score >= 75 ? 'A' : score >= 65 ? 'B' : '-',
...
function signalFromLeader(leader, settings) {
  ...
  return {
    id,
    source: 'planner-v4',
    plannerVersion: 'v4.0-backend-paper-engine',
    sym: leader.symbol,
...
function getSnapshot() {
  ...
  return { ...snapshot, signals: getSignals(), summary: computeSummary(getSignals()), lastError };
}
...
function clearSignals({ archive = true } = {}) {
  const signals = getSignals();
  if (archive && signals.length) {
    const sessions = getSessions();
    sessions.unshift({ at: Date.now(), summary: computeSummary(signals), signals });
    saveSessions(sessions);
  }
  saveSignals([]);
  snapshot = { ...snapshot, signals: [], summary: computeSummary([]), ts: Date.now() };
  store.write('v4_snapshot', snapshot);
  return { ok: true, archived: archive && signals.length ? 1 : 0, total: 0 };
}
