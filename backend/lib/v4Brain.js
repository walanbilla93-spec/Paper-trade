...
const V4_VERSION = '4.6.8.38'; // fix38: sentinel/BTCmtf conflict gate, SL penalty, symbol decay, dynamic RR CHOP cap, sweepEngine
...
function calibratedScore(ctx, decision, plan, math, btcRegime, levels) {
  ...
  // fix38e: SL% penalty — tight SL = noise stop = low WR. Data: SL<0.5% had 30% WR.
  const slPctCalc = plan.entry ? Math.abs(plan.entry - plan.sl) / Math.abs(plan.entry) * 100 : 0;
  if (slPctCalc > 0 && slPctCalc < 0.40) { penalties += 15; risks.push(`SL too tight (${slPctCalc.toFixed(2)}%) — noise stop`); }
  else if (slPctCalc > 0 && slPctCalc < 0.60) { penalties += 8; risks.push(`SL tight (${slPctCalc.toFixed(2)}%) — stop risk`); }
...
function shouldReject(leader, settings, signals = []) {
  ...
  // fix38d: stale sentinel guard — if sentinel hasn't updated in >3min, treat as UNKNOWN
  const sentinelAgeMs = sent && sent.updatedAt ? Date.now() - sent.updatedAt : Infinity;
  const sentinelStale = sentinelAgeMs > 3 * 60 * 1000;
  ...
  const sentMinScore = sentimentReady && !sentinelStale ? (isBuy ? sent.buy_score_gate : sent.sell_score_gate) : userMinScore;
  // fix38b: when sentinel and BTC MTF conflict, raise gate +15 (market direction unclear)
  // fix38d: when sentinel is stale, raise gate +10 (can't trust regime)
  const conflictPenalty = _sentinelBtcConflict ? 15 : (sentinelStale ? 10 : 0);
  const minScore = Math.max(userMinScore, sentMinScore) + conflictPenalty;
  ...
  if (_sameRecent.length >= 1) {
    ...
    if (ageMin < 15) {
      warnings.push(`SYMBOL_DECAY_FAST:${Math.round(ageMin)}min`);
      // Score effectively -20 via raising minScore (avoids mutating leader.score)
      // We can't mutate leader here, so we push a suppressible reason instead
      reasons.push(`SYMBOL_REENTRY_TOO_FAST_${Math.round(ageMin)}MIN`);
    } else {
      warnings.push(`SYMBOL_DECAY:${Math.round(ageMin)}min`);
    }
  }
