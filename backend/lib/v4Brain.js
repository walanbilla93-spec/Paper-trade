...
const ACTIVE_STATES = new Set(['WAITING_ENTRY', 'WAITING_REACTION', 'PAPER_ACTIVE', 'LIVE_RESTING', 'LIVE_OPEN', 'LIVE_PENDING']);
...
const FINAL_STATES = new Set(['TP_HIT', 'SL_HIT', 'INVALIDATED', 'EXPIRED', 'REJECTED', 'STALE_CANCELLED', 'REGIME_FLIP_EXIT', 'CONTRARIAN_FLIP', 'ACTIVE_FLIP_EXIT', 'CLOSED_WIN', 'CLOSED_LOSS', 'PAPER_EXPIRED', 'PAPER_INVALID', 'CANCELLED']);
...
function terminalStateOf(s) {
  ...
  if (st === 'INVALIDATED' || st === 'INVALID' || st === 'PAPER_INVALID') return 'INVALIDATED';
...
function getSignals() {
  ...
  const normalized = data.map(s => {
    ...
    if (!isOrderBearing(s) && (s.paperState === 'WAITING_ENTRY' || s.paperState === 'WAITING_REACTION') && _waitingAge > Math.min(num(s.expireMs, EXPIRY_MS), WAITING_HARD_CAP_MS)) {
      changed = true;
...
function normalizeLedgerTrade(s) {
  ...
  const result =
    ...
    s.paperState === 'INVALIDATED' || s.paperState === 'PAPER_INVALID' ? 'INVALIDATED' :
    ...
    s.paperState === 'PAPER_ACTIVE' || s.paperState === 'LIVE_OPEN' || s.paperState === 'LIVE_RESTING' ? 'ACTIVE' :
