...
function btcRegimeDecision(leader, settings, btcUnknownException = false) {
  ...
  if (regime === opposite) reasons.push(side === 'BUY' ? 'BTC_BEAR_AGAINST_LONG' : 'BTC_BULL_AGAINST_SHORT');
...
function globalRiskBlockReason(signals = [], settings = currentSettings()) {
  ...
  for (const r of rows) {
    if (r.result === 'LOSS') consecutiveLosses += 1;
    else break;
  }
  const maxConsecutive = Math.max(1, num(settings.v4MaxConsecutiveLosses, MAX_CONSECUTIVE_LOSSES));
  if (consecutiveLosses >= maxConsecutive) return `RISK_BLOCK_${consecutiveLosses}_CONSECUTIVE_LOSSES`;
