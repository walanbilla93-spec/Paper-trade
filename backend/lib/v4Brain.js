...
function ingestCandidate(payload = {}) {
  ...
  // fix33-obs12: reject if an existing WAITING signal for same sym+side has entry within 0.5%
...
function start() {
  ...
  // fix33-new: on startup, force-expire any WAITING signals older than session window
  // getSignals() already handles expiry by time, but we call it once here to flush them
  // before the first scan runs, so stale signals don't occupy slots during first scan.
  try {
    const now = Date.now();
    ...
    const cleaned = raw.map(s => {
      ...
      const age = now - num(s.createdAt || s.updatedAt, now);
      ...
      if (age > expMs) {
        ...
        return { ...s, paperState: 'EXPIRED', displayState: 'EXPIRED', status: 'EXPIRED',
                 ...
                 stateReason: 'V4: expired on restart — past signal window' };
      }
      return s;
    });
    if (purged > 0) {
      store.write('v4_signals', dedupeLiveSignals(cleaned).slice(0, MAX_SIGNALS));
      console.log(`[Orayan v4] Startup: expired ${purged} stale WAITING signals`);
    }
