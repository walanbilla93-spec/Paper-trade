...
function computeAllowedSides(market_regime, wrData, fundDanger) {
  ...
  if (bullRegimes.includes(market_regime)) {
    ...
    // data-driven: BEAR BUY score85+ = 100% WR, score<85 = 0% WR.
    // Allow BUY — score gate (raised to 85 in bear) handles low scorers.
    // SELL in BEAR = 29% WR — net negative. Keep blocked.
    sides.push('BUY');
  } else {
