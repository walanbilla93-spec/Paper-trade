...
function computeAllowedSides(market_regime, wrData, fundDanger) {
  ...
  if (fundDanger === 'SELL' && sides.includes('SELL')) {
    const _sellWithTrend = _confBear;            // SELL aligns only with a bear trend
    if (!_confTrend || !_sellWithTrend) {        // CHOP, or SELL against a bull trend
      sides.splice(sides.indexOf('SELL'), 1);
    } // else: SELL is with the bear trend — keep it, gate handles the crowding
  }
...
async function runSentinel() {
  ...
  try {
    ...
    const allowed_sides = computeAllowedSides(confirmedRegime, wrData, fund.danger);
