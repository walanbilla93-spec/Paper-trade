...
function computeGates(market_regime, wrData) {
  ...
  const buyBase  = Math.max(bearBuyGate, BASE_SCORE_GATE + (isChop ? 4 : 0) + (wrData.BUY?.score_adj || 0));
  const sellBase = BASE_SCORE_GATE + (sellAgainstTrend ? 5 : 0) + (isChop ? 4 : 0) + (wrData.SELL?.score_adj || 0);
...
async function runSentinel() {
  ...
  try {
    ...
    const gates = computeGates(confirmedRegime, wrData);
