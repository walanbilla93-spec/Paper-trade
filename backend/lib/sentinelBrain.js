...
async function runSentinel() {
  ...
  try {
    ...
    // fix33-regimeflip: on confirmed regime flip, cancel WAITING signals for newly-blocked sides
    // Prevents stale BUY signals from previous BULL session surviving into STRONG_BEAR
      ...
      try {
        ...
        if (blockedSides.length > 0) {
          ...
          const updated = signals.map(s => {
            if (!s || !ACTIVE_WAITING.has(s.paperState)) return s;
          ...
          if (cancelled > 0) {
            ...
            console.log(`[Sentinel] Regime flip ${prevRegime}->${confirmedRegime}: cancelled ${cancelled} WAITING ${blockedSides.join('/')} signal(s)`);
          }
        }
      } catch (e) {
