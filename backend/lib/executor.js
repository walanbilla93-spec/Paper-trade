...
async function getAccountType() {
  ...
  try {
    ...
    // fixUNIMARGIN (hotfix 17): Bybit's v5 unifiedMarginStatus enum (confirmed against Bybit's own
    // API docs) is 1=classic (deprecated, unavailable on accounts opened 2025+), 2=deprecated
    // unified-margin, 3=UTA1.0, 4=UTA1.0 Pro, 5=UTA2.0, 6=UTA2.0 Pro. The old check matched ONLY
    // value 1 — which is never a unified account — so every real UTA account (any value 2-6, which
    // is effectively every current account) fell through to the marginMode/accountType fallback
    // instead of 'UNIFIED'. That made the code below wrongly call setIsolatedMarginSafe on an
    // account where isolated margin mode doesn't exist, producing the "Isolated margin set warning
    // ... unified account is..." log line on every live order. Any status >=2 is some flavor of
    // unified account.
    const _ums = Number(data?.result?.unifiedMarginStatus);
    _cachedAccountType = _ums >= 2 ? 'UNIFIED' :
...
// fixAGREEBLOCK (hotfix 17): some Bybit products (gold/silver perpetuals confirmed; likely also
// newer stock-linked perpetuals like SKHYNIXUSDT) require the account holder to sign a
// product-specific risk agreement on Bybit's own site/app before the API will accept orders on
// them — no API call can satisfy this, it is not an account-config bug. Every retry fails
// identically. Detect the message once, persist a block, and stop wasting live attempts (and,
// pre-fixREJSTATE, stop manufacturing fake paper wins/losses) on a symbol that cannot fill until
// the user acts on Bybit's side. Cleared automatically if Bybit ever accepts an order for the
// symbol again (belt-and-suspenders — normally cleared via clearAgreementBlock below).
const AGREEMENT_BLOCK_STORE_KEY = 'agreementBlockedSymbols';
const AGREEMENT_MSG_RE = /sign\s+(the\s+)?(required|related)?\s*agreement/i;
const AGREEMENT_BLOCK_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000; // safety-net auto-expiry; normal path is the manual /agreement-blocks/clear route once signed
function isAgreementBlocked(symbol) {
  const m = store.read(AGREEMENT_BLOCK_STORE_KEY, {});
  const entry = m && m[symbol];
  if (!entry) return false;
  if (Date.now() - (entry.at || 0) > AGREEMENT_BLOCK_MAX_AGE_MS) { delete m[symbol]; store.write(AGREEMENT_BLOCK_STORE_KEY, m); return false; }
  return true;
}
function getAgreementBlocks() {
  return store.read(AGREEMENT_BLOCK_STORE_KEY, {});
}
function blockSymbolForAgreement(symbol, retMsg, retCode) {
  const m = store.read(AGREEMENT_BLOCK_STORE_KEY, {});
  if (!m[symbol]) {
    m[symbol] = { at: Date.now(), retMsg, retCode };
    store.write(AGREEMENT_BLOCK_STORE_KEY, m);
  }
  addLog('SYMBOL_BLOCKED', `${symbol}: Bybit requires a signed product agreement before this symbol can trade — auto-trading paused for ${symbol} until you open Bybit (web or app) and accept the pending agreement prompt for it`, { retMsg, retCode });
}
function clearAgreementBlock(symbol) {
  const m = store.read(AGREEMENT_BLOCK_STORE_KEY, {});
  if (m[symbol]) { delete m[symbol]; store.write(AGREEMENT_BLOCK_STORE_KEY, m); }
}
...
// ── fixANCHOR (hotfix 22, 2026-07-24): re-anchor TP to the actual market fill ────────────────────
//   market-on-confirmation (fixCONFIRM 21b) fills LATE — price has already moved toward TP — but TP
//   is a fixed level anchored to the signal's ZONE entry, so the late fill leaves almost no room and
//   winners close for scratch (live REUSDT 07/24: short before a 7% drop, banked +0.04). Fix shifts
//   TP to preserve the intended reward DISTANCE from the actual fill; structural SL untouched.
const ANCHOR_TP_TO_FILL = String(process.env.V4_ANCHOR_TP_TO_FILL || 'true').toLowerCase() === 'true';
const ANCHOR_MIN_RR = Math.max(0, Number(process.env.V4_ANCHOR_MIN_RR || '0.6')); // skip a fill so late RR falls below this
...
async function buildExecutionPlan(signalInput, opts = {}) {
  ...
  if (hasSignalLevels) {
    rawSl = signal.sl;
    rawTp = signal.tp1;
    // fixANCHOR (hotfix 22): on a MARKET fill, the entry landed late (price already moved toward TP),
    // but signal.tp1 is anchored to the ZONE entry — leaving a gutted reward. Shift TP to preserve the
    // signal's intended reward DISTANCE from where we ACTUALLY filled; keep the structural SL (it's a
    // real invalidation price, and tightening it = more stop-outs). Limit fills keep absolute levels.
    if (useMarket && signal.entry > 0 && ANCHOR_TP_TO_FILL) {
      const rewardDist = Math.abs(signal.entry - signal.tp1);
      const reTp = side === 'BUY' ? rawEntry + rewardDist : rawEntry - rewardDist;
      const goodSide = (side === 'BUY' && reTp > rawEntry) || (side === 'SELL' && reTp < rawEntry);
      if (goodSide && rewardDist > 0) {
        const riskFromFill = Math.abs(rawEntry - rawSl);
        const rrFromFill = riskFromFill > 0 ? rewardDist / riskFromFill : 0;
        if (rrFromFill < ANCHOR_MIN_RR) {
          addLog('ANCHOR_SKIP', `${signal.sym} ${side}: fill ${rawEntry} too late — RR ${rrFromFill.toFixed(2)} < ${ANCHOR_MIN_RR} vs structural SL ${rawSl}, skipping (structurally dead)`);
          throw new Error(`ANCHOR_RR: ${signal.sym} late fill RR ${rrFromFill.toFixed(2)} below floor — skipping`);
        }
        addLog('ANCHOR_TP', `${signal.sym} ${side}: TP ${signal.tp1} → ${reTp.toFixed(6)} (reward dist ${rewardDist.toFixed(6)} from actual fill ${rawEntry}, SL kept ${rawSl}, RR ${rrFromFill.toFixed(2)})`);
        rawTp = reTp;
      }
    }
    addLog('LEVELS_FROM_SIGNAL', `${signal.sym} ${side}: using SL=${rawSl} TP=${rawTp} (entry ${rawEntry})`);
  } else {
  ...
  // ── fixRISKCAP (07/21 live forensics): SIZE ON THE WORST REALISTIC FILL, NOT THE INTENDED ONE ──
  // Observed across 20 real losses: gross loss landed on the 0.25 target for 18 of them (median
  // ratio 1.01 — the risk model itself is sound), but two blew through it:
  //   ONDO BUY  planned entry 0.402820, FILLED 0.404200 (+0.343%). SL is an ABSOLUTE structural
  //             price, so the real stop distance became 1.54x the one used for sizing -> -0.517U
  //             against a 0.319U target (1.62x).
  //   LIT SELL  sized against a tighter stop than the one in force at exit -> -0.497U vs 0.250U.
  // Root cause is shared: qty is solved from the INTENDED entry while the SL never moves, so any
  // adverse fill drift silently multiplies risk. The old 0.5%-of-price drift guard cannot catch
  // this — 0.343% of price was 54% of ONDO's stop distance.
  // FIX: size against the worst entry still permitted by the drift guard (and, for market orders,
  // the live price too). Fill anywhere inside tolerance now lands at or under the target.
  // Applied ONLY where the fill price can actually run past the order price — i.e. MARKET orders.
  // A resting LIMIT can never fill worse than its own price, so limit orders keep planned sizing
  // and are unaffected (sizing every trade for worst case cut normal positions ~30% — too blunt).
  const stopDistancePlanned = Math.abs(entry - sl);
  let stopDistance = stopDistancePlanned;
  if (useMarket) {
    // Hardcoded: assume a market order can fill up to this % adverse to its trigger price.
    const MARKET_SLIP_TOLERANCE_PCT = 0.35;
    const _slipTol = Math.max(0, MARKET_SLIP_TOLERANCE_PCT) / 100;
    const _adverse = side === 'BUY' ? entry * (1 + _slipTol) : entry * (1 - _slipTol);
    const _cands = [entry, _adverse];
    if (Number.isFinite(toNum(bybitPrice, NaN))) _cands.push(toNum(bybitPrice, entry));
    const _worst = side === 'BUY' ? Math.max(..._cands) : Math.min(..._cands);
    stopDistance = Math.max(stopDistancePlanned, Math.abs(_worst - sl));
  }
  // fixEVID: evidence-brain size multiplier (<=1 always, floor 0.25). Scales BOTH the risk target
  // and the margin cap so the shrink applies in either sizing mode. Fail-open to 1.
  const evMult = Math.min(1, Math.max(0.05, toNum(signal && signal.evidence && signal.evidence.mult, 1)));
  // fixRISKCAP cost buffer — DELIBERATELY 0 (user decision 07/21). Measured on 20 real losses:
  // gross loss sat at 1.01x the target but NET landed ~10% worse (fees ~2% of budget, the rest
  // unmodelled funding). Reserving that 10% would make "0.25 risk" literally true, but at 2:1 RR
  // it scales wins and losses equally — pure labelling precision, zero edge change — so position
  // sizes are left exactly as they are. Real risk therefore remains ~0.28U per trade, by choice.
  // Raise RISK_COST_BUFFER_PCT (0-50) below only if losses landing above the nominal target
  // ever start to matter; the post-fill audit in reconciler.js reports the real figure either way.
  const RISK_COST_BUFFER_PCT = 0;
  const _costBufPct = Math.max(0, Math.min(50, RISK_COST_BUFFER_PCT)) / 100;
  const riskPerTrade = Math.max(0, toNum(settings.v4RiskPerTradeUsdt, 0)) * evMult * (1 - _costBufPct);
  const marginCapQty = (positionUsdt * evMult) / entry;
  let qty;
  ...
  return {
    ...
    riskTargetUsdt: riskPerTrade,       // fixRISKCAP: post-fill audit compares real risk to this
    slPriceAtSizing: sl,                // fixRISKCAP: the SL the size was solved against
...
function canTradeSignal(signal, settings, trades) {
  ...
  if (isAgreementBlocked(signal.sym)) {
    return { ok: false, reason: `${signal.sym}: blocked pending Bybit product agreement — sign it on Bybit web/app`, permanentBlock: true };
  }
...
// ============================ fixPARTIALCLOSE (hotfix 42) ======================================
// User's direct pushback on v1 (breakeven-only): "sometimes it reaches breakeven & then moves to
// TP. If we do not book any profit, that has no point." Correct — breakeven alone only stops the
// bleed, it never banks a gain. This is the piece deliberately deferred in hotfix41, now built.
// NOT novel API surface: this reuses the EXACT order-placement pattern already firing real orders
// tonight in executeSignal() above — same bybitPost('/v5/order/create'), same category/symbol/qty
// shape, same instrument-info qty rounding already used for entries (getInstrumentInfo + floorToStep,
// both already imported at the top of this file). The only differences from a normal entry order:
// side is INVERTED (closing a SELL position means placing a BUY, and vice versa — reduceOnly makes
// Bybit refuse to let this accidentally open a new position in the wrong direction if the qty or
// side were ever wrong, which is exactly why reduceOnly:true is used rather than a plain order),
// qty is a fraction of the original position, and Market/IOC for immediate execution at the trigger
// moment rather than waiting on a resting limit.
async function placeReduceOnlyClose(plan) {
  // plan: { symbol, side (the ORIGINAL position's side, BUY or SELL), qty (full position qty —
  // this function computes the partial fraction), fraction (0-1, default 0.5), signalId }
  const fraction = Math.min(1, Math.max(0, Number(plan.fraction ?? 0.5)));
  if (!plan.qty || plan.qty <= 0 || fraction <= 0) {
    addLog('WARN', `placeReduceOnlyClose skipped for ${plan.symbol}: qty=${plan.qty} fraction=${fraction}`);
    return { ok: false, error: 'ZERO_OR_MISSING_QTY', skipped: true };
  }
  let qty = plan.qty * fraction;
  try {
    const info = await getInstrumentInfo(plan.symbol);
    qty = floorToStep(qty, info.qtyStep);
    if (info.minOrderQty && qty < info.minOrderQty) {
      // A partial close smaller than the exchange's own minimum order size is not executable —
      // fail closed rather than send a qty Bybit would reject anyway. The caller (v4Brain) treats
      // this exactly like any other failed partial-close: log it, leave the FULL position running
      // toward its original TP/SL — never worse off than not having the feature.
      addLog('WARN', `placeReduceOnlyClose skipped for ${plan.symbol}: rounded qty ${qty} below exchange minOrderQty ${info.minOrderQty}`);
      return { ok: false, error: 'QTY_BELOW_MIN', skipped: true };
    }
  } catch (e) {
    // Fail-open on the rounding lookup only — an unrounded qty is still far better than skipping
    // the close outright; Bybit will reject it if it's genuinely invalid and this function reports
    // that rejection normally below.
    addLog('WARN', `placeReduceOnlyClose: instrument lookup failed for ${plan.symbol}, using unrounded qty: ${e.message}`);
  }
  // Closing a SELL position means BUYING it back, and vice versa — this is the one place in the
  // whole file where the order side is deliberately the OPPOSITE of the signal's own side.
  const closeSide = String(plan.side).toUpperCase() === 'BUY' ? 'Sell' : 'Buy';
  const orderLinkId = buildOrderLinkId(`${plan.signalId || plan.symbol}_pl${Date.now()}`);
  const body = {
    category: 'linear',
    symbol: plan.symbol,
    side: closeSide,
    ...
    timeInForce: 'IOC',
    orderLinkId,
    reduceOnly: true,   // Bybit itself refuses to let this open a new/wrong-direction position
    closeOnTrigger: false,
  };
  addLog('PARTIAL_CLOSE_ATTEMPT', `Reduce-only ${closeSide} ${plan.symbol} qty ${qty} (${(fraction*100).toFixed(0)}% partial lock)`, { plan, body });
  try {
    const data = await bybitPost('/v5/order/create', body);
    if (isDuplicateOrderLinkId(data)) {
      addLog('SKIP_DUPLICATE', `${plan.symbol}: partial-close orderLinkId already exists — idempotent skip`, { orderLinkId, retMsg: data.retMsg });
      return { ok: false, skipped: true, duplicate: true, error: 'DUPLICATE_ORDERLINKID', raw: data };
    }
    if (data.retCode !== 0) {
      addLog('PARTIAL_CLOSE_REJECTED', `Bybit rejected partial close for ${plan.symbol}: ${data.retMsg}`, { retCode: data.retCode, body, response: data });
      return { ok: false, error: data.retMsg, retCode: data.retCode, raw: data };
    }
    return { ok: true, orderId: data.result?.orderId || '', qty, fraction, raw: data.result };
  } catch (e) {
    addLog('ERROR', `placeReduceOnlyClose threw for ${plan.symbol}: ${e.message}`, { plan });
...
async function executeSignal(signalInput, opts = {}) {
  ...
  if (data.retCode !== 0) {
    ...
    const _permBlock = AGREEMENT_MSG_RE.test(String(data.retMsg || ''));
    if (_permBlock) blockSymbolForAgreement(plan.symbol, data.retMsg, data.retCode);
    return { ok: false, error: data.retMsg, retCode: data.retCode, plan, raw: data, permanentBlock: _permBlock };
  }
...
//   isWaitingLimit=true    → extended stale-timeout (WAITING_LIMIT_TIMEOUT_MINUTES, default 30 — fixWLT30)
//
...
async function placeLimitAtEntry(signalInput, opts = {}) {
  ...
  // fixAGREEBLOCK (hotfix 17): this path (GTC limit at WAITING creation) never went through
  // canTradeSignal, so an agreement-blocked symbol kept placing doomed limit orders every cycle.
  if (isAgreementBlocked(signal.sym)) return { ok: false, skipped: true, error: `${signal.sym}: blocked pending Bybit product agreement`, permanentBlock: true };
  ...
  try {
    ...
    if (data.retCode !== 0) {
      ...
      const _permBlock = AGREEMENT_MSG_RE.test(String(data.retMsg || ''));
      if (_permBlock) blockSymbolForAgreement(signal.sym, data.retMsg, data.retCode);
      return { ok: false, error: data.retMsg, retCode: data.retCode, permanentBlock: _permBlock };
    }
    ...
    const trade = {
      ...
      orderType:      'Limit',
      isWaitingLimit: true,       // tells reconciler to use WAITING_LIMIT_TIMEOUT_MINUTES (default 30) not pendingTimeoutMinutes
      entry,
      signalEntry:    signal.entry || null,
      sl,
      tp1,
