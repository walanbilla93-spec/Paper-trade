...
async function buildExecutionPlan(signalInput, opts = {}) {
  ...
  // fix44f: Smart limit placement — industry standard post-touch limit order
  // Problem: signal activates when price is ALREADY past entry (directional confirmation = price moved through zone)
  //          Placing limit at stale signalEntry means order sits below market → never fills
  // Solution: place limit at bybitPrice - 0.15 ATR (BUY) or bybitPrice + 0.15 ATR (SELL)
  //           This catches the micro-pullback that almost always follows a breakout candle
  //           Still a limit order (maker fee, no slippage) but at a realistic fill level
  //           If price already ran >0.5% past entry → place at bybitPrice directly (chase mode)
  let rawEntry;
  if (useMarket) {
    rawEntry = bybitPrice;
    ...
    if (hardDrift) {
      // Price ran hard — place at bybitPrice (will fill immediately as taker-like limit)
      rawEntry = bybitPrice;
      addLog('ENTRY_DRIFT_HARD', `${signal.sym} ${side}: ${(drift*100).toFixed(2)}% past entry → placing at bybitPrice ${bybitPrice}`);
    } else if (atr > 0) {
      // Standard case: price in zone or just past — place 0.15 ATR behind current price
      // Catches micro-pullback, stays competitive, remains a limit order
      const pullbackOffset = atr * 0.15;
      rawEntry = side === 'BUY'
        ? Math.max(signal.entry, bybitPrice - pullbackOffset) // below market but >= signal entry
        : Math.min(signal.entry, bybitPrice + pullbackOffset); // above market but <= signal entry
      addLog('ENTRY_SMART_LIMIT', `${signal.sym} ${side}: placing at ${rawEntry.toFixed(6)} (bybitPrice ${bybitPrice} ± 0.15 ATR ${atr.toFixed(6)})`);
    } else {
      // No ATR available — fall back to signal entry, or bybitPrice if entry missing
      rawEntry = signal.entry > 0 ? signal.entry : bybitPrice;
    }
  } else {
    rawEntry = bybitPrice;
  }
...
function canTradeSignal(signal, settings, trades) {
  ...
  // fix44e: use openedAt (when signal became PAPER_ACTIVE) not createdAt (when first detected)
  // Signal may wait minutes in WAITING_ENTRY before activating — createdAt was causing false rejections
  const maxAgeMs = Math.max(1, Number(settings.maxSignalAgeMinutes || 3)) * 60 * 1000;
  const sigTime = Number(signal.openedAt || signal.updatedAt || signal.createdAt || 0);
