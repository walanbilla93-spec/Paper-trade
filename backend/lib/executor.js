...
const ACTIVE_TRADE_STATES = new Set(['PENDING', 'PARTIALLY_FILLED', 'FILLED', 'OPEN']);
...
function canTradeSignal(signal, settings, trades) {
  ...
  if (settings.botMode === 'LIVE_REAL_BYBIT' && process.env.ALLOW_LIVE_TRADING !== 'true') {
    return { ok: false, reason: 'Live trading is locked. Set ALLOW_LIVE_TRADING=true to unlock.' };
  }
...
async function executeSignal(signalInput, opts = {}) {
  ...
  addLog('PLACE_ATTEMPT', `${plan.orderType} ${plan.bybitSide} ${plan.symbol} qty ${plan.qty}`, { plan, body });
  ...
  const trade = {
    ...
    status: plan.orderType === 'Market' ? 'FILLED' : 'PENDING',
  ...
  // Market orders usually fill immediately. Try a TP/SL position stop once.
