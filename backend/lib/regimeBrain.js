...
async function detectBTCRegime() {
  try {
    ...
    const emaPctSpread = price ? (Math.abs(ema20 - ema50) / price) * 100 : 0;
    const emasTight    = emaPctSpread < 0.3;   // EMAs nearly touching = range/chop
    const emasCrossing = emaPctSpread < 0.15;  // Very tight = possible breakout or chop
    ...
    if (breakoutUp && price > ema20) {
      ...
      // Overbought/oversold in unclear structure — treat as chop risk
      regime = 'CHOP';
