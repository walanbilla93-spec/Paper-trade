 ...
 */
function authorityOf(signal, trade) {
  if (!signal) return 'PAPER';
  // fixLINK: the presence of a RESOLVED TRADE RECORD is itself proof the exchange owns this signal.
  // Previously this checked only the signal's own claim fields (orderClaimAt/tradeId/orderId). Those
  // are written back asynchronously after placement — `getSignals() → mutate → saveSignals()` — while
  // the scan loop is concurrently saving the same array, so the writeback is routinely LOST to a
  // last-write-wins race. Result (BANKUSDT 07/18): order placed, filled and CLOSED on Bybit with a
  // real -0.288 loss, but the signal had tradeId/orderId/orderClaimAt ALL null, so authority resolved
  // to PAPER, decidePaper() ran on snapshot price, and the signal sat at LIVE_OPEN forever — it could
  // never book the real close no matter how long it waited.
  // findTradeForSignal() already matches by t.signalId === signal.id, which does NOT depend on the
  // writeback. Trusting that match here makes ownership immune to the race.
  if (trade) return 'BYBIT';
...
function decide(signal, trade, snapPrice, now, opts = {}) {
  const authority = authorityOf(signal, trade); // fixLINK: trade presence proves exchange ownership
