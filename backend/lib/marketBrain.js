...
// btcRegimeCache is now managed by regimeBrain.js — use getBTCRegime() to read it.
...
function qualityGate(leader, settings) {
  const reasons = [];
  const btcR = leader.btcRegime || 'UNKNOWN';  // fix24-a: new 6-state regime labels
  ...
  // fix24-a: log new regime on leader — blocking not enforced yet (fix24-b)
  // Old bull/bear checks still in place for backwards compatibility
  if (settings.rejectUnknownBtcRegime && (btcR === 'UNKNOWN')) reasons.push('BTC_REGIME_UNKNOWN');
  if (leader.side === 'BUY' && (btcR === 'BEAR_TREND' || leader.btcRegimeLegacy === 'bear')) reasons.push('BTC_BEAR_AGAINST_LONG');
  if (leader.side === 'SELL' && (btcR === 'BULL_TREND' || leader.btcRegimeLegacy === 'bull')) reasons.push('BTC_BULL_AGAINST_SHORT');
...
async function enrichTicker(t) {
  ...
  // fix24-a: detect local regime via regimeBrain
  const localRegimeData = detectLocalRegime(k5, k15);

  // fix24-a: read global BTC regime from cache (updated once per scan cycle)
  const btcCache = getBTCRegime();

  // fix24-a: compute allowed sides (observation only — not enforced until fix24-b)
  const sideAllowance = getAllowedSides(btcCache.regime, localRegimeData.localRegime);
  ...
  const leader = {
    ...
    // fix24-a: new 6-state BTC regime
    btcRegime: btcCache.regime,
    btcRegimeLabel: btcCache.label,
    btcRegimeLegacy: btcCache.trend15 === 'bull' ? 'bull' : btcCache.trend15 === 'bear' ? 'bear' : 'mixed',
    btcRsi: btcCache.rsi,
    btcTrend5: btcCache.trend5,
    btcTrend15: btcCache.trend15,
    btcTrend1h: btcCache.trend1h,
    btcVolMomentum: btcCache.volMomentum,

    // fix24-a: per-pair local regime
    localRegime: localRegimeData.localRegime,
    localRegimeLabel: localRegimeData.localLabel,
    localCandleBias: localRegimeData.candleBias,
    localMacdHist: localRegimeData.macdHist,

    // fix24-a: allowed-side analysis (observation only)
    regimeAllowedBuy: sideAllowance.allowedBuy,
    regimeAllowedSell: sideAllowance.allowedSell,
    regimeBuyStrength: sideAllowance.buyStrength,
    regimeSellStrength: sideAllowance.sellStrength,
    regimeBlockReason: sideAllowance.blockReason,
    ...
    source: 'backend-planner-v4.6.8.24a',
    updatedAt: Date.now(),
  };
...
async function scanOnce() {
  ...
  try {
    ...
    const settings = getSettings();
    // fix24-a: use new 6-state regime detection from regimeBrain
    await detectBTCRegime();
    const btcCache = getBTCRegime();
...
module.exports = { start, stop, scanOnce, getSnapshot, summarize, qualityGate, detectBTCRegime, getBTCRegime };
