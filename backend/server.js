...
app.use('/status', healthRoute);
app.use('/api/status', healthRoute);
...
app.listen(PORT, '0.0.0.0', () => {
  const s = getSettings();
  console.log(`[Orayan] Backend ${s.version || 'v4.3'} running on port ${PORT}`);
  console.log(`[Orayan] Mode: ${s.botMode} | Trading: ${s.tradingEnabled ? 'ENABLED' : 'disabled'} | Signals: backend-planner-v4.3`);
});
