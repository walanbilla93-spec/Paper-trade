...
router.get('/', (req, res) => {
  ...
  try {
    ...
    // fixMETA / fixSIZECAP verification
    const mf = tryRequire('../lib/metaFilter').mod;
    if (mf && typeof mf.getStats === 'function') { patchTags.push('fixMETA'); metaStats = mf.getStats(); }
    if (v4 && v4.RISK_DEFAULT_ACTIVE === 0.25) patchTags.push('fixSIZECAP');
  } catch (_e) {}
  let memory = null;
  try {
  ...
  res.json({
    ...
    metaStats,  // fixMETA: loaded? how many signals passed/blocked/errored
