...
router.get('/shadow-diag', auth, (req, res) => {
  ...
  // fix: use v4.getLedger() — the SAME source /ledger and /journal use. The raw 'signals' store
  // (store.read('signals', [])) is a different, much narrower internal store and came back
  // effectively empty in testing even when the ledger clearly had rangeTrapShadow populated.
  let ledger = [];
  try { ledger = v4.getLedger() || []; } catch (_e) {}
  const rows = [];
  ...
  res.json({ ok: true, source: 'v4.getLedger()[].diag', total: rows.length, rows });
});
