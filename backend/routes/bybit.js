...
// fix44b: Bybit execution test — place tiny order, check TP/SL, cancel immediately
router.post('/execution-test', auth, async (req, res) => {
  const results = { steps: [], passed: false };
  const log = (step, ok, detail) => {
    results.steps.push({ step, ok, detail });
    addLog(ok ? 'EXEC_TEST_OK' : 'EXEC_TEST_FAIL', `${step}: ${detail}`);
  };

  try {
    // Step 1: Get wallet balance
    const wallet = await bybitGet('/v5/account/wallet-balance', { accountType: 'UNIFIED' });
    const bal = Number(wallet.result?.list?.[0]?.totalWalletBalance || 0);
    if (bal < 1) { log('BALANCE', false, `Insufficient balance: ${bal} USDT`); return res.json({ ok: false, results }); }
    log('BALANCE', true, `${bal.toFixed(2)} USDT available`);

    // Step 2: Get BTCUSDT ticker for current price
    const ticker = await bybitGet('/v5/market/tickers', { category: 'linear', symbol: 'BTCUSDT' });
    const price = Number(ticker.result?.list?.[0]?.lastPrice || 0);
    if (!price) { log('TICKER', false, 'Could not get BTC price'); return res.json({ ok: false, results }); }
    log('TICKER', true, `BTC price: ${price}`);

    // Step 3: Set isolated margin
    const isoData = await bybitPost('/v5/position/switch-isolated', { category: 'linear', symbol: 'BTCUSDT', tradeMode: 1, buyLeverage: '2', sellLeverage: '2' });
    const isoOk = [0, 110026].includes(isoData.retCode);
    log('ISOLATED_MARGIN', isoOk, isoOk ? 'Isolated mode set' : `retCode ${isoData.retCode}: ${isoData.retMsg}`);

    // Step 4: Set leverage
    const levData = await bybitPost('/v5/position/set-leverage', { category: 'linear', symbol: 'BTCUSDT', buyLeverage: '2', sellLeverage: '2' });
    const levOk = [0, 110043].includes(levData.retCode);
    log('LEVERAGE', levOk, levOk ? '2x set' : `retCode ${levData.retCode}: ${levData.retMsg}`);

    // Step 5: Place tiny limit BUY far below market (won't fill)
    const testPrice = Math.round(price * 0.80 * 10) / 10; // 20% below market — safe, won't fill
    const testQty = '0.001'; // minimum BTC qty
    const tp = String(Math.round(testPrice * 1.02 * 10) / 10);
    const sl = String(Math.round(testPrice * 0.99 * 10) / 10);
    const orderData = await bybitPost('/v5/order/create', {
      category: 'linear', symbol: 'BTCUSDT', side: 'Buy',
      orderType: 'Limit', qty: testQty, price: String(testPrice),
      timeInForce: 'GTC', takeProfit: tp, stopLoss: sl,
      tpTriggerBy: 'LastPrice', slTriggerBy: 'LastPrice',
    });
    const orderId = orderData.result?.orderId;
    const orderOk = orderData.retCode === 0 && orderId;
    log('PLACE_ORDER', !!orderOk, orderOk ? `orderId: ${orderId} @ ${testPrice}` : `retCode ${orderData.retCode}: ${orderData.retMsg}`);
    if (!orderOk) return res.json({ ok: false, results });

    // Step 6: Check order has TP/SL
    await new Promise(r => setTimeout(r, 1500));
    const orderCheck = await bybitGet('/v5/order/realtime', { category: 'linear', symbol: 'BTCUSDT', orderId });
    const order = orderCheck.result?.list?.[0];
    const hasTP = Number(order?.takeProfit || 0) > 0;
    const hasSL = Number(order?.stopLoss || 0) > 0;
    log('TP_CHECK', hasTP, hasTP ? `TP: ${order.takeProfit}` : 'TP missing ❌');
    log('SL_CHECK', hasSL, hasSL ? `SL: ${order.stopLoss}` : 'SL missing ❌');

    // Step 7: Cancel the test order
    const cancelData = await bybitPost('/v5/order/cancel', { category: 'linear', symbol: 'BTCUSDT', orderId });
    const cancelOk = [0, 110001].includes(cancelData.retCode);
    log('CANCEL', cancelOk, cancelOk ? 'Test order cancelled ✅' : `retCode ${cancelData.retCode}: ${cancelData.retMsg}`);

    results.passed = orderOk && hasTP && hasSL && cancelOk;
    results.summary = results.passed
      ? '✅ All checks passed — execution working correctly'
      : `⚠️ Issues found — check steps above`;
